...
def filter_func(result_type=UInt(8), schedule=-1, use_uniforms=False):
    ...
    def process(raw, matrix_3200, matrix_7000, color_temp, gamma, contrast):
        ...
        raw2[x,y,c] = raw[x,y]
        print 'a'
        corrected = color_correct(raw2, matrix_3200, matrix_7000, color_temp)
        print 'b'
    ...
    if use_uniforms:
        ...
        matrix_3200[x,y] = 1.0 #select(y==0, select(x==0,  1.6697, select(x==1, -0.2693, select(x==2, -0.4004, -42.4346))),
        ...
        matrix_7000[x,y] = 1.0 #select(y==0, select(x==0,  2.2997, select(x==1, -0.4478, select(x==2,  0.1706, -39.0923))),
