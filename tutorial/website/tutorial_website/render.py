...
highlighted code with captured output/figures inlined) plus an index page
with a shared nav sidebar. No JS, no Bootstrap -- collapsible output blocks
are native <details>/<summary>.
"""
...
def _build_insertions(
    lesson: Lesson, snippets: dict[int, str], output_dir: Path
) -> dict[int, Insertion]:
    ...
    for line, figure_names in lesson.figure_lines.items():
...
def _render_blocks(
    ...
    start = 0  # 0-indexed offset into lesson.display_lines
    for line_no, insertion in sorted(insertions.items()):
        blocks.append(
            _highlight_chunk(lesson.display_lines[start:line_no], lesson.lexer)
        )
        blocks.append(insertion.output)
        blocks.extend(insertion.figures)
        start = line_no
    blocks.append(_highlight_chunk(lesson.display_lines[start:], lesson.lexer))
...
def render_assets(figures_dir: Path, output_dir: Path) -> None:
    ...
    one lesson: the stylesheet and the figures directory. Run once, up front,
    from a single process -- unlike lesson pages, these aren't safe to
    produce from several concurrent per-lesson render processes (concurrent
    writes/copies into the same shared paths would race)."""
...
def render_lesson_page(
    ...
    insertions = _build_insertions(lesson, snippets, output_dir)
    page = lesson_template.render(
        ...
        page=Page(
            ...
            blocks=_render_blocks(lesson, insertions),
        ),
        root="",
    )
