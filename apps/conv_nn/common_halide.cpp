...
Expr approx_log2(int q, const Expr &x, int q_x, const Type &type = Int(32)) {
...
Expr approx_exp2(int q, const Expr &x, const Expr &q_x, const Type &type = Int(32)) {
...
Expr approx_reciprocal(int q, const Expr &x, const Type &type = Int(32)) {
    //   precision / x
    // = precision / 2^log2(x)
    // = precision * 2^(-log2(x))
...
Expr approx_reciprocal_sqrt(int q, const Expr &x, const Type &type = Int(32)) {
    //   precision / sqrt(x)
...
Expr approx_log2p1_exp2(int q, const Expr &x, const Expr &q_x, const Type &type = Int(32)) {
    return approx_log2_exp2_plus_or_minus_one(q, x, 1, q_x, type);
}

Expr approx_log2m1_exp2(int q, const Expr &x, const Expr &q_x, const Type &type = Int(32)) {
    return approx_log2_exp2_plus_or_minus_one(q, x, -1, q_x, type);
}

const float log2_e = 1.442695f;

Expr approx_logistic(int q, const Expr &x, const Expr &q_x, const Type &type = Int(32)) {
...
Expr approx_tanh(int q, const Expr &x, const Expr &q_x, const Type &type = Int(32)) {
    // log2(e) is ~1.5, so to implement this, we quantize log2(e)/2, and adjust
    // q_x to compensate.
    const int log2_e_q = std::lround(log2_e * (1 << (x.type().bits() - 2)));
    Expr x2 = multiply_2x_high(x, cast(x.type(), log2_e_q));
