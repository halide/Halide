// Standalone round-trip check: the from-scratch Halide IQ4_XS dequantize
...
int main() {
    ...
    const ggml_type_traits *tt = ggml_get_type_traits(GGML_TYPE_IQ4_XS);
    const size_t out_bytes = ggml_row_size(GGML_TYPE_IQ4_XS, k);

    std::vector<uint8_t> ref_blocks(out_bytes), halide_blocks(out_bytes);
    tt->from_float_ref(x.data(), ref_blocks.data(), k);
    ggml_quants_halide_quantize_iq4_xs(x.data(), halide_blocks.data(), k);
    ...
    ggml_quants_halide_dequantize_iq4_xs(ref_blocks.data(), halide_y.data(), k);
