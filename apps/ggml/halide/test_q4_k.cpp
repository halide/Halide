// Standalone round-trip check: the from-scratch Halide Q4_K dequantize
...
int main() {
    ...
    const size_t out_bytes = ggml_row_size(GGML_TYPE_Q4_K, k);

    std::vector<uint8_t> ref_blocks(out_bytes), halide_blocks(out_bytes);
    tt->from_float_ref(x.data(), ref_blocks.data(), k);
    ggml_quants_halide_quantize_q4_k(x.data(), halide_blocks.data(), k);
    ...
    ggml_quants_halide_dequantize_q4_k(ref_blocks.data(), halide_y.data(), k);
