// Standalone round-trip check: the from-scratch Halide Q1_0 kernels (both
// directions fully native) vs GGML's own reference implementation, reached
// via the public ggml_get_type_traits() API.
...
int main() {
    const int64_t k = 4096;  // multiple of QK1_0 (128)

    std::vector<float> x(k);
    generate_synthetic_data(x.data(), k);

    const ggml_type_traits *tt = ggml_get_type_traits(GGML_TYPE_Q1_0);
    const size_t out_bytes = ggml_row_size(GGML_TYPE_Q1_0, k);

    std::vector<uint8_t> ref_blocks(out_bytes), halide_blocks(out_bytes);
    tt->from_float_ref(x.data(), ref_blocks.data(), k);
    ggml_quants_halide_quantize_q1_0(x.data(), halide_blocks.data(), k);
    ...
    ggml_quants_halide_dequantize_q1_0(ref_blocks.data(), halide_y.data(), k);
