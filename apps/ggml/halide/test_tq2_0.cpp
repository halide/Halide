// Standalone round-trip check: the from-scratch Halide TQ2_0 dequantize
...
// calls out to GGML's reference (see ggml_extern_quantize.cpp).
...
int main() {
    ...
    const ggml_type_traits *tt = ggml_get_type_traits(GGML_TYPE_TQ2_0);
    const size_t out_bytes = ggml_row_size(GGML_TYPE_TQ2_0, k);

    std::vector<uint8_t> ref_blocks(out_bytes), halide_blocks(out_bytes);
    tt->from_float_ref(x.data(), ref_blocks.data(), k);
    ggml_quants_halide_quantize_tq2_0(x.data(), halide_blocks.data(), k);
    ...
    ggml_quants_halide_dequantize_tq2_0(ref_blocks.data(), halide_y.data(), k);
