// Standalone check: the from-scratch Halide IQ2_XS dequantize kernel vs
// GGML's own reference implementation. GGML has no public from_float_ref
// for this importance-matrix-only codebook type (see
// ../providers/ggml_internal_abi.h's quantize_iq2_xs doc comment) -- its
// only quantizer is reached the same way ggml_provider.cpp reaches it: the
// private whole-matrix quantize_iq2_xs symbol, with a uniform (all-1.0)
// weighting since it hard-requires a non-null quant_weights.
...
int main() {
    ...
    const ggml_type_traits *tt = ggml_get_type_traits(GGML_TYPE_IQ2_XS);
    const size_t out_bytes = ggml_row_size(GGML_TYPE_IQ2_XS, k);

    std::vector<float> weights(k, 1.0f);
    std::vector<uint8_t> ref_blocks(out_bytes);
    quantize_iq2_xs(x.data(), ref_blocks.data(), /*nrows=*/1, /*n_per_row=*/k, weights.data());

    std::vector<float> ref_y(k), halide_y(k);
    tt->to_float(ref_blocks.data(), ref_y.data(), k);
    ggml_quants_halide_dequantize_iq2_xs(ref_blocks.data(), halide_y.data(), k);
