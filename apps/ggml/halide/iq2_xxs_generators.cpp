...
// vec_dot(IQ2_XXS, Q8_K): plain dequantize-then-multiply-and-sum, mirroring
// Q4_0's vec_dot generator (see q4_0_generators.cpp for the rationale).
class IQ2_XXSVecDotGenerator : public Generator<IQ2_XXSVecDotGenerator> {
public:
    ...
    void generate() {
        ...
        auto x_val = [&](Expr x) -> Expr {
            ...
            Expr grid_idx = x_blocks_(kQsOffset + ib32 * 8 + l, i);

            Expr a1b0 = cast<uint32_t>(x_blocks_(kQsOffset + ib32 * 8 + 4, i));
            Expr a1b1 = cast<uint32_t>(x_blocks_(kQsOffset + ib32 * 8 + 5, i));
            Expr a1b2 = cast<uint32_t>(x_blocks_(kQsOffset + ib32 * 8 + 6, i));
            Expr a1b3 = cast<uint32_t>(x_blocks_(kQsOffset + ib32 * 8 + 7, i));
            Expr aux32_1 = a1b0 | (a1b1 << 8) | (a1b2 << 16) | (a1b3 << 24);
...
HALIDE_REGISTER_GENERATOR(IQ2_XXSVecDotGenerator, iq2_xxs_vec_dot)
