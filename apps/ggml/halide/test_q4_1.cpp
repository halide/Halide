// Standalone round-trip check: the from-scratch Halide Q4_1 kernels vs
...
int main() {
    const int64_t k = 4096;  // multiple of QK4_1 (32)

    std::vector<float> x(k);
    generate_synthetic_data(x.data(), k);

    const ggml_type_traits *tt = ggml_get_type_traits(GGML_TYPE_Q4_1);
    const size_t out_bytes = ggml_row_size(GGML_TYPE_Q4_1, k);

    std::vector<uint8_t> ref_blocks(out_bytes), halide_blocks(out_bytes);
    tt->from_float_ref(x.data(), ref_blocks.data(), k);
    ggml_quants_halide_quantize_q4_1(x.data(), halide_blocks.data(), k);
    ...
    ggml_quants_halide_dequantize_q4_1(halide_blocks.data(), halide_y.data(), k);
