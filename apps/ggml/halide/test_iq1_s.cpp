// Standalone check: the from-scratch Halide IQ1_S dequantize kernel vs
// GGML's own reference implementation. GGML has no public from_float_ref
// for this importance-matrix-only codebook type (see
// ../providers/ggml_internal_abi.h's quantize_iq1_s doc comment) -- its
// only quantizer is reached the same way ggml_provider.cpp reaches it: the
// private whole-matrix quantize_iq1_s symbol, with a uniform (all-1.0)
...
int main() {
    ...
    const size_t out_bytes = ggml_row_size(GGML_TYPE_IQ1_S, k);

    std::vector<float> weights(k, 1.0f);
    std::vector<uint8_t> ref_blocks(out_bytes);
    quantize_iq1_s(x.data(), ref_blocks.data(), /*nrows=*/1, /*n_per_row=*/k, weights.data());

    std::vector<float> ref_y(k), halide_y(k);
    tt->to_float(ref_blocks.data(), ref_y.data(), k);
    ggml_quants_halide_dequantize_iq1_s(ref_blocks.data(), halide_y.data(), k);
