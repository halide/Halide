// Standalone round-trip check: the from-scratch Halide F16 kernels (both
...
int main() {
    ...
    const ggml_type_traits *tt = ggml_get_type_traits(GGML_TYPE_F16);
    const size_t out_bytes = ggml_row_size(GGML_TYPE_F16, k);

    std::vector<uint8_t> ref_blocks(out_bytes), halide_blocks(out_bytes);
    tt->from_float_ref(x.data(), ref_blocks.data(), k);
    ggml_quants_halide_quantize_f16(x.data(), halide_blocks.data(), k);
    ...
    ggml_quants_halide_dequantize_f16(ref_blocks.data(), halide_y.data(), k);
