...
int main(int argc, char **argv) {
  ...
  Func foo;
  foo(y) = minEnergy(y);
  foo(flipY - 1) = select(select( energy( clampW(foo(flipY) - 1), flipY - 1, 0) < energy( clampW(foo(flipY) + 1), flipY - 1, 0),
				  energy( clampW(foo(flipY) - 1), flipY - 1, 0),
				  energy( clampW(foo(flipY) + 1), flipY - 1, 0)
				  ) < 
			  energy( clampW(foo(flipY)), flipY - 1, 0),
			  select( energy( clampW(foo(flipY) - 1), flipY - 1, 0) < energy( clampW(foo(flipY) + 1), flipY - 1, 0),
				  clampW(foo(flipY) - 1),
				  clampW(foo(flipY) + 1)
				  ),
			  clampW(foo(flipY))
  ...
  outputClamped.compileToFile("local_laplacian");
  return 0;
}
