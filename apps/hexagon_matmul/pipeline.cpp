...
public:
    ...
    void generate() {
        ...
        Func AB("AB");
        AB(x, y) = u32(0);
        AB(x, y) += u32(u16(A(4 * rk + 0, y)) * u16(B_swizzled(x, rk, 0))) +
        ...
        schedule = [=]() mutable {
            ...
            if (use_hexagon) {
                ...
                AB.compute_at(output, yo).vectorize(x).unroll(y);

                AB.update(0).reorder(x, y, rk).vectorize(x).unroll(y).unroll(
                    rk, k_unroll_factor);
                ...
                AB.compute_root().vectorize(x, vector_size_u32);
                ...
                AB.update(0)
                    ...
                    .split(rk, rk, rki, kBlockSize / k_split_factor,
