...
public:
    ...
    void generate() {
        ...
        Func blur_y{ "blur_y" };
        blur_y(x, y, c) =
            blur5(input_bounded(x, y - 2, c), input_bounded(x, y - 1, c),
                  input_bounded(x, y, c), input_bounded(x, y + 1, c),
                  input_bounded(x, y + 2, c));

        blur(x, y, c) =
            blur5(blur_y(x - 2, y, c), blur_y(x - 1, y, c), blur_y(x, y, c),
                  blur_y(x + 1, y, c), blur_y(x + 2, y, c));
        ...
        schedule = [=]() mutable {
            ...
            if (get_target().features_any_of({ Target::HVX_64, Target::HVX_128 })) {
                ...
                for (int i = 1; i < 3; i++) {
                    input.dim(i).set_stride((input.dim(i).stride() / vector_size) *
                                            vector_size);
                    blur.dim(i).set_stride((blur.dim(i).stride() / vector_size) *
                                           vector_size);
                }
            } else {
                const int vector_size = natural_vector_size<uint8_t>();

                Func(blur).compute_root().parallel(y, 16).vectorize(x, vector_size);
                blur_y.compute_at(blur, y).vectorize(x, vector_size);
            }
        };
    }
