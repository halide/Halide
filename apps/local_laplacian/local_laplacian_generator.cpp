...
public:
    ...
    Func build() {
        ...
        Func gray;
        gray(x, y) = 0.299f * floating(x, y, 0) + 0.587f * floating(x, y, 1) +
        ...
        gPyramid[0](x, y, k) =
            beta * (gray(x, y) - level) + level + remap(idx - 256 * k);
        for (int j = 1; j < J; j++) {
        ...
        for (int j = J - 2; j >= 0; j--) {
            lPyramid[j](x, y, k) =
                gPyramid[j](x, y, k) - upsample(gPyramid[j + 1])(x, y, k);
        }
        ...
        for (int j = 0; j < J; j++) {
            ...
            outLPyramid[j](x, y) =
                (1.0f - lf) * lPyramid[j](x, y, li) + lf * lPyramid[j](x, y, li + 1);
        }
        ...
        for (int j = J - 2; j >= 0; j--) {
            outGPyramid[j](x, y) =
                upsample(outGPyramid[j + 1])(x, y) + outLPyramid[j](x, y);
        }
        ...
        output(x, y, c) =
            cast<uint16_t>(clamp(color(x, y, c), 0.0f, 1.0f) * 65535.0f);
        ...
        if (get_target().has_gpu_feature()) {
            ...
            for (int j = 0; j < J; j++) {
                ...
                if (j > 0) {
                    ...
                    gPyramid[j].compute_root().reorder(k, x, y).gpu_tile(x, y, xi, yi,
                                                                         blockw, blockh);
                }
...
private:
    ...
    Func downsample(Func f) {
        ...
        Func downx, downy;
        downx(x, y, _) =
            (f(2 * x - 1, y, _) + 3.0f * (f(2 * x, y, _) + f(2 * x + 1, y, _)) +
             f(2 * x + 2, y, _)) /
            8.0f;
        downy(x, y, _) = (downx(x, 2 * y - 1, _) +
    ...
    Func upsample(Func f) {
        ...
        Func upx, upy;
        upx(x, y, _) =
            0.25f * f((x / 2) - 1 + 2 * (x % 2), y, _) + 0.75f * f(x / 2, y, _);
        upy(x, y, _) =
            0.25f * upx(x, (y / 2) - 1 + 2 * (y % 2), _) + 0.75f * upx(x, y / 2, _);
        return upy;
    }
};
