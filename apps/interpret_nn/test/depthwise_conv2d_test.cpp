...
struct DepthwiseConv2D_ReferenceOp : public op_test::ReferenceOp {
    ...
    void execute() override {
        ...
        output_buf.for_each_element([&](int c, int x, int y, int b) {
            int32_t output_value = bias_buf(c);
            ...
            for (int filter_y = 0; filter_y < filter_height; filter_y++) {
                for (int filter_x = 0; filter_x < filter_width; filter_x++) {
                    ...
                    if ((x_offset >= 0) && (x_offset < input_width) && (y_offset >= 0) && (y_offset < input_height)) {
                        const int32_t input_value = (int32_t)input_buf(input_c, x_offset, y_offset, b) - input_offset;
                        const int32_t filter_value = (int32_t)filter_buf(c, filter_x, filter_y, b) - filter_offset;
            ...
            // an integer version of DepthwiseConv2D; for now, we just replicate the shift-and-add behavior
