...
namespace Internal {
  ...
  struct BoundContents {
      ...
      std::pair<int64_t, int64_t> *data() const {
          // This struct is a header
          return (std::pair<int64_t, int64_t> *)(const_cast<BoundContents *>(this) + 1);
      }

      std::pair<int64_t, int64_t> &region_required(int i) {
          return data()[i];
      }

      std::pair<int64_t, int64_t> &region_computed(int i) {
          return data()[i + layout->computed_offset];
      }

      std::pair<int64_t, int64_t> &loops(int i, int j) {
          return data()[j + layout->loop_offset[i]];
      }


      const std::pair<int64_t, int64_t> &region_required(int i) const {
          return data()[i];
      }

      const std::pair<int64_t, int64_t> &region_computed(int i) const {
          return data()[i + layout->computed_offset];
      }

      const std::pair<int64_t, int64_t> &loops(int i, int j) const {
          return data()[j + layout->loop_offset[i]];
      }
      ...
      struct Layout {
          ...
          void allocate_some_more() const {
              size_t size_of_one = sizeof(BoundContents) + total_size * sizeof(std::pair<int64_t, int64_t>);
...
struct FunctionDAG {
    ...
    struct Node {
        ...
        std::vector<Interval> region_required;

        // A concrete region required from a bounds estimate. Only
        // defined for outputs.
        std::vector<std::pair<int64_t, int64_t>> estimated_region_required;
        ...
        void required_to_computed(const std::pair<int64_t, int64_t> *required,
                                  std::pair<int64_t, int64_t> *computed) const {
        ...
        void loop_nest_for_region(int stage_idx,
                                  const std::pair<int64_t, int64_t> *computed,
                                  std::pair<int64_t, int64_t> *loop) const {
        ...
        std::vector<const Edge *> outgoing_edges, incoming_edges;
    ...
    struct Edge {
        ...
        // points required.
        void expand_footprint(const std::pair<int64_t, int64_t> *consumer_loop,
                              std::pair<int64_t, int64_t> *producer_required) const {
    ...
    FunctionDAG(const std::vector<Function> &outputs, const MachineParams &params, const Target &target) {
        ...
        for (size_t i = order.size(); i > 0; i--) {
            ...
            std::string innermost_storage_dim;
            ...
            for (int s = 0; s <= (int)consumer.updates().size(); s++) {
                ...
                if (node.is_output) {
                    ...
                    std::map<std::string, std::pair<int64_t, int64_t>> estimates;
    ...
    class Featurizer : public IRVisitor {
        ...
        // value.
        DerivativeResult differentiate(const Expr &e, const std::string &v) {
        ...
        void visit_memory_access(Type t, const std::vector<Expr> &args, PipelineFeatures::AccessType type) {
            // Compute matrix of partial derivatives of args w.r.t. loop params
            std::vector<std::vector<Expr>> matrix;
            std::vector<size_t> ones_per_row(args.size(), 0),
    ...
    public:
        ...
        void visit_store_args(Type t, std::vector<Expr> args) {
            for (auto &e : args) {
    ...
    void dump() const {
        for (const Node &n : nodes) {
            ...
            for (size_t i = 0; i < n.stages.size(); i++) {
                ...
                n.stages[i].features.dump(debug(0));
            }
        }
