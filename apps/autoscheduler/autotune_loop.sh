...
make_sample() {
    ...
    if [[ $D == */0 ]]; then
        ...
        HL_MACHINE_PARAMS=16,1,1 HL_SEED=${2} HL_FEATURE_FILE=${D}/sample.sample HL_WEIGHTS_DIR=${PWD}/weights HL_RANDOM_DROPOUT=100 HL_BEAM_SIZE=50 ${GENERATOR} -g ${PIPELINE} -o ${D} target=${HL_TARGET} auto_schedule=true -p bin/auto_schedule.so 2> ${D}/compile_log.txt
    else
        # The other samples are random probes biased by the cost model
        HL_MACHINE_PARAMS=16,1,1 HL_SEED=${2} HL_FEATURE_FILE=${D}/sample.sample HL_WEIGHTS_DIR=${PWD}/weights HL_RANDOM_DROPOUT=90 HL_BEAM_SIZE=1 ${GENERATOR} -g ${PIPELINE} -o ${D} target=${HL_TARGET} auto_schedule=true -p bin/auto_schedule.so 2> ${D}/compile_log.txt
    fi
...
benchmark_sample() {
    D=${1}
    HL_NUM_THREADS=16 ${D}/bench --output_extents=estimate --default_input_buffers=random:0:estimate_then_auto --default_input_scalars=estimate --benchmarks=all --benchmark_min_time=1 | tee ${D}/bench.txt
...
for ((i=$((FIRST+1));i<1000000;i++)); do
    ...
    find samples | grep sample$ | HL_WEIGHTS_DIR=weights ./bin/train_cost_model 100
