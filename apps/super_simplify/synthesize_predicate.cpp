...
struct System {
    ...
    void add_non_linear_term(const Expr &e) {
        ...
        if (is_const_zero(e) || !non_linear_term.defined()) {
            non_linear_term = e;
        } else {
    ...
    bool can_prove(Expr e) {
        return (simplifier->truths.count(e) ||
                ...
                is_const_one(simplifier->mutate(e, nullptr)));
    }
    ...
    void add_term(const Expr &e) {
        ...
        if (eq && eq->a.type() == Int(32)) {
            ...
            if (const Min *min_b = le->b.as<Min>()) {
                ...
                simplifier->learn_permanent_truth(e);
            } else if (is_const(le->b) && va) {
                simplifier->learn_permanent_truth(e);
            } else {
                Expr v = aux();
                simplifier->learn_permanent_truth(0 <= v);
            ...
            if (l->value.type().is_bool()) {
                ...
                simplifier->learn_permanent_truth(eq);
                add_term(eq);
                add_term(l->body);
            }
    ...
    bool infeasible() const {
        ...
        // simplifier instance.
        for (auto &e : equalities) {
            if (is_const_zero(simplifier->mutate(e.to_expr(), nullptr))) {
                return true;
            }
        }
        if (non_linear_term.defined() && is_const_zero(simplifier->mutate(non_linear_term, nullptr))) {
            return true;
        }
        return false;
    }
            ...
            if (lhs.defined()) {
                {
                    ...
                    simplifier->learn_permanent_truth(0 <= k1);
                    if (!abs_rhs.defined()) {
                        abs_rhs = max(rhs, -rhs);
                    }
        ...
        for (auto p : nonlinear_terms) {
            if (p.second > 1) {
                ...
                if (bounds.min_defined) {
                    simplifier->learn_permanent_truth(t >= (int)bounds.min);
                }
                if (bounds.max_defined) {
                    simplifier->learn_permanent_truth(t <= (int)bounds.max);
                }
            ...
            for (const auto &p : equalities[i].terms) {
                ...
                if (var) {
                    ...
                    if (!equal(p.first, rhs)) {
                        simplifier->learn_permanent_truth(p.first == rhs);
                    }
...
class RemoveMinMax : public IRMutator {
    ...
    Expr visit(const Select *op) override {
        if (const LT *lt = op->condition.as<LT>()) {
            ...
            } else if (is_const_zero(lt->b) && var_a) {
...
bool can_disprove_nonconvex(Expr e, int beam_size, Expr *implication) {
    ...
    for (const Expr &p : pieces) {
        ...
        for (Expr c : clauses) {
            ...
            simplifier.learn_permanent_truth(c);
            if (is_const_zero(c)) {
...
Expr synthesize_predicate(const Expr &lhs,
    ...
    class NormalizePrecondition : public IRMutator {
        ...
        Expr visit(const LE *op) override {
            ...
            if (is_const_zero(op->a) && can_prove(op->a != op->b)) {
                return mutate(1 <= op->b);
            } else if (is_const_zero(op->b) && can_prove(op->a != op->b)) {
            ...
            } else if (mul_a && is_const_zero(op->b)) {
                return mutate((mul_a->a <= 0 && 0 <= mul_a->b) ||
                              (mul_a->b <= 0 && 0 <= mul_a->a));
            } else if (mul_b && is_const_zero(op->a)) {
    ...
    Simplify simplifier(true, nullptr, nullptr);
    simplifier.learn_permanent_truth(implicit_assumption);
    ...
    while (changed) {
        ...
        for (size_t i = 0; i < cnf.size() && !changed; i++) {
            auto &c1 = cnf[i];
            if (c1.size() == 1 && is_const_one(*c1.begin())) continue;
            ...
            // Optimize the clause in isolation
            {
                ...
                for (size_t i = 0; i < terms.size(); i++) {
                    ...
                    for (size_t j = 0; j < terms.size(); j++) {
                        if (i == j) continue;
                        simplifier.learn_permanent_falsehood(terms[j]);
                        simplifier.learn_permanent_truth(simplify(!terms[j]));
                    }
                ...
                std::copy_if(terms.begin(), terms.end(),
                             ...
                             [](const Expr &e) { return !is_const_zero(e); });
            ...
            for (size_t j = i + 1; j < cnf.size() && !changed; j++) {
                auto &c2 = cnf[j];
                if (c2.size() == 1 && is_const_one(*c2.begin())) continue;
        ...
        if (!changed) {
            ...
            for (const auto &t : all_terms_in_a_disjunction) {
                ...
                if (result == Z3Result::Unsat) {
                    Simplify simplifier(true, nullptr, nullptr);
                    simplifier.learn_permanent_truth(implicit_assumption);
                    simplifier.learn_permanent_truth(t);
    ...
    for (auto p : find_vars(rhs)) {
        ...
        for (auto t : terms) {
            ...
            if (mul_a && !is_const_one(simplifier.mutate(mul_a->b >= 0, nullptr))) {
