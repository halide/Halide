...
public:
    ...
    int64_t net_nsec() {
        return running_ ? 0 : time_net_;
    }
    double net_usec() {
        return running_ ? 0 : (double) time_net_ / 1e3;
    }
    double net_msec() {
        return running_ ? 0 : (double) time_net_ / 1e6;
    }
    double net_sec() {
        return running_ ? 0 : (double) time_net_ / 1e9;
    }
    ...
    for (int i0 = 0; i0 < std::max(1, buf->extent[0]); ++i0) {
        ...
        for (int i1 = 0; i1 < std::max(1, buf->extent[1]); ++i1) {
            ...
            for (int i2 = 0; i2 < std::max(1, buf->extent[2]); ++i2) {
                ...
                switch (i2) {
                case 0:
                    v = i0 * kMax / std::max(1, buf->extent[0]);
                    break;
                case 1:
                    v = i1 * kMax / std::max(1, buf->extent[1]);
                    break;
                case 2:
                    v = (atan2(i1, i0) + seed) * kMax / 3.1415926535f;
                    break;
                default:
                    v = kMax;
                    break;
