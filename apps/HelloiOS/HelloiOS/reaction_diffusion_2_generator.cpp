...
class ReactionDiffusion2Init
    : public Halide::Generator<ReactionDiffusion2Init> {
public:
    ...
    void schedule() {
        if (get_target().has_gpu_feature()) {
            Func(output).reorder(c, x, y).bound(c, 0, 3).vectorize(c).gpu_tile(x, y,
                                                                               4, 4);
            output.dim(0).set_stride(3).dim(2).set_bounds(0, 3).set_stride(1);
        }
    }
...
class ReactionDiffusion2Update
    : public Halide::Generator<ReactionDiffusion2Update> {
public:
    ...
    void generate() {
        ...
        blur_x(x, y, c) =
            (clamped(x - 3, y, c) + clamped(x - 1, y, c) + clamped(x, y, c) +
             clamped(x + 1, y, c) + clamped(x + 3, y, c));
        blur_y(x, y, c) =
            (clamped(x, y - 3, c) + clamped(x, y - 1, c) + clamped(x, y, c) +
             clamped(x, y + 1, c) + clamped(x, y + 3, c));
        ...
        new_state(x, y, c) = select(c == 0, R, c == 1, G, B);
        ...
        new_state(clobber.x, clobber.y, c) =
            select(radius < 400.0f, 1.0f, new_state(clobber.x, clobber.y, c));
    }

    void schedule() {
        state.dim(2).set_bounds(0, 3);
        Func(new_state).reorder(c, x, y).bound(c, 0, 3).unroll(c);

        if (get_target().has_gpu_feature()) {
            blur.reorder(c, x, y).vectorize(c).compute_at(new_state,
                                                          Var::gpu_threads());

            Func(new_state).gpu_tile(x, y, 8, 2);

            for (int i = 0; i <= 1; ++i) {
                Func(new_state).update(i).reorder(c, x).unroll(c).gpu_tile(x, 8);
            }
            for (int i = 2; i <= 3; ++i) {
                Func(new_state).update(i).reorder(c, y).unroll(c).gpu_tile(y, 8);
            }
            Func(new_state)
                .update(4)
                .reorder(c, clobber.x)
            ...
            state.dim(0).set_stride(3).dim(2).set_stride(1).set_extent(3);
            new_state.dim(0).set_stride(3).dim(2).set_stride(1).set_extent(3);
        } else {
            Var yi;
            Func(new_state)
                .split(y, y, yi, 64)
            ...
            blur.compute_at(new_state, yi).vectorize(x, natural_vector_size<float>());
...
HALIDE_REGISTER_GENERATOR(ReactionDiffusion2Update,
                          "reaction_diffusion_2_update")

class ReactionDiffusion2Render
    : public Halide::Generator<ReactionDiffusion2Render> {
public:
    Input<Buffer<float>> state{ "state", 3 };
    // TODO(srj): should be Input<bool>; using Input<int> to work around Issue
    // #1760
    ...
    void generate() {
        ...
        // make this efficient.
        Expr bgra = select(c == 0, cast<uint8_t>(B * 255), c == 1,
                           cast<uint8_t>(G * 255), c == 2, cast<uint8_t>(R * 255),
                           /*c==3*/ cast<uint8_t>(A * 255));

        Expr rgba = select(c == 0, cast<uint8_t>(R * 255), c == 1,
                           cast<uint8_t>(G * 255), c == 2, cast<uint8_t>(B * 255),
                           /*c==3*/ cast<uint8_t>(A * 255));
    ...
    void schedule() {
        render.dim(0).set_stride(4).dim(2).set_stride(1).set_bounds(0, 4);
        if (get_target().has_gpu_feature()) {
            state.dim(0).set_stride(3).dim(2).set_stride(1).set_bounds(0, 3);
            Func(render).reorder(c, x, y).unroll(c).gpu_tile(x, y, 32, 4);
        } else {
            Var yi;
            Func(render)
                .reorder(c, x, y)
...
HALIDE_REGISTER_GENERATOR(ReactionDiffusion2Render,
