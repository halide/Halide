...
public:
  ...
  const WeightShape avg_pool_ws = {16, 5, 5, 3, 3};
  ...
  void generate() {
    ...
    upsampled.f(c, x, y, n) = select( (x % 3) == 0 || (x % 3) == 2, 
                                select( (y % 3) == 0 || (y % 3) == 2, g_lowres_conv1x1_2.f(c, x/3+1, y/3+1, n), g_lowres_conv1x1_2.f(x/3+1, y/3, n)),
                                  select( (y % 3) == 0 || (y % 3) == 2, g_lowres_conv1x1_2.f(c, x/3, y/3+1, n), g_lowres_conv1x1_2.f(x/3, y/3, n)));

    stacked.shape = upsampled.shape;
    stacked.shape[0] *= 2;
    stacked.f(c, x, y, n) = select( c < 16, upsampled.f(c, x, y, n), g_conv2d.f(c-16, x, y, n));
    ...
    g_lowres_conv2d_weights.dim(0).set_estimate(0, 16)
      .dim(1).set_estimate(0,5)
      .dim(2).set_estimate(0,5)
      .dim(3).set_estimate(0,1);
    g_lowres_1x1_1_weights.dim(0).set_estimate(0, 16)
      .dim(1).set_estimate(0,1)
      .dim(2).set_estimate(0,1)
      .dim(3).set_estimate(0,16);
    g_lowres_1x1_2_weights.dim(0).set_estimate(0, 16)
      .dim(1).set_estimate(0,1)
      .dim(2).set_estimate(0,1)
      .dim(3).set_estimate(0,16);
    ...
    downsampled.f.bound(downsampled.f.args()[0], 0, 1);
    downsampled.f.bound(downsampled.f.args()[1], 0, 44);
    downsampled.f.bound(downsampled.f.args()[2], 0, 44);
    downsampled.f.bound(downsampled.f.args()[3], 0, 32);

    upsampled.f.bound(upsampled.f.args()[0], 0, 16);
    upsampled.f.bound(upsampled.f.args()[1], 0, 128);
    upsampled.f.bound(upsampled.f.args()[2], 0, 128);
    upsampled.f.bound(upsampled.f.args()[3], 0, 32);

    stacked.f.bound(stacked.f.args()[0], 0, 32);
    stacked.f.bound(stacked.f.args()[1], 0, 128);
    stacked.f.bound(stacked.f.args()[2], 0, 128);
    stacked.f.bound(stacked.f.args()[3], 0, 32);
