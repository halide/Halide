...
message(STATUS "Found ONNX: ${ONNX_DIR} (found version \"${ONNX_VERSION}\")")
...
pybind11_add_module(model_cpp model.cpp benchmarking_utils.h common_types.h denormal_disabler.h)
...
add_test(
    ...
    COMMAND
        ${Python_EXECUTABLE} -m unittest ${CMAKE_CURRENT_SOURCE_DIR}/halide_as_onnx_backend_test.py
    WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
)
set_tests_properties(
    halide_as_onnx_backend_test model_test
    PROPERTIES
    LABELS onnx
    PASS_REGULAR_EXPRESSION "OK"
    SKIP_REGULAR_EXPRESSION "\\[SKIP\\]"
    ENVIRONMENT
        "PYTHONPATH=$<TARGET_FILE_DIR:model_cpp>;MODEL_AUTOSCHEDULER=$<TARGET_FILE:Halide::Adams2019>"
