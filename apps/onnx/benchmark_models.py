...
def main():
    ...
    for f in args.files:
        ...
        if False:
            ...
            for node in graph_def.node:
                if node.op == 'Placeholder':
                    ...
                    #n.device = '/device:gpu:0'
                    new_graph_def.node.extend([n])
        ...
        func = m.__dict__[model_name + '_li2018']
        func(*halide_inputs, *halide_outputs)
        for j in halide_outputs:
            j.device_sync()
        num_iter = 20
        min_time = 1e20
        for i in range(num_iter):
            ...
            t = ((end - beg) / (10 ** 9))
            min_time = min(t, min_time)
        print('{}: Gradient autoscheduler: {}s'.format(model_name, min_time))

        # Compile the generators
        print('Compiling with the Sioutas2020 autoscheduler')
        cmd = ('bin/host/onnx_converter.generator '
               '-e object,python_extension '
               '-g onnx_model_generator '
               '-o bin/host-cuda '
               '-f {}_sioutas2020 '
               '-p ../sioutas2020/bin/libauto_schedule.so '
               '-s Sioutas2020 '
               'target=host-cuda '
               'auto_schedule=true '
               'model_file_path={}').format(model_name, f)
        ...
        cmd = 'g++ ' + \
              ...
              '-I../../distrib/include ' + \
              '-std=c++11 ' + \
              '-c -o bin/host-cuda/tmp.o bin/host-cuda/{}_sioutas2020.py.cpp'.format(model_name)
        print(cmd)
        os.system(cmd)
        # Link
        cmd = 'g++ ' + \
              python_cflags + \
              '-std=c++11 ' + \
              '-shared -o bin/host-cuda/{}_sioutas2020.so bin/host-cuda/tmp.o bin/host-cuda/{}_sioutas2020.o'.format(
                      model_name, model_name)
        print(cmd)
        os.system(cmd)
        m = __import__(model_name + '_sioutas2020')
        func = m.__dict__[model_name + '_sioutas2020']
        func(*halide_inputs, *halide_outputs)
        for j in halide_outputs:
            j.device_sync()
        num_iter = 20
        ...
        for i in range(num_iter):
            ...
            t = ((end - beg) / (10 ** 9))
            min_time = min(t, min_time)
        print('{}: Sioutas autoscheduler: {}s'.format(f, min_time))
