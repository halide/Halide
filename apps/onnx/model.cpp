...
void PrintLoopNest(HalideModel pipeline) {
  pipeline.rep->print_loop_nest();
}

void PrintLoweredStatement(HalideModel pipeline) {
...
PYBIND11_MODULE(model_cpp, m) {
  ...
  m.def("PrintLoopNest", &PrintLoopNest, "Print a high level representation of the loop nest");
  m.def("PrintLoweredStatement", &PrintLoweredStatement, "Print a detailed representation of the lowered code");
}
