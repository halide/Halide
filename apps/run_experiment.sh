...
for ((SEED=${FIRST_SEED};SEED<${LAST_SEED};SEED++)); do
    ...
    HL_USE_PUSH_DIVISION=1 \
    HL_PERMIT_FAILED_UNROLL=1 \
    HL_SEED=${SEED} \
    HL_RANDOM_DROPOUT=1 \
    HL_BEAM_SIZE=1 \
    HL_DEBUG_CODEGEN=0 \
    ./bin/host/${APP}.generator -g ${APP} -e stmt,static_library,h,assembly,registration,compiler_log,llvm_assembly -o results/${SEED} -p ../../distrib/lib/libautoschedule_adams2019.dylib target=host-no_runtime-disable_llvm_loop_opt auto_schedule=true -s Adams2019  > results/${SEED}/stdout.txt 2> results/${SEED}/stderr.txt  &

    HL_USE_PUSH_DIVISION=0 \
    HL_PERMIT_FAILED_UNROLL=1 \
    HL_SEED=${SEED} \
    HL_RANDOM_DROPOUT=1 \
    HL_BEAM_SIZE=1 \
