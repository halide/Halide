...
Func CameraPipe::deinterleave(Func raw) {
    ...
    deinterleaved(x, y, c) =
        select(c == 0, raw(2 * x, 2 * y), c == 1, raw(2 * x + 1, 2 * y), c == 2,
...
std::pair<Func, Scheduler> CameraPipe::demosaic(Func deinterleaved) {
    ...
    Func output;
    output(x, y, c) = select(c == 0, r(x, y), c == 1, g(x, y), b(x, y));
...
Func CameraPipe::color_correct(Func input) {
    ...
    Expr r =
        matrix(3, 0) + matrix(0, 0) * ir + matrix(1, 0) * ig + matrix(2, 0) * ib;
    Expr g =
        matrix(3, 1) + matrix(0, 1) * ir + matrix(1, 1) * ig + matrix(2, 1) * ib;
    Expr b =
        matrix(3, 2) + matrix(0, 2) * ir + matrix(1, 2) * ig + matrix(2, 2) * ib;

    r = cast<int16_t>(r / 256);
    g = cast<int16_t>(g / 256);
    b = cast<int16_t>(b / 256);
    corrected(x, y, c) = select(c == 0, r, c == 1, g, b);
...
Func CameraPipe::apply_curve(Func input) {
    ...
    Expr z =
        select(g > 0.5f, 1.0f - (a * (1.0f - g) * (1.0f - g) + b * (1.0f - g)),
               a * g * g + b * g);
    ...
    if (lutResample == 1) {
        ...
        curved(x, y, c) = cast<uint8_t>(
            (cast<uint16_t>(y0) * lutResample + (y1 - y0) * u) / lutResample);
    }
...
Func CameraPipe::build() {
    ...
    Func demosaiced;
    Scheduler demosaiced_scheduler;
    ...
    // We can generate slightly better code if we know the splits divide the
    // extent.
