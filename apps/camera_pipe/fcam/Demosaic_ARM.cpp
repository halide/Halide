...
extern void makeLUT(float contrast, int blackLevel, int whiteLevel, float gamma,
                    unsigned char *lut);
...
void demosaic_ARM(Halide::Runtime::Buffer<uint16_t> input,
...
// For each block in the input
    ...
    if (1) {
        ...
        for (int y = 0; y < VEC_HEIGHT; y++) {
            for (int x = 0; x < VEC_WIDTH / 2; x++) {
                ...
                asm volatile("vld2.16  {d6-d9}, [%[rawPtr]]! \n\t"
                             "vld2.16  {d10-d13}, [%[rawPtr2]]! \n\t"
                             "vst1.16  {d6-d7}, [%[g_gr_ptr]]! \n\t"
                             "vst1.16  {d8-d9}, [%[r_r_ptr]]! \n\t"
                             "vst1.16  {d10-d11}, [%[b_b_ptr]]! \n\t"
                             "vst1.16  {d12-d13}, [%[g_gb_ptr]]! \n\t"
                             : [rawPtr] "+r"(rawPtr), [rawPtr2] "+r"(rawPtr2),
                               [g_gr_ptr] "+r"(g_gr_ptr), [r_r_ptr] "+r"(r_r_ptr),
                               [b_b_ptr] "+r"(b_b_ptr), [g_gb_ptr] "+r"(g_gb_ptr)::"d6",
                               "d7", "d8", "d9", "d10", "d11", "d12", "d13", "memory");
            }
  ...
  */
    ...
    */
        ...
        for (int y = 2; y < VEC_HEIGHT - 2; y++) {
            for (int x = 0; x < VEC_WIDTH; x++) {
...

                {
                    // red at green
                    int16x4_t r_gr =
                        vadd_s16(vhadd_s16(r_left, r_here),
                                 vsub_s16(gr_here, vhadd_s16(g_r_left, g_r_here)));
                    int16x4_t r_gb =
                        vadd_s16(vhadd_s16(r_here, r_down),
                                 vsub_s16(gb_here, vhadd_s16(g_r_down, g_r_here)));
...

                {
                    ...
                    int16x4_t rp_b =
                        vadd_s16(vhadd_s16(r_downleft, r_here),
                                 vsub_s16(g_b_here, vhadd_s16(g_r_downleft, g_r_here)));
                    int16x4_t rn_b =
                        vadd_s16(vhadd_s16(r_left, r_down),
                                 vsub_s16(g_b_here, vhadd_s16(g_r_left, g_r_down)));
...

                {
                    // blue at green
                    int16x4_t b_gr =
                        vadd_s16(vhadd_s16(b_up, b_here),
                                 vsub_s16(gr_here, vhadd_s16(g_b_up, g_b_here)));
                    int16x4_t b_gb =
                        vadd_s16(vhadd_s16(b_here, b_right),
                                 vsub_s16(gb_here, vhadd_s16(g_b_right, g_b_here)));
...

                {
                    ...
                    int16x4_t bp_r =
                        vadd_s16(vhadd_s16(b_upright, b_here),
                                 vsub_s16(g_r_here, vhadd_s16(g_b_upright, g_b_here)));
                    int16x4_t bn_r =
                        vadd_s16(vhadd_s16(b_right, b_up),
                                 vsub_s16(g_r_here, vhadd_s16(g_b_right, g_b_up)));
    ...
    // Stage 10)
    if (1) {
        // Color-correct and save the results into a 16-bit buffer for gamma
        // correction
    ...
    if (1) {
        ...
        for (int y = 0; y < BLOCK_HEIGHT; y++) {
            unsigned int *__restrict__ outPtr32 =
                (unsigned int *) (outBlockPtr + y * outWidth * 3);
            for (int x = 0; x < (BLOCK_WIDTH * 3) / 4; x++) {
                unsigned val = ((lut[out16Ptr[0]] << 0) | (lut[out16Ptr[1]] << 8) |
                                (lut[out16Ptr[2]] << 16) | (lut[out16Ptr[3]] << 24));
                *outPtr32++ = val;
                out16Ptr += 4;
                // *outPtr++ = lut[*out16Ptr++];
            }
