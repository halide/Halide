...
MACHINE_PARAMS_SIZE="32 64 96 128 256 512 1024"
...
if [ "${EXPLORE_MACHINE_PARAMS}" -ne 0 ]; then
	for i in ${MACHINE_PARAMS_SIZE}; do
		...
		grep "Classic auto-schedule" $OUTPUT_FILE
		grep "Auto-schedule" $OUTPUT_FILE
        grep "Simple auto-schedule" $OUTPUT_FILE
	done
fi
