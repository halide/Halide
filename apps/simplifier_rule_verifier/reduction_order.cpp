...
set<string> find_divisors(const Expr &e) {
    DivisorSet d;
    e.accept(&d);
    return d.divisors;
}
...
bool check_divisors(const Expr &LHS, const Expr &RHS) {
    ...
    set<string> lhs_divisors = find_divisors(LHS);
    set<string> rhs_divisors = find_divisors(RHS);
...
map<IRNodeType, int> build_histogram(const Expr &e) {
    NodeHistogram histo;
    e.accept(&histo);
...
int get_total_op_count(const Expr &e) {
    map<IRNodeType, int> histo = build_histogram(e);
    int counter = 0;
...
int compare_histograms(const Expr &LHS, const Expr &RHS) {
    map<IRNodeType, int> lhs_histo = build_histogram(LHS);
    map<IRNodeType, int> rhs_histo = build_histogram(RHS);
    ...
    for (auto const &node : node_ordering) {
        ...
        debug(1) << node << " LHS count " << lhs_node_count << " RHS count " << rhs_node_count << "\n";
...
bool valid_reduction_order(const Expr &LHS, const Expr &RHS) {
    ...
    // wildcard variables can only match scalars, so we don't need to check variable occurrence counts
