...
#include "ml_ops/batch_matrix_multiply.h"
#include "ml_ops/fully_connected.h"
#include "ml_ops/ml_common.h"
#include "ml_ops/normalization.h"
#include "ml_ops/rope_weights.h"
#include "ml_ops/softmax.h"
...
public:
    ...
    // TODO: ensure the ordering of these optional inputs/outputs matches that
    // of the actual call site(s) in a sane way
    ...
    void configure() {
        ...
        key_proj_3d_.add_inputs(QuantizationKind::QC8NoBias, Int(8), model_dim_D_, head_dim_H_, this);
        query_proj_3d_.add_inputs(QuantizationKind::QC8NoBias, Int(8), model_dim_D_, model_dim_D_, this);
        value_proj_3d_.add_inputs(QuantizationKind::QC8NoBias, Int(8), model_dim_D_, head_dim_H_, this);

        attention_mask_.set_type(processing_type_);
        post_attention_proj_.add_inputs(QuantizationKind::QC8NoBias, Int(8), model_dim_D_, model_dim_D_, this);
        if (transformer_kind_ != TransformerKind::PrefixDecodeUpdateCache) {
            final_pre_normed_.add_inputs(feed_forward_pre_norm_, processing_type_, this);
            final_post_normed_.add_inputs(feed_forward_post_norm_, processing_type_, this);
            // TODO: is the order of model and hidden dims correct?
            feed_forward_layer_1_.add_inputs(QuantizationKind::QC8NoBias, Int(8), model_dim_D_, hidden_dim_HD_, this);
            feed_forward_layer_1_gate_.add_inputs(QuantizationKind::QC8NoBias, Int(8), model_dim_D_, hidden_dim_HD_, this);
            feed_forward_layer_2_.add_inputs(QuantizationKind::QC8NoBias, Int(8), hidden_dim_HD_, model_dim_D_, this);
            layer_output_ = add_output<Buffer<>>("layer_output", processing_type_, 3);
        }

        if (attention_scale_type_ == AttentionScaleType::PerDimScale) {
            per_dim_scale_ = add_input<Buffer<>>("per_dim_scale", processing_type_, 1);
        }

        if (transformer_kind_ == TransformerKind::PrefixDecodeUpdateCache) {
            // These are N, H, T, B or S, H, T, B (Halide ordering)
            key_cache_slice_output_ = add_output<Buffer<>>("key_cache_slice_output", processing_type_, 4);
            value_cache_slice_output_ = add_output<Buffer<>>("value_cache_slice_output", processing_type_, 4);
        }
        if (transformer_kind_ == TransformerKind::PrefixDecodeUseCache) {
            key_cache_input_ = add_input<Buffer<>>("key_cache", processing_type_, 4);
            value_cache_input_ = add_input<Buffer<>>("value_cache", processing_type_, 4);
        }
    }
    ...
    void generate() {
        ...
        } else if (attention_scale_type_ == AttentionScaleType::InverseSqrtHeadDim) {
            query_scale = fast_inverse_sqrt(cast(processing_type_, head_dim_H_));
        }

        const int key_value_split = static_cast<int32_t>(head_dim_H_) / static_cast<int32_t>(num_kv_heads_);
        key_proj_4d_(s, n, t, b) = key_proj_3d_.result(s + n * key_value_split, t, b);
        value_proj_4d_(s, n, t, b) = value_proj_3d_.result(s + n * key_value_split, t, b);
        const int query_split = static_cast<int32_t>(model_dim_D_) / static_cast<int32_t>(n_heads_N_);
        query_proj_4d_(h, n, t, b) = query_proj_3d_.result(h + n * query_split, t, b);

        assert(key_value_split == query_split);

        roped_key_proj_4d_.apply(key_proj_4d_, segment_pos_values_, head_dim_H_);
        roped_query_proj_4d_.apply(query_proj_4d_, segment_pos_values_, head_dim_H_);

        if (transformer_kind_ == TransformerKind::PrefixDecodeUpdateCache) {
            (*key_cache_slice_output_)(s, n, t, b) = roped_key_proj_4d_.result(s, n, t, b);
            ...
            query_proj_permuted_(h, t, n, b) = roped_query_proj_4d_.result(h, n, t, b) * query_scale;
            ...
            Expr total_seq_len = layer_input_.dim(1).min() + layer_input_.dim(1).extent();
            ...
            if (use_mqa_) {
                ...
                broadcast_key_proj_permuted(s, t, n, b) = key_proj_permuted_(s, t, 0, b);
                Func transposed_key_proj_permuted("transposed_key_proj_permuted");
                transposed_key_proj_permuted(t, s, n, b) = broadcast_key_proj_permuted(s, t, n, b);
            ...
            // BNTS
            if (soft_cap_ > 0.0f) {
                logits(s, t, n, b) = fast_tanh(logits(s, t, n, b) / soft_cap_) * soft_cap_;
            }
            ...
            broadcast_value_proj_permuted(s, t, n, b) = value_proj_permuted_(s, t, 0, b);
            Func transposed_value_proj_permuted("transposed_value_proj_permuted");
            transposed_value_proj_permuted(t, s, n, b) = broadcast_value_proj_permuted(s, t, n, b);
            outcome_before_permute_bmm_.float32_layer(probs_softmax_.result,
                                                      transposed_value_proj_permuted,
                                                      total_seq_len,
                                                      input_seq_len_maybe,
                                                      head_dim_H_);
            ...
            outcome_reshaped_(n, t, b) = kqv_merged_(n % head_dim_H_, n / head_dim_H_, t, b);
            ...
            if (feed_forward_params_activation_ == Activation::None) {
                feed_forward_gate_(n, t, b) = feed_forward_layer_1_gate_.result(n, t, b);
            } else if (feed_forward_params_activation_ == Activation::GELU) {
                feed_forward_gate_(n, t, b) = gelu(feed_forward_layer_1_gate_.result)(n, t, b);
            } else if (feed_forward_params_activation_ == Activation::SILU) {
                feed_forward_gate_(n, t, b) = silu(feed_forward_layer_1_gate_.result)(n, t, b);
            } else if (feed_forward_params_activation_ == Activation::RELU) {
                feed_forward_gate_(n, t, b) = relu(feed_forward_layer_1_gate_.result)(n, t, b);
            } else {
                std::abort();
            }

            feed_forward_layer_1_and_gate_(n, t, b) = feed_forward_layer_1_.result(n, t, b) * feed_forward_gate_(n, t, b);
            ...
            if (transformer_kind_ != TransformerKind::PrefixDecodeUpdateCache) {
                (*layer_output_)(n, t, b) = final_post_normed_.result(n, t, b) + output(n, t, b);
            }
        }
    }
    ...
    void schedule() {
        ...
        if (transformer_kind_ == TransformerKind::PrefixDecodeUpdateCache) {
            ...
            query_proj_3d_.default_schedule(LL(roped_query_proj_4d_.inner, t), target);
            ...
            roped_query_proj_4d_.default_schedule(LL(*key_cache_slice_output_, t), target);
            roped_key_proj_4d_.default_schedule(LL(*key_cache_slice_output_, t), target);
        } else {
            ...
            if (use_mqa_) {
                ...
                logits_bmm_.default_schedule(LL(probs_softmax_.result, b), target, parallel_split);
            }
            ...
            const int parallel_split = 16;
            outcome_before_permute_bmm_.default_schedule(LL(post_attention_proj_.result, b), target, parallel_split);
            ...
            feed_forward_layer_1_and_gate_.compute_at(feed_forward_layer_2_.result, b).hoist_storage_root();
    ...
    void root_schedule() {
        ...
        if (transformer_kind_ != TransformerKind::PrefixDecodeUpdateCache) {
            ...
            outcome_before_permute_bmm_.default_schedule(LL::root(), target, /*parallel_split*/ 0);
...
public:
    ...
    void generate() {
        ...
        seq_range_ = RDom(0, embedding_dim_ / 2, input_length_, seq_length_ - input_length_);
        Expr log_timescale_inc = default_log(max_timescale_ / min_timescale_) /
                                 max(embedding_dim_ / 2.0f, 1.0f);
        Expr inv_timescale = min_timescale_ * default_exp(input_range_.x * log_timescale_inc);
...
HALIDE_REGISTER_GENERATOR(hamal::LlmRoPEValues, LlmRoPEValues);
HALIDE_REGISTER_GENERATOR(hamal::LlmPreprocessor,
                          LlmPreprocessor);
HALIDE_REGISTER_GENERATOR(hamal::LlmTransformer, LlmTransformer);
HALIDE_REGISTER_GENERATOR(hamal::LlmPostprocessor,
                          LlmPostprocessor);
HALIDE_REGISTER_GENERATOR(hamal::LlmPositionEmbedding,
