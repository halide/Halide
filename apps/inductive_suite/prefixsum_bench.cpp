...
int main(int argc, char **argv) {
    ...
    try {
        ...
        const char *label = unfolded ? "inductive UNFOLDED (fold x -> W)" : "inductive FOLDED (fold x -> 1 accum)";
        ...
        // data and we can check for exact agreement.
        {
            ...
            fwrite(out_flat.data(), sizeof(float), W * H, f);
            fclose(f);
        }
