...
function(_add_halide_library_set LIBRARY_SET)
    ...
    if (TARGET ${args_TARGET}.generator)
        ...
        target_include_directories(
            ${args_TARGET}.generator
            PRIVATE $<BUILD_INTERFACE:${hannk_SOURCE_DIR}>
        )
    endif ()

    add_halide_library(
        ${args_TARGET}
        FROM hannk::halide_generators::${args_TARGET}.generator
        NAMESPACE hannk
...
function(_finish_halide_library_set LIBRARY_SET)
    ...
    if (NOT TARGET hannk::halide_generators::common_halide)
        ...
        export(
            TARGETS common_halide
            NAMESPACE hannk::halide_generators::
            APPEND
            FILE "${hannk_BINARY_DIR}/cmake/hannk-halide_generators-config.cmake"
        )
    endif ()
endfunction()
...
_add_halide_library_set(
    halide_op_implementations
