...
int main(int argc, char **argv) {
    ...
    three_way_bench(
        [&]() { max_filter(input, output); output.device_sync(); },
        [&]() { max_filter_classic_auto_schedule(input, output); output.device_sync(); },
        [&]() { max_filter_auto_schedule(input, output); output.device_sync(); }
    );
