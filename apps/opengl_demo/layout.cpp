...
void Layout::draw_texture(enum location location, GLuint texture_id, int width,
    ...
    OpenGLHelpers::display_texture(
        texture_id, 2.0 * x0 / state.window_width - 1.0,
        2.0 * x1 / state.window_width - 1.0, 2.0 * y0 / state.window_height - 1.0,
        2.0 * y1 / state.window_height - 1.0);
    OpenGLHelpers::draw_text(label, 2.0 * lx / state.window_width - 1.0,
                             2.0 * ly / state.window_height - 1.0);
}

void Layout::draw_image(enum location location, const uint8_t *data, int width,
                        int height, const std::string &label) {
