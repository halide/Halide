...
int main(int argc, char **argv) {
    ...
    buffer_t bufs[] = { make_buffer<float>(W, H), make_buffer<double>(W, H),
                        make_buffer<int8_t>(W, H), make_buffer<uint8_t>(W, H),
                        make_buffer<int16_t>(W, H), make_buffer<uint16_t>(W, H),
                        make_buffer<int32_t>(W, H), make_buffer<uint32_t>(W, H),
                        make_buffer<int64_t>(W, H), make_buffer<uint64_t>(W, H) };
    ...
    for (int i = 0; filters[i].fn; i++) {
        filter f = filters[i];
        printf("Testing %s\n", f.name);
        f.fn(bufs + 0, bufs + 1, bufs + 2, bufs + 3, bufs + 4, bufs + 5, bufs + 6,
             bufs + 7, bufs + 8, bufs + 9, &out);
        if (*out_value) {
            printf("Error: %f\n", *out_value);
            error = true;
        }
