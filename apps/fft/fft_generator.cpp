...
std::map<std::string, FFTNumberType> fft_number_type_enum_map() {
    return { { "real", FFTNumberType::Real }, { "complex", FFTNumberType::Complex } };
}
...
public:
    ...
    // "samples_to_frequency" maps to a forward FFT. (Other packages sometimes
    // call this a sign of -1)
    // "frequency_to_samples" maps to a forward FFT. (Other packages sometimes
    // call this a sign of +1)
    GeneratorParam<FFTDirection> direction{
        "direction", FFTDirection::SamplesToFrequency, fft_direction_enum_map()
    };

    // Whether the input is "real" or "complex".
    GeneratorParam<FFTNumberType> input_number_type{
        "input_number_type", FFTNumberType::Real, fft_number_type_enum_map()
    };
    // Whether the output is "real" or "complex".
    GeneratorParam<FFTNumberType> output_number_type{
        "output_number_type", FFTNumberType::Real, fft_number_type_enum_map()
    };
    ...
    void generate() {
        ...
        if (output_number_type == FFTNumberType::Real) {
            ...
            output(x, y, c) =
                select(c == 0, re(complex_result(x, y)), im(complex_result(x, y)));
        }
    }

    void schedule() {
        const int input_comps = (input_number_type == FFTNumberType::Real) ? 1 : 2;
        const int output_comps =
            (output_number_type == FFTNumberType::Real) ? 1 : 2;

        input.dim(0)
            .set_stride(input_comps)
            .dim(2)
            .set_min(0)
            .set_extent(input_comps)
            .set_stride(1);

        output.dim(0)
            .set_stride(output_comps)
            .dim(2)
            .set_min(0)
            .set_extent(output_comps)
            .set_stride(1);
