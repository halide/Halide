...
public:
    ...
    void generate() {
        ...
        if (Halide::Internal::get_env_variable("HL_ENABLE_RAKE") == "1") {
            Expr a = bounded_input(x, y - 2);
            Expr b = bounded_input(x, y - 1);
            Expr c = bounded_input(x, y);
            Expr d = bounded_input(x, y + 1);
            Expr e = bounded_input(x, y + 2);

            rows(x, y) = ((widening_add(b, d) << cast<uint8_t>(2)) + widening_add(a, e)) + widening_mul(c, cast<uint8_t>(6));
        } else {
