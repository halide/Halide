...
int main(int argc, char *argv[]) {
    ...
    for (int p = 0; p < num_pipes; p++) {
        ...
        while (getline(data_files_f, image_dir)) {
            Runtime::Buffer<outputT> correct_output = buffer_from_file<outputT>(image_dir + "/g_at_b.data", output_shape);
            Runtime::Buffer<inputT> input0 = buffer_from_file<inputT>(image_dir + "/gr.data", input_shape);

            Runtime::Buffer<inputT> input1 = buffer_from_file<inputT>(image_dir + "/r.data", input_shape);
            Runtime::Buffer<inputT> input2 = buffer_from_file<inputT>(image_dir + "/b.data", input_shape);
            Runtime::Buffer<inputT> input3 = buffer_from_file<inputT>(image_dir + "/gb.data", input_shape);
            Runtime::Buffer<lossT> loss_buff = Runtime::Buffer<lossT>::make_scalar();

            Runtime::Buffer<outputT> output_buff = Runtime::Buffer<outputT>(output_w, output_h, 1);
