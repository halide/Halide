...
for ((b=1;b<2;b++)); do
    ...
    results/files_master_${b}_${BEST_CACHE_SIZE}_${BEST_BALANCE}.txt | sed "s/-new_autoscheduler//;s/-${BEST_CACHE_SIZE}-${BEST_BALANCE}//;s/_..times.txt/_X\/times.txt/" |  sort | uniq -d  | sed "s/${HL_TARGET}/${HL_TARGET}-new_autoscheduler/" > results/files_common1.txt

    # Generate the list of pipelines that actually load from the input
    grep -r 'input\[' bin/${HL_TARGET}-new_autoscheduler/* | cut -d/ -f1-3 | sort | uniq | while read F; do echo ${F}/schedule_0_100_50_X/times.txt; done > results/files_valid.txt
