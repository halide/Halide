...
public:
    ...
    Func build() {
        ...
        Expr val = clamped(x * s_sigma + r.x - s_sigma / 2,
        ...
        blurz(x, y, z, c) =
            (histogram(x, y, z - 2, c) + histogram(x, y, z - 1, c) * 4 +
        ...
        if (get_target().has_gpu_feature()) {
            ...
            // histogram.compute_root().reorder(c, z, x, y).gpu_tile(x, y, xi, yi, 8,
            // 8);
            // histogram.update().reorder(c, r.x, r.y, x, y).gpu_tile(x, y, xi, yi, 8,
            // 8).unroll(c);
