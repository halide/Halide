...
public:
    ...
    Func build() {
        ...
        if (transpose_) {
            ...
            result.specialize(size >= unroll_size)
                .vectorize(i, unroll_size)
                .specialize(size >= block_size_)
                .split(i, t, i, block_size_ / unroll_size)
                .parallel(t);

            accum_vecs.compute_at(result, i)
                .unroll(i)
                .unroll(j)
                .update()
                .reorder(i, j, k)
                .unroll(i)
                .unroll(j);
            accum_vecs_transpose.compute_at(result, i).unroll(i).unroll(j);
            sum_lanes.compute_at(result, i).update().unroll(lanes);
            sum_tail.compute_at(result, i).update().reorder(i, tail);  //.unroll(i);

            if (vectorize_) {
                accum_vecs.vectorize(j).update().vectorize(j);
                accum_vecs_transpose.vectorize(j);

                sum_lanes.specialize(size >= vec_size)
                    .vectorize(i, vec_size);  //.unroll(i);
                sum_lanes.update()
                    .specialize(size >= vec_size)
                    .vectorize(i, vec_size);  //.unroll(i);

                sum_tail.specialize(size >= vec_size)
                    .vectorize(i, vec_size);  //.unroll(i);
                sum_tail.update()
                    .specialize(size >= vec_size)
                    .vectorize(i, vec_size);  //.unroll(i);
        ...
        Var ii("ii");
        output.specialize(size >= vec_size)
            .vectorize(i, vec_size)
            .specialize(size >= unroll_size * vec_size)
            .unroll(i, unroll_size)
            .specialize(size >= block_size_)
...
public:
    ...
    Func build() {
        ...
        Var ii("ii"), ji("ji");
        Var ti("ti"), tj("tj"), t("t");
        result.specialize(num_rows >= vec_size)
            .vectorize(i, vec_size)
            .specialize(num_cols >= unroll_size)
            .unroll(j, unroll_size)
