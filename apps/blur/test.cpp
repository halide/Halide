...
void blur(Buffer<uint16_t, 2> in, Buffer<uint16_t, 2> tmp, Buffer<uint16_t, 2> out) {
...
void blur_fast(Buffer<uint16_t, 2> in, Buffer<uint16_t, 2> out) {
#ifdef __SSE2__
...
void blur_halide(Buffer<uint16_t, 2> in, Buffer<uint16_t, 2> out) {
...
int main(int argc, char **argv) {
    ...
    Buffer<uint16_t, 2> blur_tmp(input.width() - 8, input.height());
    Buffer<uint16_t, 2> blurry(input.width() - 8, input.height() - 2);
    Buffer<uint16_t, 2> speedy(input.width() - 8, input.height() - 2);
    Buffer<uint16_t, 2> halide(input.width() - 8, input.height() - 2);

    // Call it once to initialize the halide runtime stuff, and copy out the
    // result if it's a device buffer and dirty.
    blur_halide(input, halide);
    halide.copy_to_host();

    auto [slow, fast, halide_result] = benchmark_comparison(
        BenchmarkConfig{},
        [&]() { blur(input, blur_tmp, blurry); },
        [&]() { blur_fast(input, speedy); },
        [&]() { blur_halide(input, halide); });

    halide.copy_to_host();

    double slow_time = slow.wall_time;
    double fast_time = fast.wall_time;
    double halide_time = halide_result.wall_time;
