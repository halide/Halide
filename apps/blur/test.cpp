...
#include "halide_blur_auto_schedule_old.h"
...
Buffer<uint16_t> blur_halide_auto_schedule_old(Buffer<uint16_t> in) {
    Buffer<uint16_t> out(in.width()-8, in.height()-2);

    // Call it once to initialize the halide runtime stuff
    halide_blur_auto_schedule_old(in, out);
    // Copy-out result if it's device buffer and dirty.
    out.copy_to_host();
...
int main(int argc, char **argv) {
    ...
    Buffer<uint16_t> halide_auto_schedule_old = blur_halide_auto_schedule_old(input);
    double halide_auto_schedule_time_old = t;
