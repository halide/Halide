...
// modifications the program by compiling with -DIMGINIT
//
// These features are off by default.  To enable, compile with the following
// flags:
//   -DIMGINFO  Enable image info output
//   -DIMGINIT  Produce image info on every call to init
//
...
#if defined(IMGINFO) || defined(IMGINIT)
#include <cassert>
#include <cstdlib>
#include <limits>
#include <memory>
    ...
    if (img_bpp > 1) {
        for (int d = 0; d < dim; d++) {
            std::cout << tag << " str[" << d << "]*bpp = "
                             ...
                             << std::right << " # ";
            print_memalign(stride[d] * img_bpp);
        }
    }
