...
$(LIB_DIR)/libHalide.a: $(OBJECTS) $(INITIAL_MODULES)
	...
	$(CXX) -o /dev/null -shared $(OBJECTS) $(INITIAL_MODULES) -Wl,-t $(LLVM_STATIC_LIBS) $(LIBDL) $(JAVASCRIPT_LDFLAGS) -lz -lpthread | grep libLLVM > $(BUILD_DIR)/llvm_objects/list
...
$(BIN_DIR)/libHalide.$(SHARED_EXT): $(LIB_DIR)/libHalide.a
	@-mkdir -p $(BIN_DIR)
	$(CXX) $(BUILD_BIT_SIZE) -shared $(OBJECTS) $(INITIAL_MODULES) $(LLVM_STATIC_LIBS) $(LLVM_LDFLAGS) $(LLVM_SHARED_LIBS) $(JAVASCRIPT_LDFLAGS) $(LIBDL) -lz -lpthread -o $(BIN_DIR)/libHalide.$(SHARED_EXT)
