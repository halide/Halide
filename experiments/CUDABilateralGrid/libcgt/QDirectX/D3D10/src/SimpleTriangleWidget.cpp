...
void SimpleTriangleWidget::initializeD3D()
{
	...
	float* pStream = reinterpret_cast< float* >( m_pVertexBuffer->mapForWriteDiscard() );
...
void SimpleTriangleWidget::loadShaders()
{
	...
	(
		L"..\\QD3D10Widget\\simple.fx", // filename
		NULL, // #defines
		NULL, // includes
		NULL, // function name,
		"fx_4_0", // profile
...
void SimpleTriangleWidget::paintD3D()
{
	...
	ID3D10Buffer* pBuffer = m_pVertexBuffer->buffer();
	device()->IASetVertexBuffers( 0, 1, &pBuffer, &stride, &offset );
	device()->IASetPrimitiveTopology( D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
	device()->IASetInputLayout( m_pInputLayout );
