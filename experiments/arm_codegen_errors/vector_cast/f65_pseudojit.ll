...
@assert_message1 = internal global [49 x i8] c"Stride on innermost dimension of .i130 must be 1\00"
...
@assert_message6 = internal global [38 x i8] c"Min on dimension 0 of .i130 must be 0\00"
@assert_message7 = internal global [38 x i8] c"Min on dimension 1 of .i130 must be 0\00"
@assert_message8 = internal global [38 x i8] c"Min on dimension 2 of .i130 must be 0\00"
@assert_message9 = internal global [38 x i8] c"Min on dimension 3 of .i130 must be 0\00"
@assert_message10 = internal global [44 x i8] c"Function may load image .i130 out of bounds\00"
@assert_message11 = internal global [44 x i8] c"Function may load image .i130 out of bounds\00"
...
define void @f65_pseudojit_inner(i8* %.i130, i32 %.i130.extent.0, i32 %.i130.extent.1, i32 %.i130.extent.2, i32 %.i130.extent.3, i32 %.i130.stride.0, i32 %.i130.stride.1, i32 %.i130.stride.2, i32 %.i130.stride.3, i32 %.i130.min.0, i32 %.i130.min.1, i32 %.i130.min.2, i32 %.i130.min.3, i8* %.result, i32 %.result.extent.0, i32 %.result.extent.1, i32 %.result.extent.2, i32 %.result.extent.3, i32 %.result.stride.0, i32 %.result.stride.1, i32 %.result.stride.2, i32 %.result.stride.3, i32 %.result.min.0, i32 %.result.min.1, i32 %.result.min.2, i32 %.result.min.3) {
entry:
...
after_assert:                                     ; preds = %entry
  %1 = icmp eq i32 %.i130.stride.0, 1
  br i1 %1, label %after_assert2, label %assert1

assert1:                                          ; preds = %after_assert
  call void @halide_error(i8* getelementptr inbounds ([49 x i8]* @assert_message1, i32 0, i32 0))
  ret void
...
after_assert10:                                   ; preds = %after_assert8
  %6 = icmp eq i32 %.i130.min.0, 0
  br i1 %6, label %after_assert12, label %assert11

assert11:                                         ; preds = %after_assert10
  call void @halide_error(i8* getelementptr inbounds ([38 x i8]* @assert_message6, i32 0, i32 0))
  ret void

after_assert12:                                   ; preds = %after_assert10
  %7 = icmp eq i32 %.i130.min.1, 0
  br i1 %7, label %after_assert14, label %assert13

assert13:                                         ; preds = %after_assert12
  call void @halide_error(i8* getelementptr inbounds ([38 x i8]* @assert_message7, i32 0, i32 0))
  ret void

after_assert14:                                   ; preds = %after_assert12
  %8 = icmp eq i32 %.i130.min.2, 0
  br i1 %8, label %after_assert16, label %assert15

assert15:                                         ; preds = %after_assert14
  call void @halide_error(i8* getelementptr inbounds ([38 x i8]* @assert_message8, i32 0, i32 0))
  ret void

after_assert16:                                   ; preds = %after_assert14
  %9 = icmp eq i32 %.i130.min.3, 0
  br i1 %9, label %after_assert18, label %assert17

assert17:                                         ; preds = %after_assert16
  call void @halide_error(i8* getelementptr inbounds ([38 x i8]* @assert_message9, i32 0, i32 0))
  ret void
...
after_assert18:                                   ; preds = %after_assert16
  ...
  %12 = mul i32 %11, 2
  %13 = add i32 %12, -1
  %14 = icmp slt i32 %13, %.i130.extent.0
  br i1 %14, label %after_assert20, label %assert19

assert19:                                         ; preds = %after_assert18
  call void @halide_error(i8* getelementptr inbounds ([44 x i8]* @assert_message10, i32 0, i32 0))
  ret void

after_assert20:                                   ; preds = %after_assert18
  %15 = add i32 %.result.extent.1, -1
  %16 = icmp slt i32 %15, %.i130.extent.1
  br i1 %16, label %after_assert22, label %assert21

assert21:                                         ; preds = %after_assert20
  call void @halide_error(i8* getelementptr inbounds ([44 x i8]* @assert_message11, i32 0, i32 0))
  ret void
...
after_assert24:                                   ; preds = %after_assert22
  ...
  br label %f65.v132_loop

f65.v132_loop:                                    ; preds = %f65.v131_afterloop, %after_assert24
  %f65.v132 = phi i32 [ 0, %after_assert24 ], [ %f65.v132_nextvar, %f65.v131_afterloop ]
  %23 = add i32 %.result.extent.0, 1
  %24 = sdiv i32 %23, 2
  %25 = add i32 0, %24
  br label %f65.v131_loop

f65.v131_loop:                                    ; preds = %f65.v131_loop, %f65.v132_loop
  %f65.v131 = phi i32 [ 0, %f65.v132_loop ], [ %f65.v131_nextvar, %f65.v131_loop ]
  %memref_elem_ptr = bitcast i8* %.result to double*
  %26 = mul i32 %.result.stride.1, %f65.v132
  %27 = mul i32 %f65.v131, 2
  %28 = add i32 %27, %26
  %memref = getelementptr double* %memref_elem_ptr, i32 %28
  %typed_memref = bitcast double* %memref to <2 x double>*
  %29 = bitcast <2 x double>* %typed_memref to i8*
  %memref_elem_ptr25 = bitcast i8* %.i130 to float*
  %30 = mul i32 %f65.v132, %.i130.stride.1
  %31 = mul i32 %f65.v131, 2
  %32 = add i32 %31, %30
  %memref26 = getelementptr float* %memref_elem_ptr25, i32 %32
  %typed_memref27 = bitcast float* %memref26 to <2 x float>*
  %33 = bitcast <2 x float>* %typed_memref27 to i8*
  %34 = call <2 x float> @llvm.arm.neon.vld1.v2f32(i8* %33, i32 4)
  %35 = fpext <2 x float> %34 to <2 x double>
  %36 = bitcast <2 x double> %35 to <16 x i8>
  call void @llvm.arm.neon.vst1.v16i8(i8* %29, <16 x i8> %36, i32 1)
  %f65.v131_nextvar = add i32 %f65.v131, 1
  %37 = icmp ne i32 %f65.v131_nextvar, %25
  br i1 %37, label %f65.v131_loop, label %f65.v131_afterloop

f65.v131_afterloop:                               ; preds = %f65.v131_loop
  %f65.v132_nextvar = add i32 %f65.v132, 1
  %38 = icmp ne i32 %f65.v132_nextvar, %22
  br i1 %38, label %f65.v132_loop, label %f65.v132_afterloop

f65.v132_afterloop:                               ; preds = %f65.v131_afterloop
  ret void
}
...
entry:
  ...
  call void @f65_pseudojit_inner(i8* %.i130, i32 %.i130.extent.0, i32 %.i130.extent.1, i32 %.i130.extent.2, i32 %.i130.extent.3, i32 %.i130.stride.0, i32 %.i130.stride.1, i32 %.i130.stride.2, i32 %.i130.stride.3, i32 %.i130.min.0, i32 %.i130.min.1, i32 %.i130.min.2, i32 %.i130.min.3, i8* %.result, i32 %.result.extent.0, i32 %.result.extent.1, i32 %.result.extent.2, i32 %.result.extent.3, i32 %.result.stride.0, i32 %.result.stride.1, i32 %.result.stride.2, i32 %.result.stride.3, i32 %.result.min.0, i32 %.result.min.1, i32 %.result.min.2, i32 %.result.min.3)
  ret void
}

define void @f65_pseudojit_c_wrapper(i8**) {
entry:
  ...
  call void @f65_pseudojit(%struct.buffer_t* %3, %struct.buffer_t* %6)
  ret void
}
