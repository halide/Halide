...
public:
    ...
    Stmt construct_stmt() {
        ...
        for (int i = branches.size()-1; i >= 0; --i) {
            ...
            branch_stmt = For::make(name, branch_min, branch_extent, For::Serial, Device_Parent, branch_stmt);
            if (!stmt.defined()) {
                stmt = branch_stmt;
            } else {
...
void specialize_branched_loops_test() {
...

    {
        ...
        Stmt branch = IfThenElse::make(cond, Store::make("out", 1, x),
                                       Store::make("out", 0, x));
        Stmt stmt = For::make("x", 0, 10, For::Serial, Device_Parent, branch);
...

    {
        ...
        stmt = For::make("y", 0, 10, For::Serial, Device_Parent, branch);
        Stmt branched = specialize_branched_loops(stmt);
...

    {
        ...
        Stmt branch = IfThenElse::make(cond, Store::make("out", 1, x),
                                       Store::make("out", 0, x));
        Stmt stmt = For::make("x", 0, 11, For::Serial, Device_Parent, branch);
        Stmt branched = specialize_branched_loops(stmt);
...

    {
        ...
        Stmt branch = IfThenElse::make(cond, Store::make("out", 1, x),
                                       Store::make("out", 0, x));
        Stmt stmt = For::make("x", 0, 10, For::Parallel, Device_Parent, branch);
        Stmt branched = specialize_branched_loops(stmt);
...

    {
        ...
        Stmt stmt = For::make("x", 0, 10, For::Serial, Device_Parent, branch);
        stmt = For::make("y", 0, 11, For::Serial, Device_Parent, stmt);
        Stmt branched = specialize_branched_loops(stmt);
    ...
    /* {
        ...
        stmt = For::make("x", 0, 10, For::Serial, Device_Parent, stmt);
        stmt = For::make("y", 0, 11, For::Serial, Device_Parent, stmt);
        Stmt branched = specialize_branched_loops(stmt);
        check_num_branches(branched, "y", 3);
        Interval ivals[] = {Interval(0,1), Interval(1,9), Interval(10,1)};
