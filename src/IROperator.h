...
/** A reference to one field of a struct-typed Expr (see Type::Struct),
 * returned by field(). Implicitly converts to Expr for a scalar field;
 * supports operator[] for an array field. Using the wrong one of these for
 * how the field was actually declared is a user_error, not a silent
 * type-punning bug. */
class FieldRef {
    Expr struct_value;
    int field_index;
...
/** Extract field `name` (or `index`) from a struct-typed Expr (see
 * Type::Struct). The field's offset and type are resolved immediately,
 * at Expr-construction time, from struct_value's Type -- the field name
 * never needs to survive into the IR. */
// @{
