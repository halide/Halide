 ...
 * implementations of CompileTimeErrorReporter::error() must never return.
...
public:
    ...
    // Apparently [[noreturn]] isn't inherited, so we can't enforce it on
    // overrides. We'll just document that it shouldn't return, and add our
    // own abort() after calls to it to ensure that we don't continue.
...
struct WarningReport {
    ...
    template<typename T>
    WarningReport &operator<<(const T &x) {
        msg << x;
        return *this;
    }

    WarningReport(const char *f, int l, const char *cs, int flags);
    ~WarningReport();
};
