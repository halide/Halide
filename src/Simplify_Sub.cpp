    ...
    if (may_simplify(op->type)) {
        ...
        if (EVAL_IN_LAMBDA
            ((!op->type.is_uint() && rewrite(x - c0, x + fold(-c0), !overflows(-c0))) ||
             ...
             rewrite(ramp(x, y) - ramp(z, w), ramp(x - z, y - w), is_same_lane_num(x, z)) ||
             rewrite(ramp(x, y) - broadcast(z), ramp(x - z, y), is_same_lane_num(x, z)) ||
             rewrite(broadcast(x) - ramp(z, w), ramp(x - z, -w), is_same_lane_num(x, z)) ||
             rewrite(broadcast(x) - broadcast(y), broadcast(x - y), is_same_lane_num(x, y)) ||
             rewrite((x - broadcast(y)) - broadcast(z), x - broadcast(y + z), is_same_lane_num(y, z)) ||
             rewrite((x + broadcast(y)) - broadcast(z), x + broadcast(y - z), is_same_lane_num(y, z)) ||
