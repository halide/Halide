#ifndef HALIDE_CODEGEN_AMDGPU_DEV_H
#define HALIDE_CODEGEN_AMDGPU_DEV_H

/** \file
 * Defines the code-generator for producing AMDGPU host code
 */
...
class CodeGen_AMDGPU_Dev : public CodeGen_LLVM, public CodeGen_GPU_Dev {
public:
    friend class CodeGen_GPU_Host<CodeGen_X86>;
    friend class CodeGen_GPU_Host<CodeGen_ARM>;

    /** Create a AMDGPU device code generator. */
    CodeGen_AMDGPU_Dev(Target host);
    ~CodeGen_AMDGPU_Dev();
...
protected:
    ...
    /** (Re)initialize the AMDGPU module. This is separate from compile, since
     * a AMDGPU device module will often have many kernels compiled into it for
     * a single pipeline. */
     ...
     * amdgpu intrinsic functions to call to get them. */
