...
void IRPrinter::test() {
    ...
    Stmt assertion = AssertStmt::make(y > 3, Call::make(Int(32), "halide_error_generic",
                                                        vec<Expr>(string("y is greater than 3")), Call::Extern));
    ...
    std::string correct_source = \
        ...
        "let y = 17\n"
        "assert((y > 3), halide_error_generic(\"y is greater than 3\"))\n"
        "produce buf {\n"
        "  parallel (x, -2, (y + 2)) {\n"
        "    buf[(y - 1)] = ((x*17)/(x - 3))\n"
        "  }\n"
