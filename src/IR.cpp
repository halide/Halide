...
Expr Load::make(Type type, std::string name, Expr index, BufferRef<> image, Parameter param) {
...
Expr Call::make(Function func, const std::vector<Expr> &args, int idx) {
    ...
    return make(func.output_types()[(size_t)idx], func.name(), args, Halide, func.get_contents(), idx, BufferRef<>(), Parameter());
}
...
Expr Call::make(Type type, std::string name, const std::vector<Expr> &args, CallType call_type,
                ...
                BufferRef<> image, Parameter param) {
    for (size_t i = 0; i < args.size(); i++) {
...
Expr Variable::make(Type type, std::string name, BufferRef<> image, Parameter param, ReductionDomain reduction_domain) {
