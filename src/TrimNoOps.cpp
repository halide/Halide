...
class SimplifyUsingBounds : public IRMutator {
    ...
    Stmt visit(const For *op) override {
        ...
        containing_loops.pop_back();
        return For::make(op->name, min, extent, op->for_type, op->device_api, body, op->allow_partitioning);
    }
...
class TrimNoOps : public IRMutator {
    ...
    Stmt visit(const For *op) override {
        ...
        Stmt stmt = For::make(op->name, new_min_var, new_extent, op->for_type, op->device_api, body, op->allow_partitioning);
