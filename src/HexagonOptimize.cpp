...
class HoistSliceVector : public IRMutator {
    using IRMutator::visit;
    struct VarInfo {
        Expr replacement;
    ...
    void visit_let(NodeType &result, const LetType *op) {
        ...
        debug(4) << "HoistSliceVector: Working on Let " << op->name << " = "
                 << value << " in ... " << op->name << " ...\n";
        ...
        while (1) {
            ...
            if (call && call->is_intrinsic(Call::concat_vectors) &&
                       (call->args.size() == 2)) {
        ...
        debug (4) << "HoistSliceVector: Pushing " << op->name << " -> " << info.replacement << "\n";
    ...
    template <typename T>
    void visit_binary(const T* op) {
        debug(4) << "HoistSliceVector: Trying to hoist slice vector --> " << (Expr) op << "\n";

        Expr a = mutate(op->a);
        Expr b = mutate(op->b);

        const Call *call_a = a.template as<Call>();
        const Call *call_b = b.template as<Call>();
        ...
        if (call_a && call_b &&
            ...
            if (*start_lane_a != *start_lane_b) {
                debug(4) << "........ Unable to hoist - lanes don't match\n";
            ...
            if (*stride_a != 1 || *stride_b != 1) {
                debug(4) << "........ Unable to hoist - strides don't match\n";
                if (!a.same_as(op->a) || !b.same_as(op->b)) {
                    expr = T::make(a, b);
                } else {
                    expr = op;
            ...
            if (!concat_a || !concat_b) {
                debug(4) << "........ Unable to hoist - both are not concats\n";
                debug(4) << "a = " << call_a->args[0] << "\n";
                debug(4) << "b = " << call_b->args[0] << "\n";
                if (!a.same_as(op->a) || !b.same_as(op->b)) {
                    expr = T::make(a, b);
                } else {
                    expr = op;
            ...
            if (slices_a.size() != slices_b.size()) {
                debug(4) << "........ Unable to hoist - number of slices do not match\n";
                if (!a.same_as(op->a) || !b.same_as(op->b)) {
                    expr = T::make(a, b);
                } else {
                    expr = op;
...
Stmt hoist_slice_vector(Stmt s) {
    debug(2) << "Before hoisting :\n" << s << "\n\n";
    s = HoistSliceVector().mutate(s);
    debug(2) << "After hoisting :\n" << s << "\n\n";
    return s;
}
