...
protected:
    ...
    private:
        ...
        // Helper function to handle binary operations with signed and unsigned integers, and float types
        template<typename IntOp, typename UIntOp, typename FloatOp>
        mlir::Value BinaryOpHelper(const Halide::Type &type, const Halide::Expr &a, const Halide::Expr &b);

        // Specialization for binary operations with sign-agnostic integer types and float types
        template<typename IntOp, typename FloatOp>
        mlir::Value BinaryOpHelper(const Halide::Type &type, const Halide::Expr &a, const Halide::Expr &b) {
            return BinaryOpHelper<IntOp, IntOp, FloatOp>(type, a, b);
        }

        // Helper function to handle comparison operations with signed and unsigned integers, and float types
        ...
        mlir::Value CompareOpHelper(const Halide::Type &type, const Halide::Expr &a, const Halide::Expr &b);

        // Specialization for comparison operations with sign-agnostic integer types and float types
        template<typename IntOp, mlir::arith::CmpIPredicate IntPredicate,
                 typename FloatOp, mlir::arith::CmpFPredicate FloatPredicate>
        mlir::Value CompareOpHelper(const Halide::Type &type, const Halide::Expr &a, const Halide::Expr &b) {
            return CompareOpHelper<IntOp, IntPredicate, IntOp, IntPredicate, FloatOp, FloatPredicate>(type, a, b);
        }
    };
