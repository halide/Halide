...
Stmt build_provide_loop_nest(string func_name,
    ...
    Stmt stmt = build_provide_loop_nest_helper(
        func_name, prefix, start_fuse, dims, site, values, def.split_predicate(), f_sched, def.schedule(), is_update);
    ...
    for (size_t i = specializations.size(); i > 0; i--) {
        ...
        if (s.failure_message.empty()) {
            then_case = build_provide_loop_nest(func_name, prefix, start_fuse, dims, f_sched, s_def, is_update);
        } else {
...
private:
    ...
    Stmt build_produce_definition(const map<string, bool> &skip, Function f,
        ...
        const vector<string> f_args = f.args();
        Stmt produce = build_provide_loop_nest(f.name(), prefix, start_fuse, f_args, f.schedule(), def, is_update);
