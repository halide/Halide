...
bool validate_schedule(Function f, const Stmt &s, const Target &target, bool is_output, const map<string, Function> &env) {
    ...
    // Storage that is private to a thread or a lane is not shared by a loop
    // over them, so putting such a loop between where it is stored and where
    // it is computed is not a race: each thread gets its own copy, which is
    // what the memory type means. Whether the accesses within a thread stay
    // within its own copy is a separate question, checked later in lowering.
    const MemoryType mem = f.schedule().memory_type();
    const bool thread_private =
        mem == MemoryType::Register || mem == MemoryType::Tile;
