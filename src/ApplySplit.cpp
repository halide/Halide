...
vector<ApplySplitResult> apply_split(const Split &split, bool is_update, std::string_view prefix,
                                     StringMap<Expr> &dim_extent_alignment) {
    vector<ApplySplitResult> result;

    Expr outer = Variable::make(Int(32), concat(prefix, split.outer));
    Expr outer_max = Variable::make(Int(32), concat(prefix, split.outer, ".loop_max"));
    if (split.is_split()) {
        Expr inner = Variable::make(Int(32), concat(prefix, split.inner));
        Expr old_max = Variable::make(Int(32), concat(prefix, split.old_var, ".loop_max"));
        Expr old_min = Variable::make(Int(32), concat(prefix, split.old_var, ".loop_min"));
        Expr old_extent = Variable::make(Int(32), concat(prefix, split.old_var, ".loop_extent"));

        dim_extent_alignment[split.inner] = split.factor;

        Expr base = outer * split.factor + old_min;
        string base_name = concat(prefix, split.inner, ".base");
        Expr base_var = Variable::make(Int(32), base_name);
        string old_var_name = concat(prefix, split.old_var);
        Expr old_var = Variable::make(Int(32), old_var_name);

        StringMap<Expr>::iterator iter = dim_extent_alignment.find(split.old_var);
        ...
        if ((iter != dim_extent_alignment.end()) &&
            ...
            string guarded_var_name = concat(prefix, split.old_var, ".guarded");
            ...
            result.emplace_back(concat(prefix, split.old_var), guarded_var, substitution_type);
        ...
        Expr fused = Variable::make(Int(32), concat(prefix, split.old_var));
        Expr inner_min = Variable::make(Int(32), concat(prefix, split.inner, ".loop_min"));
        Expr outer_min = Variable::make(Int(32), concat(prefix, split.outer, ".loop_min"));
        Expr inner_extent = Variable::make(Int(32), concat(prefix, split.inner, ".loop_extent"));

        const Expr &factor = inner_extent;
        Expr inner = fused % factor + inner_min;
        Expr outer = fused / factor + outer_min;

        result.emplace_back(concat(prefix, split.inner), inner, ApplySplitResult::Substitution);
        result.emplace_back(concat(prefix, split.outer), outer, ApplySplitResult::Substitution);
        result.emplace_back(concat(prefix, split.inner), inner, ApplySplitResult::LetStmt);
        result.emplace_back(concat(prefix, split.outer), outer, ApplySplitResult::LetStmt);

        // Maintain the known size of the fused dim if
        // possible. This is important for possible later splits.
        StringMap<Expr>::iterator inner_dim = dim_extent_alignment.find(split.inner);
        StringMap<Expr>::iterator outer_dim = dim_extent_alignment.find(split.outer);
        ...
        // rename or purify
        result.emplace_back(concat(prefix, split.old_var), outer, ApplySplitResult::Substitution);
        result.emplace_back(concat(prefix, split.old_var), outer, ApplySplitResult::LetStmt);
    }

    return result;
}

vector<std::pair<string, Expr>> compute_loop_bounds_after_split(const Split &split, std::string_view prefix) {
    ...
    Expr old_var_extent = Variable::make(Int(32), concat(prefix, split.old_var, ".loop_extent"));
    Expr old_var_max = Variable::make(Int(32), concat(prefix, split.old_var, ".loop_max"));
    Expr old_var_min = Variable::make(Int(32), concat(prefix, split.old_var, ".loop_min"));
    if (split.is_split()) {
        Expr inner_extent = split.factor;
        Expr outer_extent = (old_var_max - old_var_min + split.factor) / split.factor;
        let_stmts.emplace_back(concat(prefix, split.inner, ".loop_min"), 0);
        let_stmts.emplace_back(concat(prefix, split.inner, ".loop_max"), inner_extent - 1);
        let_stmts.emplace_back(concat(prefix, split.inner, ".loop_extent"), inner_extent);
        let_stmts.emplace_back(concat(prefix, split.outer, ".loop_min"), 0);
        let_stmts.emplace_back(concat(prefix, split.outer, ".loop_max"), outer_extent - 1);
        let_stmts.emplace_back(concat(prefix, split.outer, ".loop_extent"), outer_extent);
    } else if (split.is_fuse()) {
        // Define bounds on the fused var using the bounds on the inner and outer
        Expr inner_extent = Variable::make(Int(32), concat(prefix, split.inner, ".loop_extent"));
        Expr outer_extent = Variable::make(Int(32), concat(prefix, split.outer, ".loop_extent"));
        Expr fused_extent = inner_extent * outer_extent;
        let_stmts.emplace_back(concat(prefix, split.old_var, ".loop_min"), 0);
        let_stmts.emplace_back(concat(prefix, split.old_var, ".loop_max"), fused_extent - 1);
        let_stmts.emplace_back(concat(prefix, split.old_var, ".loop_extent"), fused_extent);
    } else if (split.is_rename()) {
        let_stmts.emplace_back(concat(prefix, split.outer, ".loop_min"), old_var_min);
        let_stmts.emplace_back(concat(prefix, split.outer, ".loop_max"), old_var_max);
        let_stmts.emplace_back(concat(prefix, split.outer, ".loop_extent"), old_var_extent);
    }
