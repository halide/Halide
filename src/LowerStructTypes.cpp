...
class LowerStructTypesMutator : public IRMutator {
    ...
    Expr lower_field_read_from_load(const Load *load, const Type &field_type, int field_base_offset, const Expr &elem_index_in) {
        ...
        // *bytes* (true for every ordinary scalar type), but a struct's bits()
        // is not its (packed, possibly not power-of-two) byte size, so
        // Reinterpret would be asked to reinterpret between mismatched bit
        // widths. Nested struct fields read from a literal pack_struct() go
        // through fold_field_of_pack instead and don't hit this at all; only
        // reading a nested field from a (non-inlined) struct-typed buffer is
        // unsupported.
