...
class LowerStructTypesMutator : public IRMutator {
    ...
    // Struct-typed Let bindings (e.g. introduced by common_subexpression_elimination
    // hoisting a repeated struct-typed value) are tracked here rather than kept
    // as an opaque Variable: project_field() needs direct syntactic access to
    // the underlying struct_pack()/Select/Load, and since a struct-typed value
    // is never itself materialized (only its individual fields are), inlining
    // it back in at each field() use site costs nothing at runtime -- any
    // shared sub-computation gets re-discovered by the ordinary CSE pass that
    // runs later in Lower.cpp.
    Scope<Expr> struct_lets;

    Expr resolve_struct_value(Expr e) {
    ...
    // A literal pack_struct() being projected: pick out the relevant
    // element(s) of its args directly, no bytes ever materialized. This is
    // what makes a struct-valued Func that ends up compute_inline()'d, or a
    // literal pack_struct() used directly as an Expr, never pay for any
    // memory traffic at all.
    Expr fold_field_of_pack(const Call *pack, const StructTypeInfo &info, int field_index, const Expr &elem_index) {
        ...
        // A dynamic index into a literal, unmaterialized struct: nothing to
        // load, so build a select-chain gather over this field's own
        // elements instead. Not vectorized specially, but correct, and this
        // is an uncommon case (a runtime-dynamic index into an inlined
        // struct producer, rather than into a real buffer, where a Load
        // handles a dynamic index for free).
    ...
    // field(e, field_index)[elem_index], where e is a real (already
    // flattened) struct-typed Load -- a direct load from a struct-typed
    // ImageParam, Buffer, or Func's own storage. Rewrite into one or more
    // byte-addressed Loads plus a Reinterpret, mirroring exactly how this
    // would be hand-written against a raw byte buffer -- see delta_naive in
    // doc/StructTypeDesign.md's Motivation section. Combining independent
    // byte reads via widen+shift+or (matching delta_naive exactly), rather
    // than a single load of the field's own type, is deliberate: the
    // struct's *total* byte size need not be a multiple of the field's
    // element size (e.g. an odd-sized struct with a 4-byte field), so no
    // single fixed "index unit" is guaranteed to address every instance of
    // the field correctly. LLVM's own backend is what then fuses adjacent
    // byte loads into a single wider load where the alignment does work out
    // -- confirmed directly against generated assembly.
    Expr lower_field_read_from_load(const Load *load, Type field_type, int field_base_offset, const Expr &elem_index_in) {
        ...
        // struct fields read from a literal pack_struct() (the fully
        // inlined case) go through fold_field_of_pack instead and don't hit
        // this at all; only reading a nested field from a genuine
        // (non-inlined) struct-typed buffer is unsupported in v1.
        user_assert(!field_type.is_struct())
            << "Reading a nested struct field (a field whose own type is Type::Struct) directly "
            << "from a struct-typed buffer/Func that isn't fully inlined is not supported in v1. "
    ...
    Expr project_field(const Expr &struct_expr, int field_index, const Expr &elem_index) {
        ...
        // e is itself an as-yet-unresolved nested field() access (e.g.
        // `field(field(outer, "inner"), "a")`, written by the user as two
        // separate front-end field() calls) -- resolve the inner one first,
        // then continue projecting the requested field out of *that*
        // result. This is what makes a struct field whose own type is
        // itself Type::Struct (a sub-struct field) work.
    ...
    Expr visit(const Call *op) override {
        ...
        if (op->is_intrinsic(Call::struct_pack)) {
            // A literal pack_struct() Expr not (yet) consumed by field() --
            // e.g. it's still on its way to becoming a struct-typed Store's
            // value (see visit(Store*)), or it's nested inside another
            // pack_struct()'s field values. Mutate its args generically
            // (any nested field()/pack_struct() calls within them still need
            // lowering) and preserve the node itself; it's eliminated at its
            // actual point of consumption, not here.
            return IRMutator::visit(op);
        }

        if (op->type.is_struct()) {
            user_error << "Struct-typed Expr \"" << Expr(op) << "\" is not the direct argument of "
                       << "field(), and is not a struct-typed Store's own value. In v1, a "
                       << "struct-typed Expr may only appear in one of those two positions -- it "
                       << "can't be passed to an arbitrary intrinsic or stored in a Let generically.\n";
        }
    ...
    Expr visit(const Let *op) override {
        if (op->value.type().is_struct()) {
            ScopedBinding<Expr> bind(struct_lets, op->name, op->value);
...
// Defensive sanity net: after LowerStructTypesMutator runs, no struct-typed
// node of any kind should remain anywhere in the Stmt -- everything
// downstream (bounds inference already ran; simplification, vectorization,
// codegen prep are still ahead) assumes only ordinary scalar/vector types.
class CheckNoStructTypesRemain : public IRGraphVisitor {
    using IRGraphVisitor::visit;

    void fail(const Expr &e, const char *what) {
