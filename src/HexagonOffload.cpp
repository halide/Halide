    ...
    Stmt visit(const For *loop) override {
        ...
        for (const auto &buf : c.buffers) {
            if (buf.second.write) {
                ...
                output_buffers.emplace_back(buf.first, kind, buf.second.type, buf.second.dimensions, ArgumentEstimates{});
            } else {
                Argument::Kind kind = Argument::InputBuffer;
                input_buffers.emplace_back(buf.first, kind, buf.second.type, buf.second.dimensions, ArgumentEstimates{});
            }

            // Build a parameter to replace.
            Parameter p(buf.second.type, true, buf.second.dimensions);
            ...
            // alignment.
            replacement_params[buf.first] = p;
            ...
            if (!device_code.target().has_feature(Target::NoAsserts)) {
                Expr host_ptr = reinterpret<uint64_t>(Variable::make(Handle(), buf.first));
                Expr error = Call::make(Int(32), "halide_error_unaligned_host_ptr",
                                        {buf.first, alignment}, Call::Extern);
            ...
            Expr buffer_var = Variable::make(Handle(), buf.first + ".buffer");
            Expr host_ptr = Call::make(Handle(), "_halide_hexagon_buffer_get_host", {buffer_var}, Call::Extern);
            Expr device_ptr = Call::make(Handle(), "_halide_hexagon_buffer_get_device", {buffer_var}, Call::Extern);
            body = LetStmt::make(buf.first + ".device", device_ptr, body);
            body = LetStmt::make(buf.first, host_ptr, body);
        }
        ...
        for (const auto &v : c.vars) {
            LoweredArgument arg(v.first, Argument::InputScalar, v.second, 0, ArgumentEstimates{});
            args.push_back(arg);
        }
        ...
        for (const auto &buf : c.buffers) {
            ...
            if (buf.first != scalars_buffer_name) {
                // If this isn't the scalars buffer, assume it has a '.buffer'
                // description in the IR.
                Expr buffer_var = Variable::make(type_of<halide_buffer_t *>(), buf.first + ".buffer");
                Expr device = Call::make(UInt(64), Call::buffer_get_device, {buffer_var}, Call::Extern);
                Expr host = Call::make(Handle(), Call::buffer_get_host, {buffer_var}, Call::Extern);
                ...
                Expr host = Variable::make(Handle(), buf.first);
            ...
            int flags = 0;
            if (buf.second.read) flags |= 0x1;
            if (buf.second.write) flags |= 0x2;
            arg_flags.emplace_back(flags);
        }
        for (const auto &v : c.vars) {
            Expr arg = Variable::make(v.second, v.first);
            Expr arg_ptr = Call::make(type_of<void *>(), Call::make_struct, {arg}, Call::Intrinsic);
            arg_sizes.emplace_back((uint64_t)v.second.bytes());
