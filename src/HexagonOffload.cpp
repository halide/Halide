...
public:
    ...
    Symbol add_plt_entry(const Symbol &sym, Section &plt, Section &got, const Symbol &got_sym) override {
        ...
        Symbol plt_sym("plt_" + std::string{sym.get_name()});
        plt_sym
...
class ReplaceParams : public IRMutator {
    const StringMap<Parameter> &replacements;
...
public:
    ReplaceParams(const StringMap<Parameter> &replacements)
        : replacements(replacements) {
    }
};

Stmt replace_params(const Stmt &s, const StringMap<Parameter> &replacements) {
    return ReplaceParams(replacements).mutate(s);
}

class InjectHexagonRpc : public IRMutator {
    StringMap<Expr> state_bufs;

    Module &device_code;

    Expr state_var_ptr(std::string_view name, Type type) {
        Expr &buf = state_bufs[name];
        if (!buf.defined()) {
            auto storage = Buffer<void *>::make_scalar(concat(name, "_buf"));
            storage() = nullptr;
            buf = Variable::make(type_of<halide_buffer_t *>(),
                                 concat(storage.name(), ".buffer"), storage);
        }
    ...
    Stmt visit(const For *loop) override {
        ...
        std::string hex_name = unique_name(concat("offload_rpc.", loop->name));
        ...
        StringMap<Parameter> replacement_params;
        for (const auto &i : c.buffers) {
            ...
            if (!device_code.target().has_feature(Target::NoAsserts)) {
                ...
                Expr error = Call::make(Int(32), "halide_error_unaligned_host_ptr",
                                        {Expr(i.first), alignment}, Call::Extern);
...
Buffer<uint8_t> compile_module_to_hexagon_shared_object(const Module &device_code) {
    ...
    std::string soname = concat("lib", device_code.name(), ".so");
    ...
    if (!signer.empty()) {
        ...
        std::string cmd = concat(signer, " ", input.pathname(), " ", output.pathname());
