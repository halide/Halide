        ...
        if (!is_const_one(s->predicate)) {
            ...
            for (int ln = 0; ln < s->value.type().lanes(); ln++) {
                scalar_stmts.push_back(IfThenElse::make(
                    ...
                    s->remake(mutate(extract_lane(s->value, ln)), mutate(extract_lane(s->index, ln)), const_true(), s->alignment + ln)));
            }
    ...
    Expr visit(const Load *op) override {
        if (!is_const_one(op->predicate)) {
            ...
            for (int ln = 0; ln < op->type.lanes(); ln++) {
                Expr load_expr = op->remake(extract_lane(op->index, ln), const_true(), op->alignment + ln);
