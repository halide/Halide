  ...
  match expr with
    ...
    | Bop (op, a, b) ->
      begin match (op, recurse a, recurse b) with
        ...
        | (Div, x, y) when is_const_zero x -> make_zero (val_type_of_expr x)
        ...
        *)
        ...
        | (Add, Bop (Mul, y1, x1), Bop (Mul, y2, x2)) when y1 = y2 ->            
            recurse ((x1 +~ x2) *~ y1)
        | (Add, Bop (Mul, x1, y1), Bop (Mul, y2, x2)) when y1 = y2 ->
            recurse ((x1 +~ x2) *~ y1)            
        | (Add, Bop (Mul, y1, x1), Bop (Mul, x2, y2)) when y1 = y2 ->
            recurse ((x1 +~ x2) *~ y1)
        | (Add, Bop (Mul, x1, y1), Bop (Mul, x2, y2)) when y1 = y2 ->
            recurse ((x1 +~ x2) *~ y1)
        | (Sub, Bop (Mul, y1, x1), Bop (Mul, y2, x2)) when y1 = y2 ->
            recurse ((x1 -~ x2) *~ y1)            
        | (Sub, Bop (Mul, x1, y1), Bop (Mul, y2, x2)) when y1 = y2 ->
            recurse ((x1 -~ x2) *~ y1)            
        | (Sub, Bop (Mul, y1, x1), Bop (Mul, x2, y2)) when y1 = y2 ->
            recurse ((x1 -~ x2) *~ y1)            
        | (Sub, Bop (Mul, x1, y1), Bop (Mul, x2, y2)) when y1 = y2 ->
            recurse ((x1 -~ x2) *~ y1)
        ...
        | (Min, Bop (Add, x, IntImm a), y) when x = y ->
            if a < 0 then x +~ IntImm a else x
        | (Min, Bop (Add, x, y), Bop (Add, y2, x2))
        | (Max, Bop (Add, x, y), Bop (Add, y2, x2)) when x = x2 && y = y2 ->
          x +~ y
    ...
    | Cmp (op, a, b) ->
      begin match (recurse a, recurse b) with
        ...
        | (x, y) when x = y ->
          begin match op with
            | GE | LE | EQ -> bool_imm true
            | _ -> bool_imm false
          end
        (* These patterns are often generated by lowering *)
        | (x, Bop (Sub, y, IntImm k)) when x = y -> bool_imm (caml_op_of_cmp op 0 (-k))
        | (x, Bop (Add, y, IntImm k)) when x = y -> bool_imm (caml_op_of_cmp op 0 k)
        | (Bop (Add, y, IntImm k), x) when x = y -> bool_imm (caml_op_of_cmp op k 0)
        | (Bop (Sub, y, IntImm k), x) when x = y -> bool_imm (caml_op_of_cmp op (-k) 0)
        | (x, y) -> Cmp (op, x, y)
      end
    ...
    | And (a, b) ->
      begin match (recurse a, recurse b) with
        ...
        | (_, Cast(UInt 1, IntImm 0)) -> bool_imm false
    ...
    | Or (a, b) ->
      begin match (recurse a, recurse b) with
        ...
        | (_, Cast(UInt 1, IntImm 1)) -> bool_imm true
    ...
    | Not a ->
      begin match recurse a with
        ...
        | Cast(UInt 1, IntImm 0) -> bool_imm true
        | Cast(UInt 1, IntImm 1) -> bool_imm false
        | x -> Not x
      end
    | Select (c, a, b) ->
...
let constant_fold_stmt stmt =
  let rec inner env = function
    ...
    | LetStmt (name, value, stmt) ->        
        ...
        let rec scoped_subs_expr replacement = function
          | Let (n, v, e) when n = name -> Let (n, scoped_subs_expr replacement v, e)
          | Var (_, n) when n = name -> replacement
          | expr -> mutate_children_in_expr (scoped_subs_expr replacement) expr
        in
        let rec scoped_subs_stmt replacement = function
          | LetStmt (n, v, s) when n = name -> LetStmt (n, scoped_subs_expr replacement v, s)
          | stmt -> mutate_children_in_stmt (scoped_subs_expr replacement) (scoped_subs_stmt replacement) stmt
        in
        if (is_simple value) then begin
          LetStmt (name, value, inner env (scoped_subs_stmt value stmt))
        end else begin
          try begin
            (* Check if this value already had a name *)
            let (n, v) = List.find (fun (n, v) -> v = value) env in
            LetStmt (name, value, inner env (scoped_subs_stmt (Var (t, n)) stmt))
...
let remove_dead_lets_in_stmt stmt =
  ...
  let rec inner_expr = function
    | Var (ty, name) ->      
      begin try
        ...
        Hashtbl.add symtab name (usage+1);
        Var (ty, name)
      with Not_found ->
        Var (ty, name)
      end
    | Let (name, value, expr) ->
      ...
      let new_expr = match Hashtbl.find symtab name with 
        | 0 -> expr
        | 1 -> subs_expr (Var (val_type_of_expr value, name)) value expr
        | _ -> Let (name, value, expr)          
      in
      Hashtbl.remove symtab name;
      new_expr
    | expr -> mutate_children_in_expr inner_expr expr
  in
  ...
  let rec inner_stmt = function
    | LetStmt (name, value, stmt) ->
      ...
      (* Check the usage count *)
      let new_stmt = match Hashtbl.find symtab name with 
        | 0 -> stmt
        | 1 -> subs_expr_in_stmt (Var (val_type_of_expr value, name)) value stmt
        | _ -> LetStmt (name, value, stmt)
      in
      Hashtbl.remove symtab name;
      new_stmt
    | stmt -> mutate_children_in_stmt inner_expr inner_stmt stmt
  in
