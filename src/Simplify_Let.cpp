...
Body Simplify::simplify_let(const T *op, ConstBounds *bounds) {
    ...
    struct Frame {
        ...
        Frame(const T *op, Simplify *s, Body *result) : op(op), s(s), result(result) {

            internal_assert(!s->var_info.contains(op->name))
            ...
            value = s->mutate(op->value, &value_bounds);
            ...
            s->var_info.push(op->name, info);

            // Before we enter the body, track the alignment info

            if (new_value.defined() && s->no_overflow_scalar_int(new_value.type())) {
                ModulusRemainder mod_rem = modulus_remainder(new_value, s->alignment_info);
                if (mod_rem.modulus > 1) {
                    s->alignment_info.push(new_name, mod_rem);
                    new_value_alignment_tracked = true;
                }
                ConstBounds new_value_bounds;
                new_value = s->mutate(new_value, &new_value_bounds);
                if (new_value_bounds.min_defined || new_value_bounds.max_defined) {
                    s->bounds_info.push(new_name, new_value_bounds);
                    new_value_bounds_tracked = true;
                }
            }

            if (s->no_overflow_scalar_int(value.type())) {
                ModulusRemainder mod_rem = modulus_remainder(value, s->alignment_info);
                if (mod_rem.modulus > 1) {
                    s->alignment_info.push(op->name, mod_rem);
                    value_alignment_tracked = true;
                }
                if (value_bounds.min_defined || value_bounds.max_defined) {
                    s->bounds_info.push(op->name, value_bounds);
        ...
        ~Frame() {
            if (!value.defined()) return; // Was moved from during vector resize

            if (value_alignment_tracked) {
                s->alignment_info.pop(op->name);
            }
            if (value_bounds_tracked) {
                s->bounds_info.pop(op->name);
            }
            if (new_value_alignment_tracked) {
                s->alignment_info.pop(new_name);
            }
            if (new_value_bounds_tracked) {
                s->bounds_info.pop(new_name);
            }

            VarInfo info = s->var_info.get(op->name);
            s->var_info.pop(op->name);
            ...
            if (info.old_uses > 0 || !s->remove_dead_lets) {
...

    {
        ...
        result = mutate_let_body(body, bounds);
    }

    // The destructors of the Frame objects have rewrapped the result
    // with the mutated lets as appropriate.
    return result;
}
