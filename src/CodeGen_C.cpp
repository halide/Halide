...
// The binary2cpp-generated blobs above have internal linkage as far as the
// exported-symbols list for libHalide is concerned (only `halide_*` C-style
// symbols that are actually part of the runtime API get exported, along with
// the C++-mangled Halide:: namespace), so test/correctness/codegen_c.cpp
// can't reference them directly; these accessors re-expose them for testing.
string codegen_c_test_prologue_source() {
    return normalize_line_endings(halide_c_template_CodeGen_C_prologue);
}

string codegen_c_test_runtime_header_source() {
    return normalize_line_endings(halide_internal_runtime_header_HalideRuntime_h);
}

string codegen_c_test_inlined_c_source() {
    return normalize_line_endings(halide_internal_initmod_inlined_c);
}
