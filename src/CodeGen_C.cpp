    ...
    if (output_kind != CPlusPlusFunctionInfoHeader) {
        ...
        if (is_header_or_extern_decl()) {
            ...
            if (uses_gpu_for_loops) {
                ...
                // aliasing __user_context or nullptr. Note that we always const-cast this to void*,
                // so that we can freely pass it to halide runtime functions (which take void*),
                // or define-extern functions (which take void const *).
                stream << get_indent() << "void *_ucon = "
                       << (have_user_context ? "const_cast<void *>(__user_context)" : "nullptr")
                       << ";\n";
...
int test1(struct halide_buffer_t *_buf_buffer, float _alpha, int32_t _beta, void const *__user_context) {
 void *_ucon = const_cast<void *>(__user_context);
