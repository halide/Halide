 ...
 * in the device field of the buffer_t. If the buffer is allocated in
 * a particular way (ion_alloc), the buffer will be shared with
 * Hexagon (not copied). The dev field of the buffer_t must be NULL
 * when this routine is called. This call can fail due to running out
 * of memory or being passed an invalid device handle. The device and
 * host dirty bits are left unmodified. */
...
extern size_t halide_hexagon_get_device_size(void *user_context, struct halide_buffer_t *buf);
