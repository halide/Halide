...
WEAK int halide_metal_buffer_copy(void *user_context, struct halide_buffer_t *src,
    ...
    if (src->device_dirty() &&
        ...
        // If the source is not metal or host memory, ask the source
        // device interface to copy to dst host memory first.
        // TODO: this probably does not set the appropriate dirty bit. Probably needs to call up to generic routine. Fix CUDA too.
