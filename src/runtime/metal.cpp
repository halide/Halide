...
WEAK void set_input_texture(mtl_compute_command_encoder *encoder, mtl_texture *input_texture, uint32_t index) {
    typedef void (*set_texture_method)(objc_id encoder, objc_sel sel,
                                       mtl_texture *input_texture, size_t index);
    set_texture_method method = (set_texture_method)&objc_msgSend;
    (*method)(encoder, sel_getUid("setTexture:atIndex:"),
              input_texture, index);
}
...
extern WEAK halide_device_interface metal_textures_device_interface;
...
WEAK int common_device_malloc(void *user_context, buffer_t* buf, bool texture) {
    debug(user_context)
...
WEAK int common_device_free(void *user_context, buffer_t* buf) {
    ...
    objc_id metal_storage = (objc_id)halide_get_device_handle(buf->dev);
    release_ns_object(metal_storage);
...
WEAK int common_metal_run(void *user_context,
                           void *state_ptr,
    ...
    for (size_t i = 0; arg_sizes[i] != 0; i++) {
        if (arg_is_buffer[i]) {
            ...
            if (textures) {
                mtl_texture *metal_texture = (mtl_texture *)halide_get_device_handle(*(uint64_t *)args[i]);
                set_input_texture(encoder, metal_texture, texture_index);
                texture_index++;
            } else {
    ...
    #ifdef DEBUG_RUNTIME
    uint64_t t_after = halide_current_time_ns(user_context);
    debug(user_context) << "Time for halide_metal_run: " << (t_after - t_before) / 1.0e6 << " ms\n";
    #endif
...
WEAK int halide_metal_device_malloc(void *user_context, buffer_t* buf) {
    return common_device_malloc(user_context, buf, false);
}

WEAK int halide_metal_textures_device_malloc(void *user_context, buffer_t* buf) {
    return common_device_malloc(user_context, buf, true);
}

WEAK int halide_metal_device_free(void *user_context, buffer_t* buf) {
    return common_device_free(user_context, buf);
}

WEAK int halide_metal_textures_device_free(void *user_context, buffer_t* buf) {
    return common_device_free(user_context, buf);
}
...
WEAK int halide_metal_textures_copy_to_device(void *user_context, buffer_t* buffer) {
    // TODO(zalman): Implement.
    return -1;
}

WEAK int halide_metal_textures_copy_to_host(void *user_context, buffer_t* buffer) {
    // TODO(zalman): Implement.
    return -1;
}
...
WEAK int halide_metal_run(void *user_context,
                          ...
                          int num_coords_dim0,
                          int num_coords_dim1) {
    return common_metal_run(user_context, state_ptr, entry_name,
                            blocksX, blocksY, blocksZ,
                            threadsX, threadsY, threadsZ,
...
WEAK int halide_metal_textures_run(void *user_context,
                                   void *state_ptr,
                                   const char* entry_name,
                                   int blocksX, int blocksY, int blocksZ,
                                   int threadsX, int threadsY, int threadsZ,
...
WEAK const struct halide_device_interface *halide_metal_textures_device_interface() {
    return &metal_textures_device_interface;
}
...
WEAK halide_device_interface metal_textures_device_interface = {
    halide_use_jit_module,
    halide_release_jit_module,
    halide_metal_textures_device_malloc,
    halide_metal_textures_device_free,
    halide_metal_device_sync,
    halide_metal_device_release,
    halide_metal_textures_copy_to_host,
    halide_metal_textures_copy_to_device,
};
