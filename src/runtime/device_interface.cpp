...
WEAK halide_error_code_t copy_to_host_already_locked(void *user_context, struct halide_buffer_t *buf) {
    if (!buf->device_dirty()) {
        return halide_error_code_success;  // my, that was easy
    }
    ...
    if (buf->host_dirty()) {
        ...
        return report_error_copy_to_host_failed(user_context);
    }
    if (interface == nullptr) {
        debug(user_context) << "copy_to_host_already_locked " << buf << " interface is nullptr\n";
        return report_error_no_device_interface(user_context);
    }
    ...
    if (result != 0) {
        ...
        return report_error_copy_to_host_failed(user_context, "(", result, ")");
    }
...
struct UseModule final {
    const halide_device_interface_t *const device_interface;

    explicit UseModule(const halide_device_interface_t *device_interface)
...
int call_device_interface(const halide_device_interface_t *device_interface, Fn fn, Args &&...args) {
    UseModule use_module(device_interface);

    // The static_cast<Args&&> is our homegrown version of std::forward
...
halide_error_code_t debug_log_and_validate_buf(void *user_context, const halide_buffer_t *buf_arg, const char *function = __builtin_FUNCTION()) {
    ...
    debug(user_context) << function << " validating input buffer: " << buf << "\n";

    bool device_interface_set = (buf.device_interface != nullptr);
    bool device_set = (buf.device != 0);
    if (device_set && !device_interface_set) {
        return report_error_no_device_interface(user_context, function);
    }
    if (device_interface_set && !device_set) {
        return report_error_device_interface_no_device(user_context, function);
    }

    bool host_dirty = buf.host_dirty();
    bool device_dirty = buf.device_dirty();
    if (host_dirty && device_dirty) {
        return report_error_host_and_device_dirty(user_context, function);
    }
    /* TODO: we could test:
...
WEAK int halide_copy_to_host(void *user_context, struct halide_buffer_t *buf) {
    ...
    if (auto result = debug_log_and_validate_buf(user_context, buf); result != halide_error_code_success) {
        return result;
    }
...
WEAK int copy_to_device_already_locked(void *user_context,
    ...
    if (buf->device && buf->device_interface != device_interface) {
        return report_error_incompatible_device_interface(user_context, "halide_copy_to_device does not support switching interfaces");
    }

    if (buf->device == 0) {
        if (auto result = halide_device_malloc(user_context, buf, device_interface); result != halide_error_code_success) {
    ...
    if (buf->host_dirty()) {
        ...
        if (buf->device_dirty()) {
            ...
            return report_error_copy_to_device_failed(user_context);
        } else {
            debug(user_context) << "halide_copy_to_device " << buf << " calling copy_to_device()\n";
            if (auto result = device_interface->impl->copy_to_device(user_context, buf); result != halide_error_code_success) {
...
WEAK int halide_device_sync(void *user_context, struct halide_buffer_t *buf) {
    ...
    return device_interface->impl->device_sync(user_context, buf);
}
...
WEAK int halide_device_free(void *user_context, struct halide_buffer_t *buf) {
    ...
    if (device_interface != nullptr) {
        ...
        if (buf->device != 0) {
            return report_error_device_free_failed(user_context, "buf->device is nonzero");
        }
    } else {
        buf->set_device_dirty(false);
    }
...
WEAK int halide_device_and_host_malloc(void *user_context, struct halide_buffer_t *buf,
    ...
    // halide_device_malloc does not support switching interfaces.
    if (current_interface != nullptr && current_interface != device_interface) {
        return report_error_incompatible_device_interface(user_context, "halide_device_and_host_malloc doesn't support switching interfaces");
    }
...
WEAK int halide_device_and_host_free(void *user_context, struct halide_buffer_t *buf) {
    ...
    if (device_interface != nullptr) {
        ...
        if (buf->device != 0) {
            return report_error(user_context, "buf->device is nonzero");
        }
        return halide_error_code_success;
    } else if (buf->host) {
...
WEAK int halide_default_device_and_host_malloc(void *user_context, struct halide_buffer_t *buf,
    ...
    if (buf->host == nullptr) {
        return report_error_out_of_memory(user_context);
    }
    if (auto result = halide_device_malloc(user_context, buf, device_interface); result != halide_error_code_success) {
        halide_free(user_context, buf->host);
        buf->host = nullptr;
...
WEAK int halide_default_device_and_host_free(void *user_context, struct halide_buffer_t *buf,
    ...
    auto result = halide_device_free(user_context, buf);
    if (buf->host) {
        halide_free(user_context, buf->host);
        buf->host = nullptr;
    }
...
WEAK int halide_device_wrap_native(void *user_context, struct halide_buffer_t *buf, uint64_t handle,
    ...
    if (current_interface != nullptr && current_interface != device_interface) {
        return report_error_incompatible_device_interface(user_context, "halide_device_wrap_native doesn't support switching interfaces");
    }
...
WEAK int halide_default_device_wrap_native(void *user_context, struct halide_buffer_t *buf, uint64_t handle) {
    ...
    //
    // int result = debug_log_and_validate_buf(user_context, buf);
    // if (result != 0) {
    //     return result;
    // }
    if (buf->device != 0) {
        return report_error_device_wrap_native_failed(user_context, "buf->device is nonzero");
    }
...
WEAK int halide_default_buffer_copy(void *user_context, struct halide_buffer_t *src,
    ...
    return report_error_device_buffer_copy_failed(user_context);
}
...
WEAK int halide_buffer_copy_already_locked(void *user_context, struct halide_buffer_t *src,
    ...
    if (dst_device_interface && dst->device_interface &&
        ...
        return report_error_incompatible_device_interface(user_context, "halide_buffer_copy does not support switching device interfaces");
    }
    ...
    int result = halide_error_code_incompatible_device_interface;
    if (from_device_valid && to_device) {
        debug(user_context) << "halide_buffer_copy_already_locked: device to device case.\n";
        result = dst_device_interface->impl->buffer_copy(user_context, src, dst_device_interface, dst);
    }

    if (result == halide_error_code_incompatible_device_interface) {
        ...
        if (!from_host_exists && !to_host_exists) {
            ...
            return report_error_incompatible_device_interface(user_context);
        }
        ...
        if (to_host && from_host_valid) {
            ...
            result = src->device_interface->impl->buffer_copy(user_context, src, nullptr, dst);
            // Return on success or an error indicating something other
            // than not handling this case went wrong.
            if (result == halide_error_code_incompatible_device_interface) {
                result = copy_to_host_already_locked(user_context, src);
                if (result == halide_error_code_success) {
                    result = halide_buffer_copy_already_locked(user_context, src, nullptr, dst);
                }
            }
        } else {
            if (from_device_valid && to_host_exists) {
                ...
                if (result == halide_error_code_success) {
                    ...
                    result = copy_to_device_already_locked(user_context, dst, dst_device_interface);
                }
            } else if (to_device) {
    ...
    if (result != halide_error_code_success) {
        debug(user_context) << "halide_buffer_copy_already_locked: got error " << result << ".\n";
    } else {
        if (dst != src) {
...
WEAK int halide_buffer_copy(void *user_context, struct halide_buffer_t *src,
    ...
    UseModule use_dst(dst_device_interface), use_src(src->device_interface);
...
WEAK int halide_default_device_crop(void *user_context,
                                    ...
                                    struct halide_buffer_t *dst) {
    return report_error_device_crop_unsupported(user_context, "device_interface does not support cropping");
}
...
WEAK int halide_default_device_slice(void *user_context,
                                     ...
                                     struct halide_buffer_t *dst) {
    return report_error_device_crop_unsupported(user_context, "device_interface does not support slicing");
}
...
WEAK int halide_device_crop(void *user_context,
    ...
    if (dst->device) {
        return report_error_device_crop_failed(user_context, "destination buffer already has a device allocation");
    }

    if (src->dimensions != dst->dimensions) {
        return report_error_device_crop_failed(user_context, "src and dst must have identical dimensionality");
    }

    src->device_interface->impl->use_module();
    auto result = src->device_interface->impl->device_crop(user_context, src, dst);
...
WEAK int halide_device_slice(void *user_context,
    ...
    if (src->dimensions != dst->dimensions + 1) {
        return report_error_device_crop_failed(user_context, "dst must have exactly one fewer dimension than src");
    }

    src->device_interface->impl->use_module();
    auto result = src->device_interface->impl->device_slice(user_context, src, slice_dim, slice_pos, dst);
...
WEAK int halide_device_release_crop(void *user_context,
    ...
    if (buf->device) {
        ...
        auto result = interface->impl->device_release_crop(user_context, buf);
        buf->device = 0;
