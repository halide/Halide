...
WEAK CUmodule compile_kernel(void *user_context, CUcontext ctx, const char *ptx_src, int size) {
    ...
    // Tensor core instructions are left for the runtime to finish off, because
    // how the hardware lays a fragment out over the lanes of a warp is only
    // discoverable by trying it.
    const char *finished = finish_wmma_markers(user_context, ctx, ptx_src);
    if (!finished) {
        return nullptr;
    }
    ...
    CUresult err = cuModuleLoadData(&loaded_module, finished);
    if (finished != ptx_src) {
        free((void *)finished);
    }
