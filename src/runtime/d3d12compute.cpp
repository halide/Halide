...
#endif
#define WIN32
#define RenderDocAssert(expr) HALIDE_CHECK(user_context, expr)
...
static DXGI_FORMAT FindD3D12FormatForHalideType(void *user_context, halide_type_t type) {
    ...
    HALIDE_CHECK(user_context, (type.code >= 0) && (type.code <= 2));
    HALIDE_CHECK(user_context, (type.lanes > 0) && (type.lanes <= 4));
...
static size_t number_of_elements(void *user_context, const halide_buffer_t *buffer) {
    ...
    HALIDE_CHECK(user_context, (size_in_bytes > 0));

    size_t element_size = 1;
    element_size *= buffer->type.bytes();
    element_size *= buffer->type.lanes;
    HALIDE_CHECK(user_context, (element_size > 0));

    size_t elements = size_in_bytes / element_size;
    HALIDE_CHECK(user_context, (size_in_bytes % element_size) == 0);
...
static d3d12_buffer *peel_buffer(struct halide_buffer_t *hbuffer) {
    TRACELOG;
    HALIDE_CHECK(user_context, (hbuffer != nullptr));
    HALIDE_CHECK(user_context, (hbuffer->device_interface == &d3d12compute_device_interface));
    d3d12_buffer *dbuffer = reinterpret_cast<d3d12_buffer *>(hbuffer->device);
    HALIDE_CHECK(user_context, (dbuffer != nullptr));
    return dbuffer;
}
...
WEAK int wrap_buffer(void *user_context, struct halide_buffer_t *hbuffer, d3d12_buffer *dbuffer) {
    HALIDE_CHECK(user_context, (hbuffer->device == 0));
    if (hbuffer->device != 0) {
        return halide_error_code_device_wrap_native_failed;
    }

    HALIDE_CHECK(user_context, (dbuffer->resource != nullptr));
    ...
    HALIDE_CHECK(user_context, (hbuffer->device_interface == nullptr));
...
static void D3D12LoadDependencies(void *user_context) {
    ...
    HALIDE_CHECK(user_context, rdinit);
#endif
#endif
    ...
    HALIDE_CHECK(user_context, sizeof(BOOL) == (32 / 8));      // BOOL      must be  32 bits
    HALIDE_CHECK(user_context, sizeof(CHAR) == (8 / 8));       // CHAR      must be   8 bits
    HALIDE_CHECK(user_context, sizeof(SHORT) == (16 / 8));     // SHORT     must be  16 bits
    HALIDE_CHECK(user_context, sizeof(LONG) == (32 / 8));      // LONG      must be  32 bits
    HALIDE_CHECK(user_context, sizeof(ULONG) == (32 / 8));     // ULONG     must be  32 bits
    HALIDE_CHECK(user_context, sizeof(LONGLONG) == (64 / 8));  // LONGLONG  must be  16 bits
    HALIDE_CHECK(user_context, sizeof(BYTE) == (8 / 8));       // BYTE      must be   8 bits
    HALIDE_CHECK(user_context, sizeof(WORD) == (16 / 8));      // WORD      must be  16 bits
    HALIDE_CHECK(user_context, sizeof(DWORD) == (32 / 8));     // DWORD     must be  32 bits
    HALIDE_CHECK(user_context, sizeof(WCHAR) == (16 / 8));     // WCHAR     must be  16 bits
    HALIDE_CHECK(user_context, sizeof(INT) == (32 / 8));       // INT       must be  32 bits
    HALIDE_CHECK(user_context, sizeof(UINT) == (32 / 8));      // UINT      must be  32 bits
    HALIDE_CHECK(user_context, sizeof(IID) == (128 / 8));      // COM GUIDs must be 128 bits

    // Paranoid checks (I am not taking any chances...)
    HALIDE_CHECK(user_context, sizeof(INT8) == (8 / 8));
    HALIDE_CHECK(user_context, sizeof(INT16) == (16 / 8));
    HALIDE_CHECK(user_context, sizeof(INT32) == (32 / 8));
    HALIDE_CHECK(user_context, sizeof(INT64) == (64 / 8));
    HALIDE_CHECK(user_context, sizeof(UINT8) == (8 / 8));
    HALIDE_CHECK(user_context, sizeof(UINT16) == (16 / 8));
    HALIDE_CHECK(user_context, sizeof(UINT32) == (32 / 8));
    HALIDE_CHECK(user_context, sizeof(UINT64) == (64 / 8));
#ifdef BITS_64
    HALIDE_CHECK(user_context, sizeof(SIZE_T) == (64 / 8));
#else
    HALIDE_CHECK(user_context, sizeof(SIZE_T) == (32 / 8));
#endif
}
...
static d3d12_device *D3D12CreateSystemDefaultDevice(void *user_context) {
    ...
    HALIDE_CHECK(user_context, (dxgiAdapter == nullptr));
    size_t vram_max = 0;
    for (int i = 0;; ++i) {
...
ID3D12RootSignature *D3D12CreateMasterRootSignature(ID3D12Device *device) {
    ...
    if (D3DErrorCheck(result, pSignBlob, nullptr, "Unable to serialize the Direct3D 12 root signature")) {
        HALIDE_CHECK(user_context, pSignError);
...
WEAK void *map_buffer(d3d12_buffer *buffer) {
    ...
    HALIDE_CHECK(user_context, pData);
    buffer->mapped = pData;
...
WEAK size_t suballocate(d3d12_device *device, d3d12_buffer *staging, size_t num_bytes) {
    ...
    if (staging->sizeInBytes < num_bytes) {
        ...
        HALIDE_CHECK(user_context, (use_count == 1));
        ...
        HALIDE_CHECK(user_context, (use_count == 0));
        release_d3d12_object(staging);
        // and allocate a new one
        switch (staging->type) {
        case d3d12_buffer::Upload:
            HALIDE_CHECK(user_context, (staging == &upload));
            *staging = new_upload_buffer(device, new_capacity);
            break;
        case d3d12_buffer::ReadBack:
            HALIDE_CHECK(user_context, (staging == &readback));
    ...
    HALIDE_CHECK(user_context, (staging->sizeInBytes >= num_bytes));
...
static d3d12_command_allocator *new_command_allocator(d3d12_device *device) {
    TRACELOG;
    HALIDE_CHECK(user_context, device);
...
static d3d12_function *new_function_with_name(d3d12_device *device, d3d12_library *library, const char *name, size_t name_len,
    ...
    HALIDE_CHECK(user_context, (key.size() < key.capacity() - 1));  // make sure key fits into the stream
...
WEAK void set_input_buffer(d3d12_binder *binder, d3d12_buffer *input_buffer, uint32_t index) {
    ...
    case d3d12_buffer::Constant: {
        ...
        HALIDE_CHECK(user_context, input_buffer->format == DXGI_FORMAT_UNKNOWN);
        ...
        HALIDE_CHECK(user_context, (index < ResourceBindingLimits[CBV]));
    ...
    case d3d12_buffer::WriteOnly: {
        ...
        HALIDE_CHECK(user_context, (index < ResourceBindingLimits[UAV]));
...
static void buffer_copy_command(d3d12_copy_command_list *cmdList,
    ...
    D3D12_RESOURCE_BARRIER src_barrier = {};
    {
        ...
        if (src->state == D3D12_RESOURCE_STATE_GENERIC_READ) {
            HALIDE_CHECK(user_context, src->type == d3d12_buffer::Upload);
            src_barrier.Transition.StateAfter = D3D12_RESOURCE_STATE_GENERIC_READ;
        } else {
            HALIDE_CHECK(user_context, src->state == D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        }
    }
    ...
    D3D12_RESOURCE_BARRIER dst_barrier = {};
    {
        ...
        if (dst->state == D3D12_RESOURCE_STATE_COPY_DEST) {
            HALIDE_CHECK(user_context, dst->type == d3d12_buffer::ReadBack);
        } else {
            HALIDE_CHECK(user_context, dst->state == D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        }
    }
...
static void synchronize_host_and_device_buffer_contents(d3d12_copy_command_list *cmdList, d3d12_buffer *buffer) {
    ...
    HALIDE_CHECK(user_context, (xfer != nullptr));
...
static int d3d12compute_buffer_copy(d3d12_device *device,
    ...
    HALIDE_CHECK(user_context, device);
    HALIDE_CHECK(user_context, src);
    HALIDE_CHECK(user_context, dst);
    HALIDE_CHECK(user_context, (src->type != d3d12_buffer::Unknown));
    HALIDE_CHECK(user_context, (dst->type != d3d12_buffer::Unknown));
    ...
    HALIDE_CHECK(user_context, (src->type != d3d12_buffer::Constant));
    HALIDE_CHECK(user_context, (dst->type != d3d12_buffer::Constant));

    HALIDE_CHECK(user_context, num_bytes > 0);

    if (src->type == d3d12_buffer::Upload) {
        // host-to-device via staging buffer:
        HALIDE_CHECK(user_context, (dst->type != d3d12_buffer::Upload));
        HALIDE_CHECK(user_context, (dst->type != d3d12_buffer::ReadBack));
        ...
        xfer.size = num_bytes;
        HALIDE_CHECK(user_context, (dst->xfer == nullptr));
        dst->xfer = &xfer;
    ...
    if (dst->type == d3d12_buffer::ReadBack) {
        ...
        HALIDE_CHECK(user_context, (src->type != d3d12_buffer::Upload));
        HALIDE_CHECK(user_context, (src->type != d3d12_buffer::ReadBack));
        // TODO: assert that offsets and sizes are within bounds
        d3d12_buffer::transfer_t xfer = {};
        xfer.staging = dst;
        xfer.offset = dst_byte_offset;
        xfer.size = num_bytes;
        HALIDE_CHECK(user_context, (src->xfer == nullptr));
        src->xfer = &xfer;
...
static int d3d12_create_context(void *user_context) {
    ...
    HALIDE_CHECK(user_context, (device == nullptr));
    ...
    if (status == halide_error_code_success) {
        HALIDE_CHECK(user_context, (rootSignature == nullptr));
    ...
    if (status == halide_error_code_success) {
        HALIDE_CHECK(user_context, (queue == nullptr));
    ...
    if (status == halide_error_code_success) {
        HALIDE_CHECK(user_context, (cmd_allocator_main == nullptr));
    ...
    if (status == halide_error_code_success) {
        ...
        HALIDE_CHECK(user_context, (upload == 0));
        HALIDE_CHECK(user_context, (readback == 0));
...
WEAK int halide_d3d12compute_acquire_context(void *user_context, halide_d3d12compute_device **device_ret,
    ...
    HALIDE_CHECK(user_context, &thread_lock != nullptr);
    ...
    HALIDE_CHECK(user_context, (device == nullptr) || (queue != nullptr));
...
int do_multidimensional_copy(d3d12_device *device, const device_copy &c,
    ...
    if (dimensions == 0) {
        ...
        HALIDE_CHECK(user_context, (dsrc->halide_type == ddst->halide_type));
...
WEAK int halide_d3d12compute_copy_to_device(void *user_context, halide_buffer_t *buffer) {
    ...
    HALIDE_CHECK(user_context, buffer);
    HALIDE_CHECK(user_context, buffer->host && buffer->device);
    ...
    HALIDE_CHECK(user_context, (c.dst == buffer->device));
    d3d12_buffer *dev_buffer = peel_buffer(buffer);
    HALIDE_CHECK(user_context, buffer->size_in_bytes() == dev_buffer->sizeInBytes);
...
WEAK int halide_d3d12compute_run(void *user_context,
    ...
    HALIDE_CHECK(user_context, found && library != nullptr);
    ...
    d3d12_compute_pipeline_state *pipeline_state = nullptr;
    {
        ...
        HALIDE_CHECK(user_context, function);
        ...
        size_t total_uniform_args_size = 0;
        {
            ...
            for (size_t i = 0; i < num_kernel_args; i++) {
                if (arg_is_buffer[i]) {
                    ...
                    HALIDE_CHECK(user_context, (arg_sizes[i] & (arg_sizes[i] - 1)) == 0);
                    ...
                    HALIDE_CHECK(user_context, arg_type.lanes == 1);
                    HALIDE_CHECK(user_context, arg_sizes[i] > 0);
                    HALIDE_CHECK(user_context, arg_sizes[i] <= 4);
        ...
        if (has_uniform_arguments) {
            ...
            for (size_t i = 0; i < num_kernel_args; i++) {
                ...
                if (arg_type.code == halide_type_float) {
                    HALIDE_CHECK(user_context, (arg_type.bits == 32));
            ...
            HALIDE_CHECK(user_context, offset == total_uniform_args_size);
        }
...
WEAK int halide_d3d12compute_buffer_copy(void *user_context, struct halide_buffer_t *src,
    ...
    HALIDE_CHECK(user_context, (src->dimensions == dst->dimensions));
    ...
    if ((src->device_dirty() || src->host == nullptr) &&
        ...
        HALIDE_CHECK(user_context, dst_device_interface == &d3d12compute_device_interface);
    ...
    int err = 0;
    {
        ...
        if (!from_host && !to_host) {
            ...
            if (from_host) {
                ...
                HALIDE_CHECK(user_context, !to_host);
                HALIDE_CHECK(user_context, (dst->device_interface == &d3d12compute_device_interface));
                HALIDE_CHECK(user_context, (src->device_interface != &d3d12compute_device_interface));
                HALIDE_CHECK(user_context, (src->host != nullptr));
                ...
                HALIDE_CHECK(user_context, (dst->device == c.dst));
                if (dst->host != nullptr) {
                ...
                HALIDE_CHECK(user_context, to_host);
                HALIDE_CHECK(user_context, (src->device_interface == &d3d12compute_device_interface));
                HALIDE_CHECK(user_context, (dst->device_interface == nullptr));
                HALIDE_CHECK(user_context, (dst->host != nullptr));
                ...
                HALIDE_CHECK(user_context, (src->device == c.src));
                if (src->host != nullptr) {
...
WEAK int d3d12compute_device_crop_from_offset(void *user_context,
    ...
    HALIDE_CHECK(user_context, (new_handle != nullptr));
    HALIDE_CHECK(user_context, (new_handle->halide_type == dst->type));
    HALIDE_CHECK(user_context, (src->device_interface == dst->device_interface));
...
WEAK int halide_d3d12compute_device_crop(void *user_context,
    ...
    HALIDE_CHECK(user_context, (offset % src->type.bytes()) == 0);
...
WEAK int halide_d3d12compute_wrap_buffer(void *user_context, struct halide_buffer_t *halide_buf, uint64_t d3d12_resource) {
    ...
    HALIDE_CHECK(user_context, (pResource != nullptr));
