...
static d3d12_function* new_function_with_name(d3d12_device* device, d3d12_library* library, const char* name, size_t name_len)
{
    ...
    if (D3DError(result, (void*)0xFFFFFFFF, NULL, "Unable to serialize the Direct3D 12 root signature"))
    {
...
static void* buffer_contents(d3d12_buffer* buffer)
{
    ...
    if (D3DError(result, pData, NULL, "Unable to map Direct3D 12 buffer memory"))
        return(NULL);
    buffer->mapped = pData;
    return(pData);
