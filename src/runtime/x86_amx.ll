...
define weak_odr <256 x float> @tdpbf16ps(i16 %rows, i16 %colbytes, i16 %acc, <256 x float> %out, <512 x bfloat> %lhs, <512 x bfloat> %rhs) nounwind alwaysinline readnone {
  %1 = bitcast <512 x bfloat> %lhs to x86_amx
  %2 = bitcast <512 x bfloat> %rhs to x86_amx
