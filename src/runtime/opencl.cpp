...
WEAK void load_libopencl(void *user_context) {
    ...
    HALIDE_CHECK(user_context, clCreateContext == nullptr);
...
WEAK int halide_acquire_cl_context(void *user_context, cl_context *ctx, cl_command_queue *q, bool create = true) {
    ...
    HALIDE_CHECK(user_context, ctx != nullptr);
    HALIDE_CHECK(user_context, q != nullptr);
    ...
    HALIDE_CHECK(user_context, &context != nullptr);
    HALIDE_CHECK(user_context, &command_queue != nullptr);
    if (!context && create) {
...
WEAK bool validate_device_pointer(void *user_context, halide_buffer_t *buf, size_t size = 0) {
    ...
    if (size) {
        HALIDE_CHECK(user_context, real_size >= (size + offset) && "Validating pointer with insufficient size");
    }
    return true;
}
...
WEAK int create_opencl_context(void *user_context, cl_context *ctx, cl_command_queue *q) {
    ...
    HALIDE_CHECK(user_context, ctx != nullptr && *ctx == nullptr);
    HALIDE_CHECK(user_context, q != nullptr && *q == nullptr);
...
WEAK int halide_opencl_device_free(void *user_context, halide_buffer_t *buf) {
    ...
    cl_mem dev_ptr = ((device_handle *)buf->device)->mem;
    HALIDE_CHECK(user_context, (((device_handle *)buf->device)->offset == 0) && "halide_opencl_device_free on buffer obtained from halide_device_crop");
    ...
    HALIDE_CHECK(user_context, validate_device_pointer(user_context, buf));
...
WEAK int halide_opencl_initialize_kernels(void *user_context, void **state_ptr, const char *src, int size) {
    ...
    HALIDE_CHECK(user_context, program != nullptr);
...
WEAK int halide_opencl_device_release(void *user_context) {
    ...
    if (ctx) {
        err = clFinish(q);
        HALIDE_CHECK(user_context, err == CL_SUCCESS);
...
WEAK int halide_opencl_device_malloc(void *user_context, halide_buffer_t *buf) {
    ...
    if (buf->device) {
        HALIDE_CHECK(user_context, validate_device_pointer(user_context, buf, size));
        return 0;
    }
...
WEAK int halide_opencl_buffer_copy(void *user_context, struct halide_buffer_t *src,
    ...
    if ((src->device_dirty() || src->host == nullptr) &&
        ...
        HALIDE_CHECK(user_context, dst_device_interface == &opencl_device_interface);
    ...
    int err = 0;
    {
        ...
        uint64_t t_before = halide_current_time_ns(user_context);
        if (!from_host) {
            HALIDE_CHECK(user_context, validate_device_pointer(user_context, src));
        }
        if (!to_host) {
            HALIDE_CHECK(user_context, validate_device_pointer(user_context, dst));
        }
#endif
...
WEAK int halide_opencl_run(void *user_context,
    ...
    HALIDE_CHECK(user_context, state_ptr);

    cl_program program{};
    bool found = compilation_cache.lookup(ctx.context, state_ptr, program);
    HALIDE_CHECK(user_context, found && program != nullptr);
    ...
    while (arg_sizes[i] != 0) {
        ...
        if (arg_is_buffer[i]) {
            HALIDE_CHECK(user_context, arg_sizes[i] == sizeof(uint64_t));
...
WEAK int halide_opencl_image_buffer_copy(void *user_context, struct halide_buffer_t *src,
    ...
    if ((src->device_dirty() || src->host == nullptr) &&
        ...
        HALIDE_CHECK(user_context, dst_device_interface == &opencl_image_device_interface);
        // This is handled at the higher level.
        return halide_error_code_incompatible_device_interface;
    }
