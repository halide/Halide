...
WEAK const char *parse_openglcompute_version(const char *str, int *major, int *minor) {
...
#undef GLFUNC
    ...
    int major, minor;
    if (gles_version && parse_openglcompute_version(gles_version, &major, &minor)) {
        global_state.profile = OpenGLES;
        global_state.major_version = major;
        global_state.minor_version = minor;
    } else if (parse_openglcompute_version(version, &major, &minor)) {
        global_state.profile = OpenGL;
        global_state.major_version = major;
        global_state.minor_version = minor;
    } else {
    ...
    if (global_state.CheckAndReportError(user_context, "halide_openglcompute_init GenFramebuffers")) {
        return 1;
    }
    ...
    if (global_state.have_vertex_array_objects) {
        ...
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_init GenVertexArrays")) {
            return 1;
        }
    }
...
WEAK int halide_openglcompute_device_release(void *user_context) {
    ...
    debug(user_context) << "halide_openglcompute_release\n";
    ...
    while (tex) {
        ...
        if (tex->halide_allocated) {
            debug(user_context) << "halide_openglcompute_device_release: Deleting texture " << tex->id << "\n";
            global_state.DeleteTextures(1, &tex->id);
            if (global_state.CheckAndReportError(user_context, "halide_openglcompute_device_release DeleteTextures")) {
                return 1;
            }
            freed_textures++;
        }
    ...
    if (freed_textures > 0) {
        debug(user_context) << "halide_openglcompute_release: deleted "
...
WEAK int halide_openglcompute_device_malloc(void *user_context, buffer_t *buf) {
    ...
    if (buf->dev) {
        ...
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_device_malloc binding texture (GLES3)")) {
            return 1;
        }
        ...
        tex = (handle == HALIDE_OPENGLCOMPUTE_RENDER_TARGET) ? 0 : (GLuint)handle;
    } else {
        if (buf->extent[3] > 1) {
        ...
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_device_malloc GenTextures")) {
        ...
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_device_malloc binding texture")) {
            global_state.DeleteTextures(1, &tex);
            return 1;
        }
        ...
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_device_malloc TexImage2D")) {
            global_state.DeleteTextures(1, &tex);
            return 1;
        }

        buf->dev = halide_new_device_wrapper(tex, &openglcompute_device_interface);
        if (buf->dev == 0) {
    ...
    // halide_openglcompute_run we are only given the texture ID and not the full
...
WEAK int halide_openglcompute_device_free(void *user_context, buffer_t *buf) {
    ...
    uint64_t handle = halide_get_device_handle(buf->dev);
    GLuint tex = (handle == HALIDE_OPENGLCOMPUTE_RENDER_TARGET) ? 0 : (GLuint)handle;
    ...
    if (texinfo->halide_allocated) {
        debug(user_context) << "halide_openglcompute_device_free: Deleting texture " << tex << "\n";
        global_state.DeleteTextures(1, &tex);
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_device_free DeleteTextures")) {
            result = 1;
...
WEAK int halide_openglcompute_copy_to_device(void *user_context, buffer_t *buf) {
    ...
    int err = halide_openglcompute_device_malloc(user_context, buf);
    if (err) {
        return err;
    }
    ...
    uint64_t handle = halide_get_device_handle(buf->dev);
    if (handle == HALIDE_OPENGLCOMPUTE_RENDER_TARGET) {
        // TODO: this isn't correct; we want to ensure we copy to the current render_target.
        debug(user_context) << "halide_openglcompute_copy_to_device: called for HALIDE_OPENGLCOMPUTE_RENDER_TARGET\n";
        return 0;
    }
    GLuint tex = (GLuint)handle;
    debug(user_context) << "halide_openglcompute_copy_to_device: " << tex << "\n";

    global_state.BindTexture(GL_TEXTURE_2D, tex);
    if (global_state.CheckAndReportError(user_context, "halide_openglcompute_copy_to_device BindTexture")) {
        return 1;
    }
    ...
    if (is_interleaved && is_packed) {
        ...
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_copy_to_device TexSubImage2D(1)")) {
            return 1;
        }
    } else {
        debug(user_context)
        ...
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_copy_to_device TexSubImage2D(2)")) {
            return 1;
        }
    }
...
WEAK int halide_openglcompute_copy_to_host(void *user_context, buffer_t *buf) {
    ...
    if (handle != HALIDE_OPENGLCOMPUTE_RENDER_TARGET) {
        ...
        debug(user_context) << "halide_copy_to_host: HALIDE_OPENGLCOMPUTE_RENDER_TARGET\n";
    }
...
WEAK int halide_openglcompute_run(void *user_context,
                           void *state_ptr,
    ...
    for (int i = 0; args[i]; i++, kernel_arg = kernel_arg->next) {
        ...
        if (!kernel_arg) {
            error(user_context)
                << "Too many arguments passed to halide_openglcompute_run\n"
    ...
    for (int i = 0; args[i]; i++, kernel_arg = kernel_arg->next) {
        ...
        if (kernel_arg->kind == Argument::Outbuf) {
            ...
            if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run GetUniformLocation(InBuf)")) {
                return 1;
            }
            if (loc == -1) {
            ...
            global_state.BindTexture(GL_TEXTURE_2D, handle == HALIDE_OPENGLCOMPUTE_RENDER_TARGET ? 0 : (GLuint)handle);
    ...
    if (kernel_arg) {
        error(user_context) << "Too few arguments passed to halide_openglcompute_run";
        return 1;
    }
    ...
    for (int idx = 0; idx != num_padded_uniform_ints; idx += 4) {
        ...
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run GetUniformLocation")) {
            return 1;
        }
        if (loc == -1) {
    ...
    for (int idx = 0; idx != num_padded_uniform_floats; idx += 4) {
        ...
        GLint loc = global_state.GetUniformLocation(kernel->program_id, name.str());
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run GetUniformLocation(2)")) {
            return 1;
        }
        if (loc == -1) {
            // Argument was probably optimized away by GLSL compiler.
    ...
    for (int i = 0; args[i]; i++, kernel_arg = kernel_arg->next) {
        ...
        if (bind_render_targets) {
            ...
            global_state.FramebufferTexture2D(GL_FRAMEBUFFER,
                                    ...
                                    GL_TEXTURE_2D, tex, 0);
            if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run FramebufferTexture2D")) {
                return 1;
            }
        }
    ...
    if (num_output_textures == 0) {
        error(user_context) << "halide_openglcompute_run: kernel has no output\n";
        // TODO: cleanup
        return 1;
        ...
        if (global_state.DrawBuffers) {
            ...
            if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run DrawBuffers")) {
                return 1;
            }
        } else {
            error(user_context) << "halide_openglcompute_run: kernel has more than one output and DrawBuffers is not available (earlier than GL ES 3.0?).\n";
            // TODO: cleanup
            return 1;
        }
    }
    ...
    if (bind_render_targets) {
        // Check that framebuffer is set up correctly
        GLenum status = global_state.CheckFramebufferStatus(GL_FRAMEBUFFER);
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run CheckFramebufferStatus")) {
            return 1;
        }
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            error(user_context)
    ...
    if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run Uniform2iv(output_extent)")) {
        return 1;
    }
    loc = global_state.GetUniformLocation(kernel->program_id, "output_min");
    global_state.Uniform2iv(loc, 1, output_min);
    if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run Uniform2iv(output_min)")) {
        return 1;
    }
    ...
    if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run vertex BufferData et al")) {
        return 1;
    }
    ...
    if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run element BufferData et al")) {
        return 1;
    }
    ...
    for (int i=0;i!=num_packed_attributes;i++) {
        ...
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run VertexAttribPointer et al")) {
            return 1;
        }

        global_state.EnableVertexAttribArray(attrib_id);
        if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run EnableVertexAttribArray et al")) {
            return 1;
        }
    }
    ...
    if (global_state.CheckAndReportError(user_context, "halide_openglcompute_run DrawElements et al")) {
        return 1;
    }
...
WEAK const halide_device_interface *halide_openglcompute_device_interface() {
    return &openglcompute_device_interface;
}

WEAK void halide_openglcompute_context_lost(void *user_context) {
    if (!global_state.initialized) return;

    debug(user_context) << "halide_openglcompute_context_lost\n";
...
WEAK int halide_openglcompute_wrap_texture(void *user_context, struct buffer_t *buf, uintptr_t texture_id) {
    if (texture_id == 0) {
    ...
    buf->dev = halide_new_device_wrapper(texture_id, &openglcompute_device_interface);
    if (buf->dev == 0) {
        return -1;
    }
    return 0;
...
WEAK int halide_openglcompute_wrap_render_target(void *user_context, struct buffer_t *buf) {
    if (!global_state.initialized) {
    ...
    buf->dev = halide_new_device_wrapper(HALIDE_OPENGLCOMPUTE_RENDER_TARGET, &openglcompute_device_interface);
    if (buf->dev == 0) {
        return -1;
    }
    return 0;
...
WEAK uintptr_t halide_openglcompute_detach_texture(void *user_context, struct buffer_t *buf) {
    if (buf->dev == NULL) {
        return 0;
    }
    halide_assert(user_context, halide_get_device_interface(buf->dev) == &openglcompute_device_interface);
    uint64_t handle = halide_get_device_handle(buf->dev);
    halide_delete_device_wrapper(buf->dev);
    buf->dev = 0;
    if (handle == HALIDE_OPENGLCOMPUTE_RENDER_TARGET) {
...
WEAK uintptr_t halide_openglcompute_get_texture(void *user_context, struct buffer_t *buf) {
    if (buf->dev == NULL) {
        return 0;
    }
    halide_assert(user_context, halide_get_device_interface(buf->dev) == &openglcompute_device_interface);
    uint64_t handle = halide_get_device_handle(buf->dev);
    // client_bound always return 0 here.
    return handle == HALIDE_OPENGLCOMPUTE_RENDER_TARGET ? 0 : (uintptr_t)handle;
}

namespace {
__attribute__((destructor))
WEAK void halide_openglcompute_cleanup() {
    halide_openglcompute_device_release(NULL);
}
}
