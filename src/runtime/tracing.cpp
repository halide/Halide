...
class TraceBuffer {
    ...
    ALWAYS_INLINE halide_trace_packet_t *try_acquire_packet(void *user_context, uint32_t size) {
        lock.acquire_shared();
        HALIDE_CHECK(user_context, size <= buffer_size);
...
public:
    ...
    ALWAYS_INLINE void flush(void *user_context, int fd) {
        ...
        HALIDE_CHECK(user_context, success && "Could not write to trace file");
    }
...
WEAK int32_t halide_default_trace(void *user_context, const halide_trace_event_t *e) {
    ...
    if (fd > 0) {
        ...
        HALIDE_CHECK(user_context, print_bits <= 64 && "Tracing bad type");
        ...
        if (print_value) {
            ...
            for (int i = 0; i < e->type.lanes; i++) {
                ...
                if (e->type.code == 0) {
                    ...
                    HALIDE_CHECK(user_context, print_bits >= 16 && "Tracing a bad type");
                    if (print_bits == 32) {
                        ss << ((float *)(e->value))[i];
                    } else if (print_bits == 16) {
