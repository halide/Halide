...
public:
    ...
    static void assert_can_convert_from(const Buffer<T2, D2> &other) {
        ...
        if (Buffer<T2, D2>::T_is_void && !T_is_void) {
            assert(other.type() == static_halide_type());
        }
    }
    ...
    void check_overflow() {
        ...
        assert(size == type().bytes() && "Error: Overflow computing total size of buffer.");
    }
    ...
    /** Provide a cast operator to halide_buffer_t *, so that instances can
...
struct for_each_element_helpers {
    ...
    static void for_each_element_variadic(double, int d, Fn &&f, const halide_buffer_t &buf, Args... args) {
        int e = buf.dim[d].extent == 0 ? 1 : buf.dim[d].extent;
        for (int i = 0; i < e; i++) {
            for_each_element_variadic(0, d-1, std::forward<Fn>(f), buf, buf.dim[d].min + i, args...);
        }
    }
    ...
    for_each_element_array_helper(int, Fn &&f, const halide_buffer_t &buf, int *pos) {
        for (pos[d] = buf.dim[d].min; pos[d] < buf.dim[d].min + buf.dim[d].extent; pos[d]++) {
    ...
    template<int d>
    ALWAYS_INLINE
    static void for_each_element_array_helper(double, Fn &&f, const halide_buffer_t &buf, int *pos) {
        f(pos);
    }
