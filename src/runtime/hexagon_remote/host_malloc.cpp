...
extern "C" {
void *halide_hexagon_host_malloc(size_t);
void halide_hexagon_host_free(void *);
...
void halide_hexagon_host_malloc_init() {
    __android_log_print(ANDROID_LOG_ERROR, "halide", "entering halide_hexagon_host_malloc_init");
    if (ion_fd != -1) return;
    ...
    // This call to free established use_newer_ioctl.
    void *v = halide_hexagon_host_malloc(4);
    if (v) {
        halide_hexagon_host_free(v);
    }
    __android_log_print(ANDROID_LOG_ERROR, "halide", use_newer_ioctl ?
                            "using newer ion ioctl" : "Using old ion ioctl");
}

void halide_hexagon_host_malloc_deinit() {
    __android_log_print(ANDROID_LOG_ERROR, "halide", "entering halide_hexagon_host_malloc_deinit");
    if (ion_fd == -1) return;
    close(ion_fd);
    ion_fd = -1;
