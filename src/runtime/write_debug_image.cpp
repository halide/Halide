...
WEAK uint8_t *get_pointer_to_data(int32_t dim0, int32_t dim1, int32_t dim2, int32_t dim3,
    ...
    uint8_t *ptr = buf->host + buf->elem_size * (dim0 * buf->stride[0] + dim1 * buf->stride[1] + dim2 * buf->stride[2] + dim3 * buf->stride[3]);
    return ptr;
}
}
}
...
WEAK extern "C" int32_t halide_debug_to_file(void *user_context, const char *filename,
    ...
    if (has_tiff_extension(filename)) {
        ...
        tag++->assign32(279, channels,
                        (channels == 1) ? elts * bytes_per_element : sizeof(header) + channels * sizeof(int32_t));  // strip byte counts, bug if 32-bit truncation
