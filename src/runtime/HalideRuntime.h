...
struct HALIDE_ATTRIBUTE_ALIGN(8) halide_profiler_func_stats {
     ...
     * order of the counter enum in src/Profiling.cpp (bit 0 = memory_peak,
     * 1 = stack_peak, 2 = memory_total, 3 = num_allocs, ... through
     * productions_if_inwards). active_threads_numerator and _denominator are
     * sampled at runtime rather than summed over loops, so they are never
     * approximated and have no bit. A set bit only happens on GPU, where a
     * guarded contribution can't be summed exactly and is bounded instead;
     * the reporter marks such columns with a leading '<'. */
     ...
     * peak, not a sum -- do not divide by runs). Max-aggregated by
     * halide_profiler_update_counters. */
    uint64_t memory_peak;

    /** The peak stack allocation of this Func's threads, a maximum over all
     * runs like memory_peak. Max-aggregated by
     * halide_profiler_update_counters. */
    uint64_t stack_peak;
 ...
 * counters are summed over `runs` (divide by `runs` for per-run), and the
 * active_threads pair is a ready-made average. */
