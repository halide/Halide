...
//Internal Function
static inline int halide_hexagon_dma_memory_free(void* user_context, void* vret) {
	if (vret == NULL) {
		return HEX_ERROR;
	}

	t_mem_desc* cache_mem = (t_mem_desc *)vret;
...
void* halide_hexagon_dma_memory_alloc(void* user_context, dma_context pdma_context, halide_hexagon_dma_user_component_t comp,
    ...
    t_mem_desc* cache_mem = (t_mem_desc *) malloc (sizeof(t_mem_desc));
    ...
    if(nRet != 0) {
        ...
        nRet = halide_hexagon_dma_memory_free(user_context, cache_mem);     // free l2$ above
        if(nRet != 0) {
...
int  halide_hexagon_dma_wrap_memory_free(void* user_context, dma_context pdma_context, void* vret) {
	  if (vret == NULL) {
	        return HEX_ERROR;
	    }
	    ...
	    int nRet = halide_hexagon_dma_memory_free(user_context, cache_mem);
	    if (nRet != 0) {
	        error(user_context) << "Failed to free host memory. The error code is: " << nRet <<"\n";
	        return HEX_ERROR;
	    }
...
void *halide_hexagon_dma_memory_get(void *user_context, halide_buffer_t* roi_buf) {
