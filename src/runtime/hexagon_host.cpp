...
WEAK int halide_hexagon_initialize_kernels(void *user_context, void **state_ptr,
    ...
    HALIDE_CHECK(user_context, state_ptr != nullptr);
    ...
    if (!shared_runtime) {
        ...
        if (result == 0) {
            ...
            HALIDE_CHECK(user_context, shared_runtime != 0);
        } else {
...
WEAK int halide_hexagon_run(void *user_context,
                            ...
                            int arg_flags[]) {
    HALIDE_CHECK(user_context, state_ptr != nullptr);
    HALIDE_CHECK(user_context, function != nullptr);
    int result = init_hexagon_runtime(user_context);
    if (result != 0) {
        return result;
    }
...
WEAK int halide_hexagon_copy_to_device(void *user_context, halide_buffer_t *buf) {
    ...
    HALIDE_CHECK(user_context, buf->host && buf->device);
...
WEAK int halide_hexagon_detach_device_handle(void *user_context, struct halide_buffer_t *buf) {
    ...
    HALIDE_CHECK(user_context, buf->device_interface == &hexagon_device_interface);
...
WEAK int halide_hexagon_buffer_copy(void *user_context, struct halide_buffer_t *src,
    ...
    if ((src->device_dirty() || src->host == nullptr) &&
        ...
        HALIDE_CHECK(user_context, dst_device_interface == &hexagon_device_interface);
    ...
    HALIDE_CHECK(user_context, from_host || src->device);
    HALIDE_CHECK(user_context, to_host || dst->device);
