...
ALWAYS_INLINE ion_device_handle *uint64_to_ion_device_handle(const uint64_t &dev) {
    static_assert(sizeof(ion_device_handle *) <= sizeof(uint64_t), "");
    ion_device_handle *h = nullptr;
    memcpy(&h, &dev, sizeof(dev));
    return h;
}

ALWAYS_INLINE uint64_t ion_device_handle_to_uint64(ion_device_handle *h) {
    static_assert(sizeof(ion_device_handle *) <= sizeof(uint64_t), "");
    uint64_t dev = 0;
    memcpy(&dev, &h, sizeof(h));
    return dev;
}
    ...
    for (int i = 0; i < arg_count; i++) {
        ...
        if (arg_flags[i] != 0) {
            ...
            if (device) {
                ...
                ion_device_handle *ion_handle = uint64_to_ion_device_handle(device);
...
WEAK int halide_hexagon_copy_to_device(void *user_context, halide_buffer_t *buf) {
    ...
    c.dst = ion_device_handle_to_uint64((ion_device_handle *)halide_hexagon_get_device_handle(user_context, buf));
...
WEAK int halide_hexagon_copy_to_host(void *user_context, struct halide_buffer_t *buf) {
    ...
    // Get the descriptor associated with the ion buffer.
    c.src = ion_device_handle_to_uint64((ion_device_handle *)halide_hexagon_get_device_handle(user_context, buf));
    copy_memory(c, user_context);
...
WEAK int halide_hexagon_wrap_device_handle(void *user_context, struct halide_buffer_t *buf,
    ...
    buf->device = ion_device_handle_to_uint64(handle);
    return 0;
}
...
WEAK int halide_hexagon_detach_device_handle(void *user_context, struct halide_buffer_t *buf) {
    ...
    ion_device_handle *handle = uint64_to_ion_device_handle(buf->device);
    free(handle);
...
WEAK int halide_hexagon_buffer_copy(void *user_context, struct halide_buffer_t *src,
    ...
    // Get the descriptor associated with the ion buffer.
    if (!from_host) {
        c.src = ion_device_handle_to_uint64((ion_device_handle *)halide_hexagon_get_device_handle(user_context, src));
    }
    if (!to_host) {
        c.dst = ion_device_handle_to_uint64((ion_device_handle *)halide_hexagon_get_device_handle(user_context, dst));
    }
    copy_memory(c, user_context);
...
WEAK int hexagon_device_crop_from_offset(const struct halide_buffer_t *src, int64_t offset, struct halide_buffer_t *dst) {
    ...
    dst->device = ion_device_handle_to_uint64(dst_handle);
