...
MemoryRegion *VulkanMemoryAllocator::create_crop(void *user_context, MemoryRegion *region, const RegionIndexing &indexing) {
    ...
    const bool allocator_owned = owner->ownership == MemoryOwnership::Owned;
    if (allocator_owned) {
        RegionAllocator *region_allocator = RegionAllocator::find_allocator(user_context, owner);
        if (region_allocator == nullptr) {
    ...
    if (allocator_owned) {
        ...
        memory_region = reinterpret_cast<MemoryRegion *>(
            vk_host_malloc(user_context, sizeof(MemoryRegion), 0, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT, callbacks()));
    }
    ...
    memory_region->ownership = MemoryOwnership::CropAlias;
...
int VulkanMemoryAllocator::destroy_crop(void *user_context, MemoryRegion *region) {
    ...
    if (owner->ownership == MemoryOwnership::Owned) {
        RegionAllocator *region_allocator = RegionAllocator::find_allocator(user_context, owner);
        if (region_allocator == nullptr) {
    ...
    if (owner->ownership == MemoryOwnership::Owned) {
        ...
        vk_host_free(user_context, region, callbacks());
    }
    return halide_error_code_success;
}
...
MemoryRegion *VulkanMemoryAllocator::owner_of(void *user_context, MemoryRegion *region) {
    ...
    while (current != nullptr && current->ownership == MemoryOwnership::CropAlias) {
...
int vk_destroy_memory_allocator(void *user_context, VulkanMemoryAllocator *allocator) {
    if (allocator != nullptr) {
        allocator->collect(user_context);
