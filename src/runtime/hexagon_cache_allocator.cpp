...
static void *hexagon_cache_pool_get (void *user_context, size_t size, bool retry_effort) {
    ...
    if (mem == NULL) {
        ...
        free(temp);
        if (!retry_effort) {
            halide_print(NULL, "HAP_cache_lock failed try one more time \n");
            ...
            return hexagon_cache_pool_get(user_context, size, true);
        } else {
            halide_print(NULL, "HAP_cache_lock failed \n");
            return NULL;
        }
    }
...
static void hexagon_cache_pool_put(void *user_context, void *cache_mem) {
...
WEAK void *halide_hexagon_dma_allocate_from_l2_pool(void *user_context, size_t size) {
    halide_print(NULL, "halide_hexagon_dma_allocate_from_l2_pool\n");
    return hexagon_cache_pool_get(user_context, size, false);
}
