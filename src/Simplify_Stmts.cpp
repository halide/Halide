...
Stmt Simplify::visit(const IfThenElse *op) {
    ...
    if (then_acquire &&
        ...
        return then_pc->remake(mutate(IfThenElse::make(condition, then_pc->body, else_pc->body)));
    } else if (then_pc &&
               is_no_op(else_case)) {
        return then_pc->remake(mutate(IfThenElse::make(condition, then_pc->body)));
    } else if (then_block &&
               else_block &&
...
Stmt Simplify::visit(const For *op) {
    ...
    if (is_no_op(new_body)) {
        ...
        return op->remake(make_zero(Int(32)), shifted_max, new_body);
...
Stmt Simplify::visit(const Store *op) {
    ...
    if (is_const_zero(predicate)) {
        ...
        return IfThenElse::make(scalar_pred->value,
                                op->remake(value, index, const_true(value.type().lanes(), nullptr), align));
        ...
        for (const Expr &idx : shuf->vectors) {
            stores.push_back(op->remake(Shuffle::make_slice(var, lanes, 1, idx.type().lanes()), idx, Shuffle::make_slice(predicate, lanes, 1, idx.type().lanes()), ModulusRemainder{}));
        ...
        return mutate(op->remake(permuted_value, mr.to_expr(), permuted_predicate, align));
        ...
        return op->remake(value, index, predicate, align);
    }
}
