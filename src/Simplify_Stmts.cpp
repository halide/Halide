...
Stmt Simplify::visit(const For *op) {
    ...
    if (is_no_op(new_body)) {
        ...
        return For::make(op->name, make_zero(Int(32)), new_extent, op->for_type, op->device_api, new_body, op->allow_partitioning);
    } else if (op->min.same_as(new_min) &&
               op->extent.same_as(new_extent) &&
               op->body.same_as(new_body)) {
        return op;
    } else {
        return For::make(op->name, new_min, new_extent, op->for_type, op->device_api, new_body, op->allow_partitioning);
    }
}
