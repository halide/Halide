...

    {
        ...
        if (t && *t == MemoryType::GPUSharedAsync) {
            user_error
                ...
                << reason << ".\n\n"
                << "The requirement is simple: the Func must be a plain copy from a "
                << "buffer the kernel did not allocate - no cast, arithmetic, or "
                << "boundary condition, because the bytes move untouched - and each "
                << "thread must store a dense vector of 4, 8 or 16 bytes, aligned to "
                << "its own size in both the source and the destination. The target "
                << "must also be CUDA compute capability 8.0 or above.\n\n"
                << "So the only part of the schedule the copy itself cares about is "
                << "the vectorize. For 16-bit data:\n\n"
                << "    stage.compute_at(consumer, r)\n"
                << "         .store_in(MemoryType::GPUSharedAsync)\n"
                << "         .vectorize(x, 8);   // 8 halves is 16 bytes\n\n"
                << "Everything else is only there to spread the copies over the "
                << "threads of the block, which is ordinary GPU scheduling. If the "
                << "dimensions already have the right extents, hand them straight to "
                << "gpu_threads:\n\n"
                << "    stage.compute_at(consumer, r)\n"
                << "         .store_in(MemoryType::GPUSharedAsync)\n"
                ...
                << "         .vectorize(xi, 8)\n"
                << "         .gpu_threads(xi, yi);\n\n"
                << "If they don't - a panel 32 elements wide gives only 4 chunks in x, "
                << "not the 32 a warp wants - flatten them and split the flat index to "
                << "whatever thread counts the block has:\n\n"
                << "    stage.compute_at(consumer, r)\n"
                << "         .store_in(MemoryType::GPUSharedAsync)\n"
                << "         .align_storage(x, padded_width)  // a multiple of 8\n"
                << "         .split(x, xo, xi, 8)\n"
                << "         .fuse(xo, y, t)        // flatten, purely to reshape\n"
