...

    {
        ...
        if (t && *t == MemoryType::GPUSharedAsync) {
            user_error
                ...
                << "going through registers. For that to be possible the Func must "
                << "be a plain copy - a single load from a buffer the kernel did not "
                << "allocate, with no cast, arithmetic, or boundary condition - and "
                << "each thread must move 4, 8 or 16 bytes from consecutive "
                << "addresses at both ends. Any align_storage must be a multiple of "
                << "the number of elements a thread copies, and the target must be "
                << "CUDA compute capability 8.0 or above.\n\n"
                << "If the dense dimension is long enough to keep the block busy, no "
                << "fuse is needed. For 16-bit data and a block of 32 by 8 threads:\n\n"
                ...
                << "         .gpu_threads(xi, yi);\n\n"
                << "The tile has to divide into the block's thread counts: its width "
                << "over the vector width must be the number of threads in x, and its "
                << "height the number in y. Any remaining extent is covered by the "
                << "serial loops the tile leaves outside.\n\n"
                << "If the dense dimension is too short for that - a panel only 32 "
                << "elements wide cannot fill 32 threads at 8 elements each - split it "
                << "into chunks and fuse the chunk index with the other dimensions, "
                << "then spread that flat index over the threads:\n\n"
                << "    stage.compute_at(consumer, r)\n"
                << "         .store_in(MemoryType::GPUSharedAsync)\n"
                << "         .align_storage(x, padded_width)  // a multiple of 8\n"
                << "         .split(x, xo, xi, 8)   // xi is the dense chunk\n"
                << "         .fuse(xo, y, t)        // t enumerates chunks, not elements\n"
                << "         .split(t, t, ti, 32)   // one chunk per lane\n"
                << "         .split(t, t, tw, warps_per_block)\n"
                << "         .gpu_lanes(ti)\n"
                << "         .gpu_threads(tw)\n"
                << "         .vectorize(xi);\n\n"
                << "Note that it is the chunk index being fused, not the elements, so "
                << "each copy still lies within one row and the source does not have "
                << "to be contiguous across the fused dimensions.\n\n"
