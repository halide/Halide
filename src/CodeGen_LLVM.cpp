    ...
    if (intrin_lanes != arg_lanes) {
        ...
        for (int start = 0; start < arg_lanes; start += intrin_lanes) {
            ...
            for (auto &arg_value : arg_values) {
                if (arg_value->getType()->isVectorTy()) {
                    int arg_i_lanes = (int)arg_value->getType()->getVectorNumElements();
                    ...
                    args.push_back(slice_vector(arg_value, start * reduce, intrin_lanes * reduce));
                } else {
