...
CodeGen_LLVM::CodeGen_LLVM(Target t) :
    ...
    strict_float(t.has_feature(Target::ForceStrictFloat)) {
    initialize_llvm();
}
...
std::unique_ptr<llvm::Module> CodeGen_LLVM::compile(const Module &input) {
    ...
    module->addModuleFlag(llvm::Module::Warning, "halide_allow_strict_float",
                          input.target().has_feature(Target::AllowStrictFloat) ||
                          input.target().has_feature(Target::ForceStrictFloat));
