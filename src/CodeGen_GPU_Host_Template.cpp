...
bool CODEGEN_GPU_CLASS::lib_cuda_linked = false;

class CODEGEN_GPU_CLASS::Closure : public Halide::Internal::Closure {
public:
...
vector<Argument> CODEGEN_GPU_CLASS::Closure::arguments() {
    vector<Argument> res;
...
void CODEGEN_GPU_CLASS::Closure::visit(const For *loop) {
    if (skip_gpu_loops &&
...
CODEGEN_GPU_CLASS::CODEGEN_GPU_CLASS(Target target) :
    CODEGEN_GPU_PARENT(target),
    dev_malloc_fn(NULL),
    dev_free_fn(NULL),
    copy_to_dev_fn(NULL),
...
CodeGen_GPU_Dev* CODEGEN_GPU_CLASS::make_dev(Target t)
{
...
CODEGEN_GPU_CLASS::~CODEGEN_GPU_CLASS() {
    delete cgdev;
}

void CODEGEN_GPU_CLASS::compile(Stmt stmt, string name,
...
void CODEGEN_GPU_CLASS::jit_init(ExecutionEngine *ee, Module *module) {
...
void CODEGEN_GPU_CLASS::jit_finalize(ExecutionEngine *ee, Module *module, vector<void (*)()> *cleanup_routines) {
...
void CODEGEN_GPU_CLASS::visit(const For *loop) {
    if (CodeGen_GPU_Dev::is_gpu_var(loop->name)) {
        ...
        CODEGEN_GPU_PARENT::visit(loop);
    }
}

void CODEGEN_GPU_CLASS::visit(const Allocate *alloc) {
...
void CODEGEN_GPU_CLASS::visit(const Free *f) {
...
void CODEGEN_GPU_CLASS::visit(const Call *call) {
...
Value *CODEGEN_GPU_CLASS::get_module_state() {
