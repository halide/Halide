...
void WEAK __init(const char* src)
{
    int err;
    cl_device_id dev;
...
cl_kernel WEAK __get_kernel(const char* entry_name)
{
    cl_kernel f;
    #ifdef NDEBUG
    char msg[1];
...
cl_mem WEAK __dev_malloc(size_t bytes) {
    cl_mem p;
    #ifdef NDEBUG
    char msg[1];
    #else
...
int main(int argc, char* argv[]) {
    ...
    __dev_malloc_if_missing(&in);
    __dev_malloc_if_missing(&out);
    __copy_to_dev(&in);
