...
Stmt IRMutator::visit(const AssertStmt *op) {
    ...
    return op->remake(std::move(condition), std::move(message));
}
...
Stmt IRMutator::visit(const Store *op) {
    ...
    Expr value = mutate(op->value);
    Expr index = mutate(op->index);
    return op->remake(std::move(value), std::move(index), std::move(predicate), op->alignment);
}
...
Stmt IRMutator::visit(const Block *op) {
    ...
    return op->remake(std::move(first), std::move(rest));
}
...
Stmt IRMutator::visit(const IfThenElse *op) {
    ...
    return op->remake(std::move(condition), std::move(then_case), std::move(else_case));
}
...
Stmt IRMutator::visit(const Acquire *op) {
    ...
    Stmt body = mutate(op->body);
    return op->remake(std::move(sema), std::move(count), std::move(body));
}
