...
public:
     ...
     * auto-generated name */
    Param() : param(type_of<T>(), false, 0, Internal::make_entity_name(this, "Halide::Param<?", 'p')) {}

    /** Construct a scalar parameter of type T with the given name. */
    explicit Param(const std::string &n) : param(type_of<T>(), false, 0, n) {}
    ...
    Param(const std::string &n, T val) : param(type_of<T>(), false, 0, n) {
        set(val);
    }
