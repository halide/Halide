...
public:
     ...
     */
    virtual void record_matched_simplifier_rule(std::string_view rulename, Expr expr) = 0;

    /** Record when an expression is non-monotonic in a loop variable.
     */
    virtual void record_non_monotonic_loop_var(std::string_view loop_var, Expr expr) = 0;
...
public:
    ...
    void record_matched_simplifier_rule(std::string_view rulename, Expr expr) override;
    void record_non_monotonic_loop_var(std::string_view loop_var, Expr expr) override;
...
protected:
    ...
    StringMap<std::vector<Expr>> matched_simplifier_rules;

    // Maps loop_var -> list of Exprs that were nonmonotonic for that loop_var
    StringMap<std::vector<Expr>> non_monotonic_loop_vars;
