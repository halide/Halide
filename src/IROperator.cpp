...
Expr int64_to_immediate_expr(int64_t i) {
    int32_t lo32 = (int32_t) i;
    int32_t hi32 = (int32_t) (i >> 32);
    Expr e;
    if (hi32 == -1 && lo32 < 0) {
        // negative number that will fit in int32
        e = Cast::make(Int(64), Expr(lo32));
    } else {
        e = Cast::make(Int(64), Cast::make(UInt(32), Expr(lo32)));
        if (hi32 != 0) {
            e = (Cast::make(Int(64), Cast::make(Int(32), Expr(hi32))) << Expr(32)) | e;
        }
        // else positive number that will fit in int32
    }
    e = simplify(Cast::make(Int(64), e));
    return e;
}
...
bool int64_from_immediate_expr(Expr e, int64_t* value) {
    ...
    if (!e.defined() || (!e.type().is_int() && !e.type().is_uint())) {
        return false;
    }
    ...
    if (const Cast* c = e.as<Cast>()) {
        if (c->type == Int(64)) {
            return int64_from_immediate_expr(c->value, value);
        }
        return false;
    }
