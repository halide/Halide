    ...
    class VectorSubs : public IRMutator {
        ...
        void visit(const Select *op) {
            ...
            if (condition.same_as(op->condition) &&
                ...
                int width = std::max(true_value.type().lanes(), false_value.type().lanes());
                width = std::max(width, condition.type().lanes());
        ...
        void visit(const Load *op) {
            ...
            if (internal_allocations.contains(op->name)) {
                int width = replacement.type().lanes();
        ...
        void visit(const Call *op) {
            ...
            if (op->name == Call::shuffle_vector &&
                ...
                int replacement_width = replacement.type().lanes();
                ...
                if (shuffle_width == 1) {
                    ...
                    int mutated_width = mutated_channel.type().lanes();
                    ...
                    if (equal(replacement,mutated_channel) &&
                        (shuffled_expr.type().lanes() == replacement_width)) {
                        ...
                        expr = Call::make(op->type.with_lanes(replacement_width),
                ...
                for (size_t i = 0; i < op->args.size(); i++) {
                    ...
                    new_args[i] = new_arg;
                    max_width = std::max(new_arg.type().lanes(), max_width);
                }
                ...
                if (!changed) {
                    ...
                    expr = Call::make(op->type.with_lanes(max_width), op->name, new_args,
        ...
        void visit(const Provide *op) {
            ...
            for (size_t i = 0; i < op->args.size(); i++) {
                ...
                max_width = std::max(new_value.type().lanes(), max_width);
            }
        ...
        void visit(const Store *op) {
            ...
            if (value.same_as(op->value) && index.same_as(op->index)) {
                ...
                int width = std::max(value.type().lanes(), index.type().lanes());
