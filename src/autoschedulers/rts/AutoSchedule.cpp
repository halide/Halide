...
void generate_schedule(const std::vector<Function> &outputs,
    ...
    // TODO: make this only prkint if a verbose flag is set. (default prints for now).
    ...
    try {
        CPU_State optimal = solver.random_schedule(start_state, /* n_decisions*/ dag.nodes.size() * 2, seed);
        // (MCTS::use_beam()) ? 
    ...
    */
    ...
    if (auto_scheduler_results) {
        auto_scheduler_results->scheduler_name = "rts";
