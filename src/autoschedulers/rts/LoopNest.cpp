...
TailStrategy random_strategy(const std::vector<TailStrategy> &strategies) {
    // TODO: need to print rng seed value
...
void LoopNest::apply(LoopLevel here,
    ...
    if (is_root()) {
        ...
        if (stage->index > 0) {
            ...
            // recompute
            // Could use Predicate instead
            pure_var_tail_strategy = random_strategy(pred_strategies);
            ...
            // their loop nest.
            // Any are legal here
            pure_var_tail_strategy = random_strategy(all_strategies);
