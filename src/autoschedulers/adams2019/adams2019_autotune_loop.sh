...
TIMEOUT_CMD="timeout"
if [ "$(uname -s)" = "Darwin" ] && ! command -v "$TIMEOUT_CMD" >/dev/null 2>&1; then
...
FIRST=$(find "${SAMPLES}" -maxdepth 1 -type d -name 'batch_*' -print 2>/dev/null | sed -e "s|.*/batch_||;s|_.*||" | sort -n | tail -n1)
