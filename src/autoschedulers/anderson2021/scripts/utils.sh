...
function get_timeout_cmd() {
    ...
    if [ "$(uname -s)" = "Darwin" ] && ! command -v "$timeout_cmd_ref" >/dev/null 2>&1; then
        # OSX doesn't have timeout; gtimeout is equivalent and available via Homebrew
        timeout_cmd_ref="gtimeout"
        if ! command -v "$timeout_cmd_ref" >/dev/null 2>&1; then
...
function save_best_schedule_result() {
    ...
    local current_best_run_time
    current_best_run_time=$(tail -n 1 "$best_details_file" | cut -d" " -f 5)
...
function print_best_schedule_times() {
    ...
    for app in $apps; do
        ...
        local current_best_run_time
        current_best_run_time=$(tail -n 1 "$file" | cut -d" " -f 5)
