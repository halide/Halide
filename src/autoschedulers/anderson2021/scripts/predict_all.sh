...
awk -F", " '{printf("%f, %f\n", $2, $3);}' "${PREDICTIONS_FILE}" >"${PREDICTIONS_FILE}.tmp"
mv "${PREDICTIONS_FILE}.tmp" "${PREDICTIONS_FILE}"
awk -F", " '{printf("%f, %f\n", $2, $3);}' "${PREDICTIONS_FILE}_validation_set" >"${PREDICTIONS_FILE}_validation_set.tmp"
mv "${PREDICTIONS_FILE}_validation_set.tmp" "${PREDICTIONS_FILE}_validation_set"
