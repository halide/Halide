...
template<typename A>
std::ostream &operator<<(std::ostream &s, const RCastOp<A> &op) {
    s << "rcast(" << op.t << ", " << op.a << ")";
    return s;
}

template<typename A>
HALIDE_ALWAYS_INLINE auto rcast(A &&a) noexcept -> RCastOp<decltype(pattern_arg(a))> {
    assert_is_lvalue_if_expr<A>();
    return {pattern_arg(a)};
}
