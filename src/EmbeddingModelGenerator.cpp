...
// missing
extern "C" {
void *halide_autoscheduler_embedding_model = nullptr;
void *halide_autoscheduler_train_embedding_model = nullptr;
}
...
class EmbeddingModel : public Generator<EmbeddingModel<training>> {
public:
    ...
    void generate() {
        ...
        embedding(n, c) = sum(relu5_padded(n, c, r_reduce));

        if (!training) {
            output(n, c) = embedding(n, c);

            // schedule
            output.bound(n, 0, batch_size);
            output.bound(c, 0, conv5_channels);
            const int vec = 8;
            ...
            for (Func f : {normalized_schedule_features, head2_relu, relu1, relu2, relu3, pool3, relu4, pool4, relu5}) {
...
using EmbeddingModelInference = EmbeddingModel<false>;
using EmbeddingModelTraining = EmbeddingModel<true>;

HALIDE_REGISTER_GENERATOR(EmbeddingModelInference, halide_autoscheduler_embedding_model);
HALIDE_REGISTER_GENERATOR(EmbeddingModelTraining, halide_autoscheduler_train_embedding_model);
