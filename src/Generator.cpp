...
Outputs compute_outputs(const Target &target,
    ...
    if (options.emit_h) {
        output_files.c_header_name = base_path + ".h";
    }
    if (options.emit_cpp) {
        output_files.c_source_name = base_path + ".halide_generated.cpp";
    }
    if (options.emit_python_extension) {
        output_files.python_extension_name = base_path + ".py.cpp";
    }
    if (options.emit_registration) {
        output_files.registration_name = base_path + ".registration.cpp";
    }
    if (options.emit_stmt) {
        output_files.stmt_name = base_path + ".stmt";
    }
    if (options.emit_stmt_html) {
        output_files.stmt_html_name = base_path + ".html";
    }
    if (options.emit_static_library) {
        if (is_windows_coff) {
            output_files.static_library_name = base_path + ".lib";
        } else {
            output_files.static_library_name = base_path + ".a";
        }
    }
    if (options.emit_schedule) {
        output_files.schedule_name = base_path + ".schedule.h";
    }
    if (options.emit_featurization) {
        output_files.featurization_name = base_path + ".featurization";
    }
    if (options.emit_pytorch_wrapper) {
        output_files.pytorch_wrapper_name = base_path + ".pytorch";
    }
    return output_files;
}
