...
std::map<OutputFileType, const OutputInfo> get_output_info(const Target &target) {
    ...
    std::map<OutputFileType, const OutputInfo> ext = {
        ...
        {OutputFileType::path_featurization, {"path_featurization", ".path_featurization", IsMulti}},
        ...
        {OutputFileType::python_schedule, {"python_schedule", "_schedule.py", IsSingle}},
...
void Module::compile(const std::map<OutputFileType, std::string> &output_files) const {
    ...
    if (contains(output_files, OutputFileType::python_schedule)) {
        debug(1) << "Module.compile(): python_schedule " << output_files.at(OutputFileType::python_schedule) << "\n";
        std::ofstream file(output_files.at(OutputFileType::python_schedule));
        auto *r = contents->auto_scheduler_results.get();
#ifdef HALIDE_ALLOW_LEGACY_AUTOSCHEDULER_API
        std::string scheduler = r ? r->scheduler_name : "(None)";
        std::string machine_params = r ? r->machine_params_string : "(None)";
    ...
    if (contains(output_files, OutputFileType::featurization)) {
        ...
        binfile.close();
        std::ofstream featurization_index(output_files.at(OutputFileType::featurization) + ".index", std::ios::binary | std::ios_base::trunc);
        if (r) {
            featurization_index.write((const char *)r->featurization_index.data(), r->featurization_index.size());
        }
        featurization_index.close();
    }
    if (contains(output_files, OutputFileType::path_featurization)) {
        debug(1) << "Module.compile(): path_featurization " << output_files.at(OutputFileType::path_featurization) << "\n";
        // If the path_featurization data is empty, just write an empty file
        std::ofstream file(output_files.at(OutputFileType::path_featurization));
        auto *r = contents->auto_scheduler_results.get();
        if (r) {
...
void compile_multitarget(const std::string &fn_name,
    ...
    for (size_t i = 0; i < targets.size(); ++i) {
...

        {
            ...
            sub_out.erase(OutputFileType::python_schedule);
    ...
    if (contains(output_files, OutputFileType::python_schedule)) {
        debug(1) << "compile_multitarget: python_schedule " << output_files.at(OutputFileType::python_schedule) << "\n";
#ifdef HALIDE_ALLOW_LEGACY_AUTOSCHEDULER_API
        std::string scheduler = auto_scheduler_results.front().scheduler_name;
        std::string machine_params = auto_scheduler_results.front().machine_params_string;
#else
        std::string scheduler = auto_scheduler_results.front().autoscheduler_params.name;
        std::string machine_params = "";
#endif
        if (scheduler.empty()) {
            scheduler = "(None)";
        }
