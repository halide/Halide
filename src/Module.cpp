...
std::map<Output, const OutputInfo> get_output_info(const Target &target) {
    ...
    std::map<Output, const OutputInfo> ext = {
        ...
        {Output::lua_schedule, {"lua_schedule", ".lua", IsSingle}},
...
void emit_lua_schedule_file(const std::string &name,
                            const std::vector<Target> &targets,
                            const std::string &scheduler_name,
                            const std::string &machine_params_string,
                            const std::string &body,
...
$NAMESPACEOPEN$
$NESTING$apply_schedule_$SHORTNAME$ = function( pipeline, target) 
$BODY$
end
$NAMESPACECLOSE$
)INLINE_CODE";
    ...
    std::vector<std::string> namespaces;
    std::string simple_name = extract_namespaces(name, namespaces);
    std::string nsopen, nsclose, nesting;
    size_t i = 0;
    for (const auto &ns : namespaces) {
        if (i == 0) {
            nsopen += ns + " = " + ns + " or {}\n";
            ...
            nesting += "." + ns;
            nsopen += nesting + " = " + nesting + std::string(" or {}\n");
        }
        i++;
        nsclose += "-- namespace " + ns + "\n";
    }
    ...
    s = replace_all(s, "$SCHEDULER$", scheduler_name);
    s = replace_all(s, "$NAMESPACEOPEN$", nsopen);
    s = replace_all(s, "$NESTING$", nesting);
    s = replace_all(s, "$SHORTNAME$", simple_name);
    s = replace_all(s, "$CLEANNAME$", clean_name);
    s = replace_all(s, "$NAMESPACECLOSE$", nsclose);
    s = replace_all(s, "$TARGET$", target_string);
...
void emit_python_schedule_file(const std::string &name,
                               const std::vector<Target> &targets,
                               const std::string &scheduler_name,
                               const std::string &machine_params_string,
                               const std::string &body,
...
void Module::compile(const std::map<Output, std::string> &output_files) const {
    ...
    if (contains(output_files, Output::lua_schedule)) {
        debug(1) << "Module.compile(): lua_schedule " << output_files.at(Output::lua_schedule) << "\n";
        std::ofstream file(output_files.at(Output::lua_schedule));
        auto *r = contents->auto_scheduler_results.get();
        std::string scheduler = r ? r->scheduler_name : "(None)";
        std::string machine_params = r ? r->machine_params_string : "(None)";
        std::string body = r && !r->lua_schedule_source.empty() ? r->lua_schedule_source : "-- No autoscheduler has been run for this Generator.\n";
        emit_lua_schedule_file(name(), {target()}, scheduler, machine_params, body, file);
    }
    if (contains(output_files, Output::python_schedule)) {
        debug(1) << "Module.compile(): python_schedule " << output_files.at(Output::python_schedule) << "\n";
...
void compile_multitarget(const std::string &fn_name,
    ...
    if (contains(output_files, Output::lua_schedule)) {
        debug(1) << "compile_multitarget: lua_schedule " << output_files.at(Output::lua_schedule) << "\n";
        std::string scheduler = auto_scheduler_results.front().scheduler_name;
        if (scheduler.empty()) {
            scheduler = "(None)";
        }
        ...
        if (baseline_target.os == Target::OSUnknown && baseline_target.arch == Target::ArchUnknown) {
            ...
            for (size_t i = 0; i < auto_scheduler_results.size(); i++) {
                ...
                if (i == auto_scheduler_results.size() - 1) {
                    ...
                    body << "if (" << condition.str() << ") then\n";
                }
                body << indent_string(a.lua_schedule_source, "    ");
                body << "    return\n";
                body << "end";
            }
        }
        ...
        std::ofstream file(output_files.at(Output::schedule));
        emit_lua_schedule_file(fn_name, targets, scheduler, machine_params, body.str(), file);
    }
    if (contains(output_files, Output::python_schedule)) {
        debug(1) << "compile_multitarget: python_schedule " << output_files.at(Output::python_schedule) << "\n";
        std::string scheduler = auto_scheduler_results.front().scheduler_name;
        if (scheduler.empty()) {
            scheduler = "(None)";
        }
        ...
        if (baseline_target.os == Target::OSUnknown && baseline_target.arch == Target::ArchUnknown) {
            ...
            for (size_t i = 0; i < auto_scheduler_results.size(); i++) {
                ...
                body << indent_string(a.python_schedule_source, "    ");
                body << "    return\n";
                body << "\n";
            }
        }
