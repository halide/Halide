 ...
 * precompiled Metal shaders (metallib files). If both are set to
 * non-empty strings, the Metal code generator will invoke these tools
 * to precompile shaders instead of embedding source code.
 *
 * The compiler should typically be set to something like "xcrun -sdk macosx metal"
 * and the linker to "xcrun -sdk macosx metallib".
 *
 * If either is empty, Metal shaders will be embedded as source code.
 *
...
/** Get the Metal compiler command that was set via set_metal_compilation_tools().
 * Returns empty string if not set. */
std::string get_metal_compiler();

/** Get the Metal linker command that was set via set_metal_compilation_tools().
 * Returns empty string if not set. */
