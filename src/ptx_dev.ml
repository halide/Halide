...
let rec cg_expr con = function
  | Debug (e, _, _) ->
      dbg 2 "Skipping Debug expr inside device kernel\n%!";
      cg_expr con e
  | e -> con.cg_expr e
...
let rec cg_stmt con stmt = match stmt with
  ...
  | Assert _ | Print _ ->
      dbg 2 "Dropping Print/Assert stmt inside device kernel\n%!";
  ...
  | For (name, base, width, ordered, body) when is_simt_var name ->
      ...
      dbg 2 "Dropping %s loop on %s (%s..%s)\n%!"
...
let malloc ?force_heap:(force_heap=false) con name count elem_size =
  ...
  in
  dbg 2 "malloc %s[%d bytes] on PTX device\n%!" name size;
