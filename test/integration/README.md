...
this folder **should not** be added to the main Halide build via
`add_subdirectory`.
...
compilation from x86 to aarch64 (tested via Qemu). This test in particular
cannot be easily run on the buildbots because it requires two VMs: an Ubuntu
build machine for Halide, and an Ubuntu developer machine which installs the DEB
packages.

Consult the `packaging.yml` workflow file for precise steps to run these
locally.
