...
bool test(int w, bool div) {
    ...
    double t_correct = benchmark([&]() { g.realize(correct); }).wall_time;

    Buffer<T> fast = f.realize(input.width(), num_vals);
    double t_fast = benchmark([&]() { f.realize(fast); }).wall_time;

    Buffer<T> fast_dynamic = h.realize(input.width(), num_vals);
    double t_fast_dynamic = benchmark([&]() { h.realize(fast_dynamic); }).wall_time;
