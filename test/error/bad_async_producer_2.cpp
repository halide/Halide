#include "Halide.h"
#include "halide_test_error.h"

using namespace Halide;

// From https://github.com/halide/Halide/issues/5201

namespace {
void TestBadAsyncProducer2() {
    Func producer1, producer2, consumer;
    Var x, y;

    producer1(x, y) = x + y;
    producer2(x, y) = producer1(x, y);
    consumer(x, y) = producer2(x, y - 1) + producer2(x, y + 1);

    consumer.compute_root();

    producer1.compute_at(consumer, y).async();
    producer2.store_root().compute_at(consumer, y).async();

    consumer.bound(x, 0, 16).bound(y, 0, 16);

    Buffer<int> out = consumer.realize({16, 16});
}
}  // namespace

TEST(ErrorTests, BadAsyncProducer2) {
    EXPECT_COMPILE_ERROR(
        TestBadAsyncProducer2,
        MatchesPattern(R"(The Func f\d+ is consumed by async Func f\d+ and )"
                       R"(has a compute_at location in between the store_at )"
                       R"(location and the compute_at location of f\d+\. )"
                       R"(This is only legal when f\d+ is both async and has )"
                       R"(a store_at location outside the store_at location )"
                       R"(of the consumer\.)"));
}
