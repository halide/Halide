...
// Reinterpret a field's bytes into a same-sized plain integer, which doesn't
// work for a nested struct field (whose bits() is not its byte size).
// Populating the buffer manually (rather than via a materialized Halide
// Store) sidesteps the separate, equally-unsupported Store-side restriction
// (see struct_nested_field_store.cpp) so that this specific, documented
// read-side restriction is what actually fires.
int main(int argc, char **argv) {
    ...
    Buffer<> Wt_buf(outer_t, data.data(), 1, shape);
