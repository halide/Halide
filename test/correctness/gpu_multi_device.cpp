...
struct MultiDevicePipeline {
    ...
    MultiDevicePipeline(Func input) {
        ...
        if (jit_target.has_feature(Target::OpenCL)) {
            ...
            stage[current_stage].compute_root().reorder(c, x, y).gpu_tile(x, y, tx, ty, 32, 32, TailStrategy::Auto, DeviceAPI::OpenCL);
            current_stage++;
        }
        if (jit_target.has_feature(Target::CUDA)) {
            stage[current_stage](x, y, c) = stage[current_stage - 1](x, y, c) + 69;
            stage[current_stage].compute_root().reorder(c, x, y).gpu_tile(x, y, tx, ty, 32, 32, TailStrategy::Auto, DeviceAPI::CUDA);
            current_stage++;
        }
        ...
        if (jit_target.has_feature(Target::OpenGLCompute)) {
            stage[current_stage](x, y, c) = stage[current_stage - 1](x, y, c) + 69;
            stage[current_stage].compute_root().reorder(c, x, y).gpu_tile(x, y, tx, ty, 32, 32, TailStrategy::Auto, DeviceAPI::OpenGLCompute);
            current_stage++;
        }
    }
