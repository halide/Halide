...
TEST(Undef, Fibonacci) {
    // Compute fibonacci with and without undef initialization
    Var x{"x"};
...
TEST(Undef, TupleStencil) {
    // Use undef in a tuple. The following code ping-pongs between the two tuple components using a stencil:
    Var x{"x"};
    RDom rx(0, 100);
    Func f3;
    ...
    for (int i = 0; i < 100; i++) {
        ASSERT_LT(std::abs(o1(i)), 1.0f) << "o1(" << i << ") = " << o1(i) << " outside of [-1, 1]";
        ASSERT_LT(std::abs(o2(i)), 1.0f) << "o2(" << i << ") = " << o2(i) << " outside of [-1, 1]";
    }
}

TEST(Undef, SelectUndef) {
    ...
    for (int i = 0; i < 4; i++) {
        ASSERT_EQ(expected[i], output(i)) << "Mismatch at index " << i << ": expected " << expected[i] << ", got " << output(i);
    }
}