...
int main(int argc, char **argv) {
    ...
    // The kernel in this test is just a simple box blur.
    Func box;
    box(x, y) = cast<uint16_t>(1);
     ...
     */
    Func blur1("blur1");
    blur1(x, y) += box(r.x, r.y) * input(x + r.x - 1, y + r.y - 1);
     ...
     */
    Func blur2("blur2");
    blur2(x, y) = sum(box(r.x, r.y) * input(x + r.x - 1, y + r.y - 1));
    ...
    for (int y = 1; y < H-1; y++) {
        for (int x = 1; x < W-1; x++) {
            uint16_t correct = (1*in(x-1, y-1) + 1*in(x, y-1) + 1*in(x+1, y-1) +
                                1*in(x-1, y)   + 1*in(x, y) +   1*in(x+1, y) +
                                1*in(x-1, y+1) + 1*in(x, y+1) + 1*in(x+1, y+1));
            if (out1(x, y) != correct) {
