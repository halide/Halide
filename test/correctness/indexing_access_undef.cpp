...
TEST(IndexingAccessUndefTest, ClampUnsafeAccessesWithFunctions) {
    Var x{"x"};
    ...
    ASSERT_NO_THROW(out.realize({1}));
}

TEST(IndexingAccessUndefTest, ClampUnsafeAccessesWithInputImage) {
    Var x{"x"};
    ...
    // No crash means success
    ASSERT_NO_THROW(h.realize({1024}));
}
