...
TEST(ReorderRvarsTest, ReorderSemantics) {
    Var x("x"), y("y");
    ...
    EXPECT_EQ(err, 0) << "Reordering rvars affected the meaning!";
}

TEST(ReorderRvarsTest, SummedAreaTable) {
    ...
    Buffer<int> result;
    ASSERT_NO_THROW(result = sat.realize({100, 100}));
}
