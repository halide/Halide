...
TEST(MultiPassReduction, TwoUpdateSteps) {
    ...
    for (int i = 0; i < 11; i++) {
        ASSERT_NEAR(result(i), ref[i], 0.0001f) << "result(" << i << ") = " << result(i) << " instead of " << ref[i];
    }
}

TEST(MultiPassReduction, ComplexUpdates) {
    ...
    for (int i = 0; i < 110; i++) {
        ASSERT_EQ(correct[i], result(i)) << "result(" << i << ") = " << result(i) << " instead of " << correct[i];
    }
}

TEST(MultiPassReduction, Fibonacci) {
    ...
    for (int i = 2; i < 20; i++) {
        ...
        ASSERT_EQ(ref[i], result(i)) << "fibonacci(" << i << ") = " << result(i) << " instead of " << ref[i];
    }
}

TEST(MultiPassReduction, IntegralImage) {
    ...
    for (int y = 0; y < 100; y++) {
        for (int x = 0; x < 100; x++) {
            ASSERT_NEAR(ref(x, y), result(x, y), 0.0001f) << "integral image at (" << x << ", " << y << ") = " << result(x, y) << " instead of " << ref(x, y);
        }
    }
}