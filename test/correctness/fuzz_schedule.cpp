...
namespace {
class FuzzScheduleTest : public testing::Test {
protected:
    Buffer<int> correct;
    ...
    void expect_blur_output_correct(Func output) const {
        Buffer<int> out;
        ASSERT_NO_THROW(out = output.realize({32, 32}));
        for (int y = 0; y < out.height(); y++) {
            for (int x = 0; x < out.width(); x++) {
                EXPECT_EQ(out(x, y), correct(x, y)) << "x = " << x << ", y = " << y;
            }
        }
    }
};
...
TEST_F(FuzzScheduleTest, Issue7851) {
    Func input("input");
    Func local_sum("local_sum");
    Func blurry("blurry");
    Var x("x"), y("y");
    ...
    blurry.split(y, yo, yi, 2, TailStrategy::RoundUp)
        .fuse(yo, x, yo_x_f)
        .vectorize(yi)
        .split(yo_x_f, yo_x_fo, yo_x_fi, 2, TailStrategy::Predicate)
        .reorder(yo_x_fo, yo_x_fi, yi);
    input.split(y, yo, yi, 2, TailStrategy::PredicateStores)
        .fuse(yo, x, yo_x_f)
        .vectorize(yi)
        .split(yo_x_f, yo_x_fo, yo_x_fi, 2, TailStrategy::Predicate)
        .reorder(yo_x_fo, yo_x_fi, yi);
    ...
    expect_blur_output_correct(blurry);
}

// https://github.com/halide/Halide/issues/7873
TEST_F(FuzzScheduleTest, Issue7873) {
    Func input("input");
    Func local_sum("local_sum");
    Func blurry("blurry");
    Var x("x"), y("y");
...
TEST_F(FuzzScheduleTest, Issue7872) {
    Func input("input");
    Func local_sum("local_sum");
    Func blurry("blurry");
    Var x("x"), y("y");
...
TEST_F(FuzzScheduleTest, Issue7891) {
    Func input("input");
    Func local_sum("local_sum");
    Func blurry("blurry");
    Var x("x"), y("y");
...
TEST_F(FuzzScheduleTest, Issue7892) {
    Func input("input");
    Func local_sum("local_sum");
    Func blurry("blurry");
    Var x("x"), y("y");
...
TEST_F(FuzzScheduleTest, Issue7906) {
    Func input("input");
    Func local_sum("local_sum");
    Func blurry("blurry");
    Var x("x"), y("y");
    ...
    Var yo, yi, x_yo_f;
    input.vectorize(y)
        .split(y, yo, yi, 2, TailStrategy::ShiftInwards)
        .unroll(x)
        .fuse(x, yo, x_yo_f);
    blurry.compute_root();
...
TEST_F(FuzzScheduleTest, Issue7909) {
    Func input("input");
    Func local_sum("local_sum");
    Func blurry("blurry");
    Var x("x"), y("y");
...
TEST_F(FuzzScheduleTest, Issue8038) {
    Func input("input");
    Func local_sum("local_sum");
    Func blurry("blurry");
...
TEST_F(FuzzScheduleTest, Issue7890) {
    Func input("input");
    Func local_sum("local_sum");
    Func blurry("blurry");
    Var x("x"), y("y");
...
TEST_F(FuzzScheduleTest, Issue8054) {
    ...
    blury.compute_root()
        .split(x, x, v6, 6, TailStrategy::GuardWithIf)
        .split(z, zo, vzi, 8, TailStrategy::GuardWithIf)
        .reorder(y, x, c, vzi, zo, v6)
        .vectorize(vzi)
        .vectorize(v6);

    EXPECT_NO_THROW(p.compile_to_module({input}, "bilateral_grid", {Target("host")}));
}
