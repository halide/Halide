...
void expect_failure(int r, const char *expected_msg) {
    EXPECT_NE(r, 0) << "Expected failure, got success";
    EXPECT_THAT(error_msg, HasSubstr(expected_msg));
    error_msg = "";
}

void expect_success(int r) {
    EXPECT_EQ(r, 0) << "Expected success, got failure";
    EXPECT_EQ(error_msg, "");
    error_msg = "";
}
}  // namespace

TEST(CallableErrorsTest, BadUntypedCallsHandleWithJIT) {
...
TEST(CallableErrorsTest, BadUntypedCallsHandleWithUserContext) {
...
TEST(CallableErrorsTest, BadTypedCallsHandleWithJIT) {
    // Test custom error handler in the JITHandler
    Param<int32_t> p_int("p_int");
    Param<float> p_float("p_float");
    ImageParam p_img(UInt(8), 2, "p_img");
...
TEST(CallableErrorsTest, BadTypedCallsHandleWithUserContext) {
    // Test custom error handler in the JITUserContext
