...
// supported for the fully-inlined case in v1 (see the user_error in
// LowerStructTypes.cpp's lower_field_read_from_load for the buffer-backed
// case) -- this test exercises the supported path, and confirms the
// unsupported one fails with a clear error rather than miscompiling.
void test_nested_struct() {
    ...
    // The unsupported path (reading a nested field from a non-inlined,
    // buffer-backed struct) should fail with a clear Halide::Error, not
    // miscompile or crash.
    bool threw = false;
    try {
        Func producer2("producer2");
        producer2(i) = pack_struct(outer_t, {pack_struct(inner_t, {cast<int32_t>(i * 10), cast<uint8_t>(i)}), cast<float>(i) + 0.5f});
        producer2.compute_root();
        Func consumer2("consumer2");
        Expr inner_val2 = field(producer2(i), "inner");
        consumer2(i) = field(inner_val2, "a");
        consumer2.realize({n});
    } catch (const Halide::Error &) {
        threw = true;
    }
    if (!threw) {
        printf("nested_struct: expected reading a nested field from a non-inlined struct buffer to fail\n");
        exit(1);
    }
}
