...
// A block_q4_0-shaped struct: one float16 scale followed by 16 packed byte-wide "codes".
...
void test_type_struct_basics() {
    ...
    if (block_a.code() != halide_type_struct || block_a.lanes() != 1) {
        printf("Expected a struct type to have code halide_type_struct and one lane\n");
        exit(1);
    }
    if (block_a.is_uint() || block_a.is_int() || block_a.is_handle()) {
        printf("A struct type should not report as a numeric or handle type\n");
        exit(1);
    }
    if (!(block_a == block_b)) {
    ...
    // silently degrade into a plain integer partway through lowering (e.g.
    // StorageFlattening rounding bits up to a byte multiple). Only with_code
    // to a different code drops struct-ness.
    if (!block_a.with_bits(8).is_struct() || !block_a.with_bits(16).is_struct()) {
        printf("Type::with_bits should preserve struct-ness\n");
        exit(1);
    }
    if (block_a.with_bits(16).bytes() != 18) {
        printf("with_bits should not disturb a struct's real byte size\n");
        exit(1);
    }
    ...
    if (block_a.with_code(Type::UInt).is_struct()) {
        printf("Type::with_code(non-struct code) should drop struct-ness\n");
        exit(1);
    }
}
...
void test_read_from_buffer() {
    ...
    // row's stride is num_blocks), the same convention any other typed buffer
    // uses. The buffer's element type is the struct itself, so its element size
    // is block_bytes and the strides scale up to the byte layout above.
    halide_dimension_t shape[2] = {
        {0, num_blocks, 1, 0},
        {0, num_rows, num_blocks, 0},
    };
    Buffer<> Wt_buf(block_t, data.data(), 2, shape);
...
void test_realized_writer_as_output() {
    ...
    // Explicit gather() generator form for the array field (extent inferred
    // from the struct field), alongside a scalar field initializer.
    producer(blk) = pack_struct(
        block_t,
        {
            cast<float16_t>(0.5f + cast<float>(blk)),                        // d
            gather(16, [&](Expr k) { return cast<uint8_t>(blk * 5 + k); }),  // qs
        });

    Buffer<> raw = producer.realize({num_blocks});
    // The realized output buffer reports a struct element type of the right byte
    // size, but not its field layout -- Halide::Buffer<>::type() round-trips
    // through the ABI-level halide_type_t, which carries a struct's packed size
    // but forgets its fields (the same limitation Handle-typed buffers have for
    // their pointee type). Dimensionality is exactly what producer declared
    // (1, just `blk`) -- no hidden extra dimension.
    if (!raw.type().is_struct() || raw.type().bytes() != block_t.bytes()) {
        printf("Realized struct output should report a struct element type of %d bytes, got %d-byte %s\n",
               block_t.bytes(), raw.type().bytes(), raw.type().is_struct() ? "struct" : "non-struct");
        exit(1);
    }
...
// One initializer per field of make_block_type(): a scalar `d`, and the array
// field `qs` filled by sweeping the `_` placeholder over its extent -- note the
// index arithmetic (blk * code_mul + _), which the bare placeholder form
// supports because the extent is a compile-time constant.
std::vector<StructFieldInit> block_field_values(const Var &blk, float d_offset, int code_mul) {
    return {
        cast<float16_t>(d_offset + cast<float>(blk)),  // d
        cast<uint8_t>(blk * code_mul + _),             // qs[16], swept over [0, 16)
    };
}
...
void test_non_dense_stride() {
    ...
    Buffer<> Wt_buf(block_t, data.data(), 1, shape);
...
// pack_struct can initialize an array field by copying a whole same-typed
// array field out of another struct value, via field(other, "name"). Here we
// re-pack a struct keeping its `qs` array verbatim while replacing `d`.
void test_pack_struct_field_copy() {
    Type block_t = make_block_type();
    Var blk("blk"), k("k");
    const int num_blocks = 5;

    Func src("src");
    src(blk) = pack_struct(block_t, block_field_values(blk, 2.0f, 3));
    src.compute_root();  // force a materialized struct buffer, so the copy reads a real field

    Func repacked("repacked");
    Expr s = src(blk);
    repacked(blk) = pack_struct(
        block_t,
        {
            cast<float16_t>(100.0f + cast<float>(blk)),  // new d
            field(s, "qs"),                              // copy qs[16] from src
        });

    Func out_d("out_d"), out_q("out_q");
    out_d(blk) = cast<float>(field(repacked(blk), "d"));
    out_q(k, blk) = cast<int32_t>(field(repacked(blk), "qs")[k]);

    Buffer<float> d_res = out_d.realize({num_blocks});
    Buffer<int32_t> q_res = out_q.realize({16, num_blocks});
    for (int blk_i = 0; blk_i < num_blocks; blk_i++) {
        float exp_d = (float)float16_t(100.0f + (float)blk_i);
        if (d_res(blk_i) != exp_d) {
            printf("field_copy: d(%d) = %f instead of %f\n", blk_i, d_res(blk_i), exp_d);
            exit(1);
        }
        for (int k_i = 0; k_i < 16; k_i++) {
            int32_t exp_q = (uint8_t)(blk_i * 3 + k_i);  // matches block_field_values(blk, 2.0f, 3)
            if (q_res(k_i, blk_i) != exp_q) {
                printf("field_copy: qs(%d, %d) = %d instead of %d\n", k_i, blk_i, q_res(k_i, blk_i), exp_q);
                exit(1);
            }
        }
    }
