...
namespace {
class GPUNonMonotonicSharedMemSizeTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(GPUNonMonotonicSharedMemSizeTest, SingleDimensionQuadratic) {
    Func f, g;
    Var x;
    const int size = 256;
    ...
    ASSERT_NO_THROW(g.realize({size}))
        << "Should use " << max_elements << " bytes of shared memory";
}

TEST_F(GPUNonMonotonicSharedMemSizeTest, MultiDimensionQuadratic) {
    ...
    ASSERT_NO_THROW(g.realize({width, height}))
        << "Should use " << max_elements << " bytes of shared memory";
}

TEST_F(GPUNonMonotonicSharedMemSizeTest, GlobalMemoryAllocation) {
    ...
    ASSERT_NO_THROW(g.realize({width, height}))
        << "Should use " << max_elements << " bytes of global memory";
}

TEST_F(GPUNonMonotonicSharedMemSizeTest, MixedMemoryTypes) {
    ...
    ASSERT_NO_THROW(g.realize({width, height}))
        << "should use " << heap_bytes << " bytes of global memory "
        << "and " << max_elements << " bytes of shared memory";
}
