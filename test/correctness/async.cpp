...
TEST_F(AsyncTest, BasicComputeRootAsyncProducer) {
    ...
    out.for_each_element([&](int x, int y) {
        int correct = 2 * (x + y);
        EXPECT_EQ(out(x, y), correct) << "out(" << x << ", " << y << ") = " << out(x, y) << " instead of " << correct;
    });
}

TEST_F(AsyncTest, SlidingAndFoldingSingleVariable) {
    ...
    out.for_each_element([&](int x) {
        int correct = 2 * x - 1;
        EXPECT_EQ(out(x), correct) << "out(" << x << ") = " << out(x) << " instead of " << correct;
    });
}

TEST_F(AsyncTest, SlidingAndFoldingSingleVariableFlipped) {
...
TEST_F(AsyncTest, SlidingAndFoldingOverY) {
...
TEST_F(AsyncTest, SlidingOverXAndYFoldingOverY) {
...
TEST_F(AsyncTest, SlidingOverXFoldingOverXAndY) {
...
TEST_F(AsyncTest, MultipleAsyncProducersAtRoot) {
...
TEST_F(AsyncTest, MultipleAsyncProducersInsideOuterParallelFor) {
...
TEST_F(AsyncTest, MultipleAsyncProducersInsideOuterParallelForWithSlidingWindow) {
    // Multiple async producers inside an outer parallel for loop
...
TEST_F(AsyncTest, NestedAsyncTasts) {
...
TEST_F(AsyncTest, TwoAsyncProducerConsumerPairs) {
...
TEST_F(AsyncTest, SlidingAndFoldingOverYWithVariableResources) {
...
TEST_F(AsyncTest, SlidingAndFoldingOverYWithVariableResourcesAndFlipInY) {
    // Sliding and folding over y, with a non-constant amount of stuff
...
TEST_F(AsyncTest, DownsampleBy2xWithSlidingAndFolding) {
...
TEST_F(AsyncTest, DownsampleBy1Point5xWithSlidingAndFolding) {
...
TEST_F(AsyncTest, IncludeProducersComputedAtOutermost) {
