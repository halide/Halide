...
TEST(BoundStorageTest, ConstantBound) {
    Func f("f"), g("g");
    Var x("x"), y("y");
    f(x, y) = x + y;
    g(x, y) = 2 * f(x, y);
    ...
    ASSERT_EQ(s.allocation_size["f"], fixed_alloc_size);
    ...
    for (int y = 0; y < im.height(); y++) {
        for (int x = 0; x < im.width(); x++) {
            int correct = 2 * (x + y);
            ASSERT_EQ(im(x, y), correct);
        }
    }
}

TEST(BoundStorageTest, MultipleBounds) {
    Func f("f"), h("h"), g("g");
    Var x("x"), y("y");
    f(x, y) = x + y;
    h(x, y) = x - 2 * y;
    ...
    g.add_custom_lowering_pass(&s, []() {});
    Module m = g.compile_to_module({});
    EXPECT_EQ(s.allocation_size["f"], fixed_alloc_size_f);
    EXPECT_EQ(s.allocation_size["h"], fixed_alloc_size_h * fixed_alloc_size_h);
...
TEST(BoundStorageTest, ExpressionBound) {
