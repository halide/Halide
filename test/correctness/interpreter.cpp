...
protected:
    ...
    void SetUp() override {
        if (target.has_feature(Target::Vulkan)) {
            GTEST_SKIP() << "Skipping test for Vulkan (which doesn't support dynamically allocated shared mem)!";
        }

        // Workaround for https://github.com/halide/Halide/issues/7420
        if (target.has_feature(Target::WebGPU)) {
            GTEST_SKIP() << "workaround for issue #7420";
        }
        ...
        if (target.has_gpu_feature()) {
            ...
            scratch.compute_at(output, x).gpu_threads(x, y);
        } else {
...
TEST_F(InterpreterTest, BasicInterpreter) {
    ...
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
            ...
            ASSERT_EQ(out_buf(x, y), correct) << "out_buf(" << x << ", " << y << ") = " << out_buf(x, y) << " instead of " << correct;
        }
    }
}

TEST_F(InterpreterTest, SqrtProgram) {
    const int W = 128, H = 128;
    Buffer<uint8_t> in_buf(W + 2, H + 2);
    in_buf.set_min(-1, -1);
    std::mt19937 rng{0};
