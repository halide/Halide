...
int main(int argc, char **argv) {

    {
        ...
        if (target.has_gpu_feature()) {
            Var tx, ty;
            f.compute_root().gpu_tile(x, y, tx, ty, 1, 1).debug_to_file("f.tmp");
            g.compute_root().gpu_tile(x, y, tx, ty, 1, 1).debug_to_file("g.tmp");
            h.compute_root().gpu_tile(x, y, tx, ty, 1, 1).debug_to_file("h.tmp");
        } else {
