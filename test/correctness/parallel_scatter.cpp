...
class ParallelScatterTest : public ::testing::TestWithParam<ExecutionMode> {};
TEST_P(ParallelScatterTest, ParallelScatter) {
    ...
    if (GetParam() == ExecutionMode::Parallel) {
        hist.update().atomic().parallel(ri).parallel(x).vectorize(ro);
    }

    Buffer<int> result;
    ASSERT_NO_THROW(result = hist.realize({10, 100}));
    for (int i = 0; i < result.width(); i++) {
        for (int j = 0; j < result.height(); j++) {
            ...
            EXPECT_EQ(result(i, j), correct) << "result(" << i << ", " << j << ")";
        }
    }
}
...
INSTANTIATE_TEST_SUITE_P(
    ExecutionModes,
    ParallelScatterTest,
    ::testing::Values(ExecutionMode::Serial, ExecutionMode::Parallel)
);
