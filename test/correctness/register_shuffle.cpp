...
class RegisterShuffleTest : public ::testing::Test {
protected:
    Target target{get_jit_target_from_environment()};
    void SetUp() override {
        int cap = target.get_cuda_capability_lower_bound();
        if (cap < 50) {
            GTEST_SKIP() << "CUDA with capability greater than or equal to 5.0 required, cap:" << cap;
        }
    }
};
...
TEST_F(RegisterShuffleTest, ShuffleSmallConvolution) {
...
TEST_F(RegisterShuffleTest, BroadcastOuterProduct) {
...
TEST_F(RegisterShuffleTest, VectorizedBroadcast) {
    ...
    if (Internal::get_llvm_version() == 211) {
        GTEST_SKIP() << "Skipping due to LLVM 21.1.0 float32x2 vector bugs";
    }
...
TEST_F(RegisterShuffleTest, StencilChainMaskedLanes) {
...
TEST_F(RegisterShuffleTest, StencilChainHalfWarps) {
...
TEST_F(RegisterShuffleTest, ShuffleWithVariableShift) {
...
TEST_F(RegisterShuffleTest, BilinearUpsample) {
    // As 8/26/2025 llvm 21.1.0 includes some bugs w.r.t float32x2 vectors.
    // See https://github.com/llvm/llvm-project/pull/126337#issuecomment-3192412599
    if (Internal::get_llvm_version() == 211) {
        GTEST_SKIP() << "Skipping due to LLVM 21.1.0 float32x2 vector bugs";
...
TEST_F(RegisterShuffleTest, BoxDownsampleSummation) {
...
TEST_F(RegisterShuffleTest, BoxDownsampleNarrowTile) {
...
TEST_F(RegisterShuffleTest, LookupTableInRegister) {
...
TEST_F(RegisterShuffleTest, WarpShuffleReduction) {
    ...
    for (int y = 0; y < out.width(); y++) {
        ...
        EXPECT_EQ(out(y), correct) << "y = " << y;
    }
}

TEST_F(RegisterShuffleTest, CombinatorialExplosion) {
