...
void test(Target t) {
    for (int factor : {4, 8, 16}) {
        for (int vec : {1, 2, 4}) {
            ...
            std::basic_regex<char> regex("dp4a[.].32[.].32");
            if (!std::regex_search((const char *)buf.begin(), (const char *)buf.end(), regex)) {
                printf("Did not find use of dp4a in compiled code. Rerun test with HL_DEBUG_CODEGEN=1 to debug\n");
                exit(-1);
            }
        }
    }
