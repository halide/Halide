...
namespace {
struct StorageFoldingContext : JITUserContext {
    std::set<size_t> custom_malloc_sizes{};

    StorageFoldingContext() {
        handlers.custom_malloc = my_malloc;
        handlers.custom_free = my_free;
    }

    static void *my_malloc(JITUserContext *ctx, size_t x) {
        auto *self = static_cast<StorageFoldingContext *>(ctx);
        self->custom_malloc_sizes.insert(x);
    ...
    static void my_free(JITUserContext *, void *ptr) {
        free(((void **)ptr)[-1]);
    }
};

struct ExpectedErrorContext : JITUserContext {
    ...
    static void custom_error(JITUserContext *ctx, const char *msg) {
        ...
        auto *self = static_cast<ExpectedErrorContext *>(ctx);
        printf("Expected err: %s\n", msg);
        self->error_occurred = true;
    }
};

class StorageFoldingTest : public ::testing::Test {
protected:
    StorageFoldingContext ctx;
    ...
    void check_expected_malloc(size_t expected) const {
        for (size_t i : ctx.custom_malloc_sizes) {
            if (std::abs((int)i - (int)expected) <= tolerance) {
                return;
            }
        }
        testing::Message msg;
        msg << "Expected an allocation of size " << expected << " (tolerance " << tolerance << ")";
        for (size_t i : ctx.custom_malloc_sizes) {
            msg << "\n  " << i;
        }
        ADD_FAILURE() << msg;
    ...
    void check_expected_mallocs(const std::vector<size_t> &expected) const {
        for (size_t i : expected) {
            check_expected_malloc(i);
        }
    }

    static void realize_and_expect_error(Func f, int w, int h) {
        ExpectedErrorContext err_ctx;
        f.realize(&err_ctx, {w, h});
        EXPECT_TRUE(err_ctx.error_occurred);
    }
};

// An extern stage that copies input -> output
extern "C" HALIDE_EXPORT_SYMBOL int storage_folding_buffer_copy(halide_buffer_t *in, halide_buffer_t *out) {
...
TEST_F(StorageFoldingTest, AutomaticFoldingInYAndC) {
    Func f, g;
    ...
    ASSERT_NO_THROW(g.realize(&ctx, {100, 1000, 3}));

    size_t expected_size = 101 * 4 * sizeof(int);
    check_expected_mallocs({expected_size});
}

TEST_F(StorageFoldingTest, NoFoldingWithMultipleProducers) {
    Func f, g;
...
TEST_F(StorageFoldingTest, ExplicitFoldingWithMultipleProducers) {
    Func f, g;
    ...
    ASSERT_NO_THROW(g.realize(&ctx, {100, 1000}));

    size_t expected_size = 101 * 3 * sizeof(int);
    check_expected_mallocs({expected_size});
}

TEST_F(StorageFoldingTest, StackAllocationForNonOverlappingBoxes) {
    Func f, g;
    ...
    Buffer<int> im = f.realize(&ctx, {1000, 1000});

    EXPECT_TRUE(ctx.custom_malloc_sizes.empty()) << "There should not have been a heap allocation";

    for (int y = 0; y < im.height(); y++) {
        for (int x = 0; x < im.width(); x++) {
            EXPECT_EQ(im(x, y), 2 * x * (2 * y) + (2 * x + 1) * (2 * y + 1)) << "x = " << x << ", y = " << y;
        }
    }
}

TEST_F(StorageFoldingTest, FoldBothDimensionsNonOverlappingSlices) {
    Func f, g;
    ...
    for (int y = 0; y < im.height(); y++) {
        for (int x = 0; x < im.width(); x++) {
            EXPECT_EQ(im(x, y), x * (2 * y) + (x + 3) * (2 * y + 1)) << "x = " << x << ", y = " << y;
        }
    }
}

TEST_F(StorageFoldingTest, CannotFoldXDueToYStencil) {
    Func f, g;
    ...
    for (int y = 0; y < im.height(); y++) {
        for (int x = 0; x < im.width(); x++) {
            EXPECT_EQ(im(x, y), 2 * x * y + (2 * x + 1) * (y + 3)) << "x = " << x << ", y = " << y;
        }
    }
}

TEST_F(StorageFoldingTest, SplitLogicWithBoundedHeight) {
    Func f, g;
    ...
    for (int y = 0; y < im.height(); y++) {
        for (int x = 0; x < im.width(); x++) {
            EXPECT_EQ(im(x, y), x * y) << "x = " << x << ", y = " << y;
        }
    }
}

TEST_F(StorageFoldingTest, ExplicitFoldByThreeElements) {
    Func f, g;
    ...
    for (int y = 0; y < im.height(); y++) {
        for (int x = 0; x < im.width(); x++) {
            EXPECT_EQ(im(x, y), 2 * x * y + (2 * x + 1) * (y + 2)) << "x = " << x << ", y = " << y;
        }
    }
}

TEST_F(StorageFoldingTest, UpsamplingWithStorageFolding) {
    Func f, g;
    ...
    for (int y = 0; y < im.height(); y++) {
        for (int x = 0; x < im.width(); x++) {
            EXPECT_EQ(im(x, y), x * (y / 2) + x * (y / 2 + 1)) << "x = " << x << ", y = " << y;
        }
    }
}

TEST_F(StorageFoldingTest, TwoStagesOfUpsamplingWithFolding) {
    Func f, g, h;
    ...
    check_expected_mallocs({expected_size_g, expected_size_h});

    auto correct_h = [](int xx, int yy) { return xx * yy; };
    auto correct_g = [=](int xx, int yy) { return correct_h(xx, yy / 2) + correct_h(xx, yy / 2 + 1); };
    auto correct_f = [=](int xx, int yy) { return correct_g(xx, yy / 2) + correct_g(xx, yy / 2 + 1); };
    for (int y = 0; y < im.height(); y++) {
        for (int x = 0; x < im.width(); x++) {
            EXPECT_EQ(im(x, y), correct_f(x, y)) << "x = " << x << ", y = " << y;
        }
    }
}

TEST_F(StorageFoldingTest, ExplicitFoldingWithOuterLoop) {
    for (bool interleave : {false, true}) {
        ...
        Buffer<int> im = g.realize(&ctx, {100, 1000, 3});
...
TEST_F(StorageFoldingTest, FoldExternStageOutput) {
    // Fold the storage of the output of an extern stage
    Func f, g, h;
    Var x, y;
    f(x, y) = x + y;
    g.define_extern("storage_folding_buffer_copy", {f}, Int(32), 2);
    h(x, y) = g(x, y);
    ...
    Buffer<int> out = h.realize(&ctx, {64, 64});
    for (int yy = 0; yy < out.height(); yy++) {
        for (int xx = 0; xx < out.width(); xx++) {
            EXPECT_EQ(out(xx, yy), xx + yy) << "xx = " << xx << ", yy = " << yy;
        }
    }
}

TEST_F(StorageFoldingTest, FoldExternStageInput) {
...
// TODO: move these to test/error
// Now we check some error cases.
TEST_F(StorageFoldingTest, ErrorTooSmallFoldFactor) {
...
TEST_F(StorageFoldingTest, ErrorOverlapsFoldBoundary) {
...
TEST_F(StorageFoldingTest, ErrorNonMonotonicExternStage) {
...
TEST_F(StorageFoldingTest, ErrorExternOutputCrossesFoldBoundary) {
...
TEST_F(StorageFoldingTest, ComplexTilingWithFoldingAndAlignment) {
    ...
    ASSERT_NO_THROW(output.realize({64, 64}));
}
