...
int main(int argc, char **argv) {
...

    {
        ...
        if (target.has_gpu_feature()) {
            Var tx, ty;
            f.compute_root().gpu_tile(x, y, tx, ty, 1, 1).reorder_storage(y, x).debug_to_file("f2.tmp");
            g.compute_root().gpu_tile(x, y, tx, ty, 1, 1).reorder_storage(y, x).debug_to_file("g2.tmp");
            h.compute_root().gpu_tile(x, y, tx, ty, 1, 1).debug_to_file("h2.tmp");
        } else {
