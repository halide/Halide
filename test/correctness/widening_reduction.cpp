...
namespace {
class WideningReductionTest : public ::testing::Test {
protected:
    ...
    void SetUp() override {
        if (target.has_feature(Target::Vulkan) && ((target.os == Target::IOS) || target.os == Target::OSX)) {
            GTEST_SKIP() << "Skipping test for Vulkan on iOS/OSX (MoltenVK fails to convert max/min intrinsics correctly)!";
        }
        ...
        // Boundary condition.
        input = BoundaryConditions::repeat_edge(in);
        input.compute_root();
    }
};
}  // namespace

TEST_F(WideningReductionTest, WideningReductionNarrowing) {
    Func f;
    ...
    for (int y = 1; y < H - 1; y++) {
        for (int x = 1; x < W - 1; x++) {
            ...
            EXPECT_EQ(correct, out(x, y)) << "out(" << x << ", " << y << ") = " << (int)out(x, y) << " instead of " << correct;
        }
    }
}

TEST_F(WideningReductionTest, TupleReductionWidening) {
...
TEST_F(WideningReductionTest, UnalignedOutput) {
    ...
    for (int y = 1; y < H - 1; y++) {
        for (int x = 0; x < W - 3; x++) {
            ...
            EXPECT_EQ(correct, out(x, y)) << "out(" << x << ", " << y << ") = " << (int)out(x, y) << " instead of " << (int)correct;
        }
    }
}
