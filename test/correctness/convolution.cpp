...
int main(int argc, char **argv) {
    ...
    if (target.has_gpu_feature()) {
        ...
        blur1.gpu_tile(x, y, 16, 16, Device_Default_GPU);

        // Summation is done as an outermost loop on the cpu
        blur1.update().reorder(x, y, r.x, r.y).gpu_tile(x, y, 16, 16, Device_Default_GPU);

        // Summation is done as a sequential loop within each gpu thread
        blur2.gpu_tile(x, y, 16, 16, Device_Default_GPU);
    } else {
