...
int main(int argc, char **argv) {
    ...
    if (target.has_gpu_feature()) {
        ...
        int vector_size = target.has_feature(Target::HVX_128) ? 64 : 32;
        // Take this opportunity to test scheduling the pure dimensions in a reduction
        Var xi("xi"), yi("yi");
        blur1.hexagon().tile(x, y, xi, yi, 6, 6);
        blur1.update().hexagon().tile(x, y, xi, yi, vector_size, 4).vectorize(xi).parallel(y);

        blur2.hexagon().vectorize(x, vector_size).parallel(y);
    } else {
        // Take this opportunity to test scheduling the pure dimensions in a reduction
        Var xi("xi"), yi("yi");
