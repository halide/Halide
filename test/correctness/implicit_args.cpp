...
namespace {
class ImplicitArgsTest : public ::testing::Test {
protected:
    Var x{"x"}, y{"y"}, z{"z"}, w{"w"};
    Buffer<int> test_image;
    void SetUp() override {
        test_image = lambda(x, y, z, w, (x << 24) | (y << 16) | (z << 8) | w)
                         .realize({10, 10, 10, 10});
    }
};
}  // namespace

TEST_F(ImplicitArgsTest, BasicImplicitArguments) {
    ImageParam im1(Int(32), 3);
    ASSERT_EQ(im1.dimensions(), 3);
    ...
    ASSERT_EQ(im2.dimensions(), 2);
    EXPECT_EQ(im2(4, 6), 10);
    // im2 is a 2d image
        ...
        for (int j = 0; j < 2; j++) {
            for (int i = 0; i < 2; i++) {
                for (int x = 0; x < 2; x++) {
                    ...
                    EXPECT_EQ(result1(x, i, j, k), correct)
                        << "result1(" << x << ", " << i << ", " << j << ", " << k << ")";
                }
            }
        }
...
TEST_F(ImplicitArgsTest, NestedImplicitArguments) {
    ImageParam im1(Int(32), 3);
    Buffer<int> im1_val = lambda(x, y, z, x * y * z).realize({10, 10, 10});
    im1.set(im1_val);
    Buffer<int> im2 = lambda(x, y, x + y).realize({10, 10});
...
TEST_F(ImplicitArgsTest, TransposeLastTwo) {
    Func transpose_last_two;
    transpose_last_two(_, x, y) = test_image(_, y, x);
        ...
        for (int j = 0; j < 10; j++) {
            for (int k = 0; k < 10; k++) {
                for (int l = 0; l < 10; l++) {
                    ...
                    EXPECT_EQ(transposed(i, j, k, l), correct)
                        << "transposed(" << i << ", " << j << ", " << k << ", " << l << ")";
                }
            }
        }
...
TEST_F(ImplicitArgsTest, HairyTranspose) {
    Func hairy_transpose;
    hairy_transpose(_, x, y) = test_image(y, _, x) + test_image(y, x, _);
        ...
        for (int j = 0; j < 10; j++) {
            for (int k = 0; k < 10; k++) {
                for (int l = 0; l < 10; l++) {
                    ...
                    EXPECT_EQ(hairy_transposed(i, j, k, l), correct)
                        << "hairy_transposed(" << i << ", " << j << ", " << k << ", " << l << ")";
                }
            }
        }
    }
...
TEST_F(ImplicitArgsTest, HairyTranspose2) {
    Func hairy_transpose2;
    hairy_transpose2(_, x) = test_image(_, x) + test_image(x, x, _);
        ...
        for (int j = 0; j < 10; j++) {
            for (int k = 0; k < 10; k++) {
                for (int l = 0; l < 10; l++) {
                    ...
                    int correct = correct1 + correct2;
                    EXPECT_EQ(hairy_transposed2(i, j, k, l), correct)
                        << "hairy_transposed2(" << i << ", " << j << ", " << k << ", " << l << ")";
                }
            }
        }
    }
