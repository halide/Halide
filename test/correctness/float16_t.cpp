...
int run_test() {
    ...
    {
        for (double f : {1.0, -1.0, 0.235, -0.235, 1e-7, -1e-7}) {
            {
                // Test double -> float16 doesn't have double-rounding issues
                float16_t k{f};
                float16_t k_plus_eps = float16_t::make_from_bits(k.to_bits() + 1);
                const bool k_is_odd = k.to_bits() & 1;
                float16_t to_even = k_is_odd ? k_plus_eps : k;
                ...
                // odd if rounding directly from double, but rounding via
                // float does the wrong thing.
                double halfway_plus_eps = std::nextafter(halfway, (double)to_odd);
                assert(std::abs(halfway_plus_eps - (double)to_odd) <
                ...
                // Now test the same thing in generated code. We need strict float to
                // prevent Halide from fusing multiple float casts into one.
                Param<double> p;
                p.set(halfway_plus_eps);
                // halfway plus epsilon rounds to exactly halfway as a float
                assert(evaluate<float>(strict_float(cast(Float(32), p))) == halfway);
                // So if we go via float we get the even outcome, because
                // exactly halfway rounds to even
                assert(evaluate<float16_t>(strict_float(cast(Float(16), cast(Float(32), p)))) == to_even);
                // But if we go direct, we should go to odd, because it's closer
                assert(evaluate<float16_t>(strict_float(cast(Float(16), p))) == to_odd);
            }

            {
                // Test the same things for bfloat
                bfloat16_t k{f};
                bfloat16_t k_plus_eps = bfloat16_t::make_from_bits(k.to_bits() + 1);
                const bool k_is_odd = k.to_bits() & 1;

                bfloat16_t to_even = k_is_odd ? k_plus_eps : k;
                bfloat16_t to_odd = k_is_odd ? k : k_plus_eps;
                float halfway = (float(k) + float(k_plus_eps)) / 2.f;

                assert(bfloat16_t(halfway) == to_even);
                double halfway_plus_eps = std::nextafter(halfway, (double)to_odd);
                assert(std::abs(halfway_plus_eps - (double)to_odd) <
                       std::abs(halfway_plus_eps - (double)to_even));

                assert(float(halfway_plus_eps) == halfway);
                assert(bfloat16_t(halfway_plus_eps) == to_odd);

                Param<double> p;
                p.set(halfway_plus_eps);
                assert(evaluate<float>(strict_float(cast(Float(32), p))) == halfway);
                assert(evaluate<bfloat16_t>(strict_float(cast(BFloat(16), cast(Float(32), p)))) == to_even);
                assert(evaluate<bfloat16_t>(strict_float(cast(BFloat(16), p))) == to_odd);
            }
        }
    }
    ...
    if ((false)) {  // Intentional dead code. Extra parens to pacify clang-tidy.
        ...
        src.compute_root().vectorize(x, 16, TailStrategy::RoundUp);
        to_f16.compute_root().vectorize(x, 16, TailStrategy::RoundUp);
        from_f16.compute_root().vectorize(x, 16, TailStrategy::RoundUp);
