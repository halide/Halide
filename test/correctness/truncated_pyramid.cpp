...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < levels; i++) {
        ...
        pyr_down[i].compute_root().specialize(max(width, height) > 32).vectorize(x, 16).parallel(y, 16, TailStrategy::GuardWithIf);
    }
    ...
    for (int i = levels - 2; i >= 0; i--) {
        ...
        pyr_up[i].compute_root().specialize(max(width, height) > 32).vectorize(x, 16).parallel(y, 16, TailStrategy::GuardWithIf);
    }
