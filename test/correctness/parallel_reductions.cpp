...
TEST(ParallelReductionsTest, HierarchicalSum) {
    ...
    Buffer<int> im;
    ASSERT_NO_THROW(im = f2.realize());

    int correct = (256 * 255) / 2;
    EXPECT_EQ(im(0), correct);
}

TEST(ParallelReductionsTest, ParallelPrefixSum) {
    Var i, j;
    ...
    Buffer<int> result;
    ASSERT_NO_THROW(result = out.realize({256}));

    for (int i = 0; i < 256; i++) {
        EXPECT_EQ(result(i), correct(i)) << "i = " << i;
    }
}
