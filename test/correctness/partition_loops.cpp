...
TEST(PartitionLoopsTest, PartitionConditions) {
    ...
    Buffer<float> im;
    ASSERT_NO_THROW(im = output.realize({1024, 1024, 3}));

    for (int y = 0; y < input.height(); y++) {
        for (int x = 0; x < input.width(); x++) {
            for (int c = 0; c < input.channels(); c++) {
                float correct = (input(std::min(2 * x, input.width() - 1), y, 2) < x + y) ? x + y : y + c;
                EXPECT_EQ(im(x, y, c), correct) << "im(" << x << ", " << y << ", " << c << ") = " << im(x, y, c) << " instead of " << correct;
            }
        }
    }
}

TEST(PartitionLoopsTest, LoopPartitioningBug7742) {
    ...
    Pipeline p(f);
    ASSERT_NO_THROW(p.realize({100, 100}));
}
