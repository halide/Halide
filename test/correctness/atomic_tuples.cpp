...
class AtomicTuplesTest : public ::testing::Test {
protected:
    ...
    static void check_result(const Buffer<int> &out, const std::function<int(int, int)> &reference) {
        for (int y = 0; y < out.height(); y++) {
            for (int x = 0; x < out.width(); x++) {
                int correct = reference(x, y);
...
TEST_F(AtomicTuplesTest, IndependentTupleComponents) {
    Func f, g;
    Var x, y;
    ...
    check_result(out, [](int x, int y) { return 2 * x + 2 * y; });

    EXPECT_EQ(checker.count_atomics, 2) << "Expected two atomic nodes";
    EXPECT_EQ(checker.count_atomics_with_mutexes, 0) << "Expected no mutexes";
}

TEST_F(AtomicTuplesTest, CoupledTupleComponents) {
    Func f, g;
    Var x, y;
    ...
    EXPECT_EQ(checker.count_atomics, 1) << "Expected one atomic node";
    EXPECT_EQ(checker.count_atomics_with_mutexes, 1) << "Expected one mutex";
}

TEST_F(AtomicTuplesTest, ThreeComponentTuple) {
    Func f, g;
    Var x, y;
    ...
    Buffer<int> out = g.realize({128, 128});
    check_result(out, [](int x, int y) { return 2 * x + 2 * y + 1; });

    EXPECT_EQ(checker.count_atomics, 1) << "Expected one atomic node";
    EXPECT_EQ(checker.count_atomics_with_mutexes, 1) << "Expected one mutex";
}

TEST_F(AtomicTuplesTest, FourComponentTupleWithCoupling) {
    Func f, g;
    Var x, y;
    ...
    Buffer<int> out = g.realize({128, 128});
    check_result(out, [](int x, int y) { return 4 * x + 4 * y; });

    EXPECT_EQ(checker.count_atomics, 1) << "Expected one atomic node";
    EXPECT_EQ(checker.count_atomics_with_mutexes, 1) << "Expected one mutex";
}

TEST_F(AtomicTuplesTest, EvenOddSplitWithMultipleUpdates) {
    Func f, g;
    Var x, y;
    ...
    EXPECT_EQ(checker.count_atomics, 4) << "Expected four atomic nodes";
    EXPECT_EQ(checker.count_atomics_with_mutexes, 0) << "Expected no mutexes";
}
