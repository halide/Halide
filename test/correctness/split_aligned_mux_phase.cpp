...
// LetStmt and the offset is a runtime value (see the aligned_split() rewrite
// rule in Simplify_Add.cpp). And the vectorized loop has to be deinterleaved
// by the period, so that each of the resulting slices has a phase of its own
// (see Deinterleave.cpp).
