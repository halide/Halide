...
int main(int argc, char **argv) {
    ...
    // Test calls into the wrappers that accept const-ref for inputs, pointers for output
    {
        const Buffer<int32_t> &i1 = input1;
        const Buffer<int32_t> i2 = input2;
        result = constinput(i1, i2, &output);
        verify(result, output);
    }
    ...
    // with Buffer<const T> for inputs
    {
        const Buffer<const int32_t> i1 = input1;
        const Buffer<const int32_t> &i2 = input2;
        result = constinput(i1, i2, &output);
        verify(result, output);
    }
