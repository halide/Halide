...
void test_threads(int num_halide_threads, int num_cpp_threads) {
    ...
    if (num_halide_threads + num_cpp_threads > 16) {
        std::cout << "[WASM] Skipping num_halide_threads=" << num_halide_threads << " called from " << num_cpp_threads << " C++ threads\n";
        return;
    }
#endif
...
#ifdef __EMSCRIPTEN__
    // Wasm runtimes often severely limits the number of threads we can spawn, so dial it down
    constexpr int max_cpp_threads = 2;
#else
