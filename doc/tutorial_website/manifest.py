...
generate step, so doc/CMakeLists.txt writes them out via file(GENERATE) with
$<TARGET_FILE:...> rather than us trying to compute them ourselves.
"""
...
def load_python_env(path: Path) -> PythonEnv:
    """WITH_DOCS DEPENDS on WITH_PYTHON_BINDINGS (see the top-level
    CMakeLists.txt), so this file always exists and always names a real
    interpreter -- there's no "Python bindings weren't built" case to
