...
AOT usage are supported via the `vulkan` target flag (e.g.
...
[Getting Started Guide](https://vulkan.lunarg.com/doc/view/latest/linux/getting_started_ubuntu.html).
...
present. Specifically, the seemingly generic `mesa-vulkan-drivers` actually
...
Alternatively, if you have the [Homebrew](https://brew.sh/) package manager
...
Vulkan loader library (e.g. `/usr/local/lib64/libvulkan.so.1`). This can be
...
supports the given list of layers. If not set, `VK_INSTANCE_LAYERS=...` will be
...
environment variable list delimiter (`:` on Linux/OSX/Posix, or `;` on Windows).
...
the appropriate environment variable list delimiter (e.g. `N:N:N:N:N` on
Linux/OSX/Posix, or `N;N;N;N;N` on Windows). These values correspond to
...
be injected by adding `VK_LAYER_KHRONOS_validation` to the `VK_INSTANCE_LAYERS=`
