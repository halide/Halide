...
#     level_heights = [Param(int_t,'level_heights%d'%i) for i in range(levels)]
    ...
    if sched == 0:
        ...
        yo, yi, xo, xi, tx, ty, tc = Var('yo'), Var('yi'), Var('xo'), Var('xi'), Var("tx"), Var("ty"), Var("tc")
        final.reorder(c, x, y).bound(c, 0, 3).vectorize(x, 4)
        final.tile(x, y, xo, yo, xi, yi, input.width()/4, input.height()/4)
        normalize.compute_at(final, xo).reorder(c, x, y).gpu_tile(x, y, tx, ty, 16, 16, GPU_Default).unroll(c)
        ...
        for l in range(1, levels):
            ...
            downsampled[l].compute_root().gpu_tile(x, y, c, tx, ty, tc, tile_size, tile_size, 4, GPU_Default)
            interpolated[l].compute_at(final, xo).gpu_tile(x, y, c, tx, ty, tc, tile_size, tile_size, 4, GPU_Default)
