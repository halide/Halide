    ...
    if target.has_gpu_feature():
        ...
        for j in range(J):
            ...
            if j > 3:
                ...
                blockh = 2
            if j > 0:
                inGPyramid[j].compute_root().gpu_tile(x, y, tx, ty, blockw, blockh, GPU_Default)
            if j > 0:
                gPyramid[j].compute_root().reorder(k, x, y).gpu_tile(x, y, tx, ty, blockw, blockh, GPU_Default)
            outGPyramid[j].compute_root().gpu_tile(x, y, tx, ty, blockw, blockh, GPU_Default)
    else:
        # CPU schedule
