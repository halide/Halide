...
function(_add_python_aot_and_stub_extension)
    ...
    if (ARG_SOURCES MATCHES ".py$")
        add_halide_generator(${GEN_TARGET} SOURCES ${ARG_SOURCES})
    else ()
        add_halide_generator(${GEN_TARGET} PYSTUB ${GEN_NAME} SOURCES ${ARG_SOURCES})
    endif ()
...
_add_python_aot_and_stub_extension(
    SOURCES addconstantcpp_generator.cpp
    GENERATORS
    addconstantcpp
...
_add_python_aot_and_stub_extension(
    SOURCES addconstantpy_generator.py
    GENERATORS
    addconstantpy
...
if (NOT MSVC)
    ...
    add_test(
        NAME test_export_single_symbol
        COMMAND
            $ENV{SHELL}
            -c "[ $(nm $<TARGET_FILE:pyext_multi_method_module> | grep ' T ' | wc -l) -eq 1 ] && \
